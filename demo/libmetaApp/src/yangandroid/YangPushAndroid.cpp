...
YangPushAndroid::YangPushAndroid(YangContext* pcontext) {
	m_context=pcontext;
	//m_context->streams.m_playBuffer=new YangSynBuffer();

	m_context->nativeWindow=NULL;//pwindow;
	m_push = NULL;
...
void YangPushAndroid::startCamera(){
	m_push = new YangPushHandleImpl(true,false,Yang_VideoSrc_Camera,NULL,NULL,m_context,this);
