...
int32_t yang_ipc_startRtc(YangIpcSession* session,char* remoteIp,char* sdp,char* response ) {
	...
	yang_ipc_initRtc2(session,hasplay?yangtrue:yangfalse);
	return err;
}
...
void yang_create_ipc(YangIpc* ipc){
	...
	yang_getLocalInfo(session->avinfo.sys.familyType,session->avinfo.sys.localIp);
	...
	if(session->avinfo.rtc.iceUsingLocalIp){
		...
		yang_getIp(session->avinfo.sys.familyType,ipstr,session->avinfo.rtc.iceLocalIP);
	}
