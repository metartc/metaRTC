...
int32_t YangH264EncoderSoft::encode(YangFrame* pframe, YangEncoderCallback* pcallback) {
	...
	for (int32_t i = 0; i < m_i264Nal; i++) {
		...
		nal_len = 4;
		nalusize=m_264Nal[i].i_payload;
		if(m_nal[2]==0x01){
			nalutype = m_nal[3] & kNalTypeMask;
			nal_len=3;
		}
		if ( nalutype == YangAvcNaluTypeSPS || nalutype == YangAvcNaluTypePPS)
				isKeyFrame = true;


		if (isKeyFrame) {
			yang_put_be32((char*) (m_vbuffer + destLength),1);
			memcpy(m_vbuffer + destLength + 4, m_nal + nal_len,	nalusize - nal_len);
			destLength += (nalusize - nal_len + 4);
		} else {
			memcpy(m_vbuffer + destLength, m_nal + nal_len,	nalusize - nal_len);
			destLength += (nalusize - nal_len);
		}
