...
void YangVideoDecoderHandles::addVideoStream(YangVideoCodec videoCodec,uint8_t *ps, int32_t puid,
		int32_t pisAdd) {
	if (pisAdd) {

		YangDecoderFactory df;
		m_decs->push_back(df.createVideoDecoder(videoCodec, &m_context->avinfo.video));
		m_decs->back()->m_uid = puid;
		m_decs->back()->init();
		int32_t wid = 0, hei = 0, fps = 10;

		m_decs->back()->parseHeader(ps, 128, &wid, &hei, &fps);
