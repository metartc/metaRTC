...
void YangVideoDecoderFfmpeg::parseHeader(uint8_t *p, int32_t pLen, int32_t *pwid,
	...
	if(m_encDecType==Yang_VED_H264){
		...
		yang_meta_parseH265Header(p, pLen,headers, &headerLen);
		yang_meta_getH265Sps(p,pLen,&sps,&spsLen);
		yang_h265_decode_sps(sps,spsLen,(uint32_t*)pwid,(uint32_t*)phei);
		*pfps=30;
	}
...
void YangVideoDecoderFfmpeg::init() {
	...
	if(m_encDecType==Yang_VED_H264){
		m_codec = (AVCodec*)yang_avcodec_find_decoder(AV_CODEC_ID_H264);
	}else if(m_encDecType==Yang_VED_H265){
		m_codec = (AVCodec*)yang_avcodec_find_decoder(AV_CODEC_ID_H265);
	}
