...
void YangDecoderMediacodec::parseHeader(uint8_t *p, int32_t pLen, int32_t *pwid,
		int32_t *phei, int32_t *pfps) {

	uint8_t headers[128];
	uint8_t spsHeader[128];
	yang_memset(headers, 0, 128);
	yang_memset(spsHeader, 0, 128);
	int32_t headerLen = 0;
	if(m_encDecType==Yang_VED_H264){
		uint8_t* sps=NULL;
		int32_t spsLen=0;
		yang_meta_parseH264Header(p, pLen,headers, &headerLen);
		yang_meta_getH264Sps(p,pLen,&sps,&spsLen);
		yang_memcpy(spsHeader,sps,spsLen);
		yang_h264_decode_spspps(spsHeader,spsLen,pwid,phei,pfps);
	...
	yang_meta_parseH264(&meta,p,pLen);
	YangSample sps2,sps;
	YangSample pps2,pps;

	sps.bytes=(char*)yang_calloc(meta.spsLen+4,1);
	pps.bytes=(char*)yang_calloc(meta.ppsLen+4,1);

	sps.bytes[3]=0x01;
	pps.bytes[3]=0x01;
	memcpy(sps.bytes+4,headers+meta.spsPos,meta.spsLen);
	memcpy(pps.bytes+4,headers+meta.ppsPos,meta.ppsLen);
	sps.nb=meta.spsLen+4;
	pps.nb=meta.ppsLen+4;
