...
void YangVideoDecoderHandle::addVideoStream(YangVideoCodec videoCodec,uint8_t *ps, int32_t puid,
		int32_t pisAdd) {    
	if (pisAdd) {

		YangDecoderFactory df;
		m_decs=df.createVideoDecoder(videoCodec, &m_context->avinfo.video);
		m_decs->m_uid = puid;
		m_decs->init();
		int32_t wid = 0, hei = 0, fps = 10;

		m_decs->parseHeader(ps, 128, &wid, &hei, &fps);
...
void YangVideoDecoderHandle::startLoop() {
	...
	while (m_isConvert == 1) {
		...
		if (videoFrame.frametype == YANG_Frametype_I&&m_decs == NULL) {
			addVideoStream((YangVideoCodec)videoFrame.mediaType,temp, videoFrame.uid, 1);
		}
