...
static void yang_initHeader(YangEncoderMacSession *session,CMSampleBufferRef sampleBuffer){
    ...
    yang_put_be32((char*)session->buffer,1);
	yang_memcpy(session->buffer+4,sps, spsLen);
    yang_put_be32((char*)session->buffer+spsLen+4,1);
...
static int32_t yang_ios_enc_init(void* psession,YangVideoInfo* video,YangVideoEncInfo* enc,YangCodecCallback* callback){
	...
	// 设置码率
    VTSessionSetProperty(session->encoder, kVTCompressionPropertyKey_AverageBitRate, (__bridge CFTypeRef)@(video->rate));
    VTSessionSetProperty(session->encoder, kVTCompressionPropertyKey_DataRateLimits, (__bridge CFTypeRef)@[@(video->rate/8), @1.0]);
	// 设置帧率
