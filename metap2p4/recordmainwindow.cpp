...
#include <QSettings>
void g_qt_p2p_receiveData(void* context,char* data,int32_t nb){
    RecordMainWindow* win=(RecordMainWindow*)context;

    win->setRecvText(data,nb);
}
...
{   
    if(m_context->channeldataSend.sendData)
        m_context->channeldataSend.sendData(m_context->channeldataSend.context,ui->m_t_data->text().toLatin1().data(),strlen(ui->m_t_data->text().toLatin1().data()));
