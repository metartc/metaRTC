...
void YangVideoDecoderHandles::addVideoStream(uint8_t *ps, int32_t puid,
		int32_t pisAdd) {
	if (pisAdd) {
		YangVideoEncDecType aet = ps[0] == 0x17 ? Yang_VED_264 : Yang_VED_265;
		YangDecoderFactory df;
		m_decs->push_back(df.createVideoDecoder(aet, &m_context->video));
		...
		m_out_videoBuffer->push_back(
				new YangVideoBuffer(m_context->video.bitDepth == 8 ? 1 : 2));
		m_out_videoBuffer->back()->m_uid = puid;
		m_out_videoBuffer->back()->init(wid, hei, m_context->video.videoDecoderFormat);
...
void YangVideoDecoderHandles::startLoop() {
    ...
    //int32_t isFFmpeg = 1;		//(m_context->videoDecoderType == 1 ? 1 : 0);
	int32_t index = 0;
