...
//int32_t g_m_fx2=2;
void YangFfmpegEncoderMeta::yang_find_next_start_code(YangVideoEncDecType pve,uint8_t *buf,int32_t bufLen,int32_t *vpsPos,int32_t *vpsLen,int32_t *spsPos,int32_t *spsLen,int32_t *ppsPos,int32_t *ppsLen)
{
   int32_t i = 0;
 //  printf("\n**********************extradate.....=%d\n",bufLen);
...
void YangFfmpegEncoderMeta::yang_getSpsPps(YangH2645Conf *pconf,
	...
	m_lib.loadObject("libavcodec");
		...
		loadLib();
#endif
		YangVideoEncDecType m_encoderType=(YangVideoEncDecType)p_yvp->videoEncoderType;
