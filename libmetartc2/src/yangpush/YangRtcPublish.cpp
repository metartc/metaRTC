...
int32_t YangRtcPublish::init(int32_t nettype, string server, string localIp,int32_t localPort,int32_t pport,
		...
		if (m_pushs.size() == 0) {
				m_pushs.push_back(sf.createStreamHandle(m_context->avinfo.sys.transType,0,m_context));
...
void YangRtcPublish::startLoop() {
	...
	data.initAudio(m_context->avinfo.sys.transType,m_context->avinfo.audio.sample, m_context->avinfo.audio.channel,
			(YangAudioCodec) m_context->avinfo.audio.audioEncoderType);
	data.initVideo(m_context->avinfo.sys.transType);
