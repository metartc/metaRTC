...
void YangPushPublish::setVideoInfo(YangVideoInfo* pvideo){
	...
	if(m_captureType==Yang_VideoSrc_OutInterface) {
		if(m_outVideoBuffer==NULL) m_outVideoBuffer=new YangVideoBuffer(pvideo->width,pvideo->height,pvideo->videoEncoderFormat,m_context->video.bitDepth==8?1:2);
		if(m_outPreVideoBuffer==NULL) m_outPreVideoBuffer=new YangVideoBuffer(pvideo->width,pvideo->height,pvideo->videoEncoderFormat,m_context->video.bitDepth==8?1:2);
	}
}
