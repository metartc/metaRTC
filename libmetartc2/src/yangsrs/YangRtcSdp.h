...
#include <yangsrs/YangMediaDesc.h>
...
public:
	...
	int32_t genLocalSdp(YangSdp* local_sdp,int32_t localport,struct YangRtcContext* conf, char *sdp,YangStreamOptType role);
	void init(struct YangCertificate* pcer);

private:
	struct YangCertificate* m_cer;
	int32_t generate_local_sdp(YangSdp* local_sdp,struct YangRtcContext* pconf, bool unified_plan,int32_t role);
