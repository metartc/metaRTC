...
public:
	...
	int32_t genLocalSdp(YangSdp* local_sdp,int32_t localport,YangRtcSession* session, char *sdp,YangStreamOptType role);
	void init(YangCertificate* pcer);

private:
	YangCertificate* m_cer;
	int32_t generate_local_sdp(YangSdp* local_sdp,YangRtcSession* session, bool unified_plan,int32_t role);
