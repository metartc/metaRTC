...
void YangSdpHandle::init(struct YangCertificate* pcer){
	m_cer=pcer;
}
int32_t YangSdpHandle::genLocalSdp(YangSdp* local_sdp,int32_t localport,struct YangRtcContext* pconf, char *sdp,YangStreamOptType role) {
	//char s[1024 * 10] = { 0 };
	/**FILE *f = NULL;
	...
	yang_replace(ss,"127.0.0.1 8000",string(pconf->streamConf->serverIp)+" "+to_string(localport));
	...
	generate_local_sdp(local_sdp,pconf, 1, role);
	stringstream os;
	int32_t err = 0;
...
int32_t YangSdpHandle::generate_local_sdp(YangSdp* local_sdp,struct YangRtcContext* pconf, bool unified_plan, int32_t role) {
	int32_t err = Yang_Ok;
	...
	std::string stream_id = string(pconf->streamConf->app) + "/" + string(pconf->streamConf->stream);
