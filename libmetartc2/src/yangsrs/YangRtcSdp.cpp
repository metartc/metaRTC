...
#include <sstream>
#include <stdarg.h>
#include <yangsrs/YangLocalH264Sdp.h>
#include <yangsrs/YangLocalH265Sdp.h>
#include <yangsrs/YangMediaPayloadType.h>
#include <queue>
using namespace std;
...
int32_t YangSdpHandle::genLocalSdp(YangSdp* local_sdp,int32_t localport,YangRtcSession* session, char *sdp,YangStreamOptType role) {
	//char s[1024 * 10] = { 0 };
	/**FILE *f = NULL;
	...
	//printf("\n******************************gensdp localport==%d\n",localport);
	string ss;
	if(session->context->context->video->videoEncoderType==Yang_VED_264){
		ss=local_h264_sdp_str;
	}
#if	Yang_H265_Encoding
	if(session->context->context->video->videoEncoderType==Yang_VED_265){
	...
	yang_replace(ss,"144706935",to_string(session->context->audioSsrc));
	...
	yang_replace(ss,"144706936",to_string(session->context->videoSsrc));
	yang_replace(ss,"127.0.0.1 8000",string(session->context->streamConf->serverIp)+" "+to_string(localport));
	if(session->context->context&&session->context->context->audio->audioEncoderType==Yang_AED_OPUS){
		char str1[64]={0};
		sprintf(str1,"opus/%d/%d",session->context->context->audio->sample,session->context->context->audio->channel);
	...
	local_sdp->set_ice_ufrag(yang_random_str(4));
	local_sdp->set_ice_pwd(yang_random_str(32));
	local_sdp->set_fingerprint_algo("sha-256");
	local_sdp->set_fingerprint(m_cer->fingerprint);
	local_sdp->m_session_negotiate = local_sdp->m_session_config;
	local_sdp->m_session_negotiate.m_dtls_role = "actpass"; //"passive","active";
	local_sdp->set_dtls_role(local_sdp->m_session_negotiate.m_dtls_role);


	generate_local_sdp(local_sdp,session, 1, role);
	stringstream os;
	int32_t err = 0;
	if ((err = local_sdp->encode(os)) != Yang_Ok) {
...
int32_t YangSdpHandle::generate_local_sdp(YangSdp* local_sdp,YangRtcSession* session, bool unified_plan, int32_t role) {
	int32_t err = Yang_Ok;

	local_sdp->m_version = "0";

	local_sdp->m_username = "YangRtc";
	local_sdp->m_session_id = yang_int2str(((int64_t) this));
	...
	local_sdp->m_session_name = "YangRtcSession";

	local_sdp->m_msid_semantic = "WMS";
	std::string stream_id = string(session->context->streamConf->app) + "/" + string(session->context->streamConf->stream);
	local_sdp->m_msids.push_back(stream_id);

	local_sdp->m_group_policy = "BUNDLE";

	//std::string cname = srs_random_str(16);
	YangMediaDesc *audio_media_desc = local_sdp->find_media_descs("audio");//new SrsMediaDesc("audio");//
	...
	YangMediaDesc *video_media_desc =local_sdp->find_media_descs("video"); //SrsMediaDesc("video");
