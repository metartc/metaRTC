...
YangRtcDtls::YangRtcDtls(YangRtcContext* pcontext) {
	m_context=pcontext;
	m_sslctx = NULL;
	m_ssl = NULL;
	m_bioIn = NULL;
...
int32_t YangRtcDtls::on_handshake_done()
{
    ...
    char send_key[50];
     ...
     if ((err = get_srtp_key(recv_key, &recvKeyLen,send_key,&sendKeyLen)) != Yang_Ok) {
      ...
      if ((err = yang_init_srtp(&m_context->srtp,recv_key,recvKeyLen, send_key,sendKeyLen)) != Yang_Ok) {
...
SSL_CTX* YangRtcDtls::yang_build_dtls_ctx(YangDtlsVersion version, std::string role) {
 ...
 if (version == YangDtlsVersion1_2) {
	...
	if (m_cer->ecdsa_mode) { // By ECDSA, https://stackoverflow.com/a/6006898
#if OPENSSL_VERSION_NUMBER >= 0x10002000L // v1.0.2
	...
	if (true) {
        ...
        if(SSL_CTX_use_certificate(dtls_ctx,m_cer->dtls_cert) != 1) yang_error("SSL_CTX_use_certificate error");
        //yang_assert(SSL_CTX_use_PrivateKey(dtls_ctx,	m_cer->get_public_key()) == 1);
        if(SSL_CTX_use_PrivateKey(dtls_ctx,	m_cer->dtls_pkey) != 1)  yang_error("SSL_CTX_use_PrivateKey error");
