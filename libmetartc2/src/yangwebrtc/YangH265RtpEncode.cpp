...
YangH265RtpEncode::YangH265RtpEncode() {
    m_videoSsrc = 0;
    m_videoSeq = 0;
    m_audioSeq = 0;
    m_audioSsrc = 0;
...
void YangH265RtpEncode::init(YangRtpBuffer *rtpBuffer,
                             YangRtcSessionI *psession) {
    m_rtpBuffer = rtpBuffer;
    m_session = psession;
}
int32_t YangH265RtpEncode::on_audio(YangStreamCapture *audioFrame) {
    int err = 0;
...
int32_t YangH265RtpEncode::on_h265_video(YangStreamCapture *videoFrame) {
    int32_t err = Yang_Ok;
...
int32_t YangH265RtpEncode::package_single_nalu(YangStreamCapture *videoFrame) {
    int32_t err = Yang_Ok;
...
int32_t YangH265RtpEncode::package_single_nalu(char *p, int32_t plen,
                                               int64_t timestamp) {
...
int32_t YangH265RtpEncode::package_fu_a(YangStreamCapture *videoFrame,
    ...
    for (int32_t i = 0; i < num_of_packet; ++i) {
        ...
        memset(&m_videoFua2Data,0,sizeof(YangFua2H265Data));
        m_videoFua2Data.nri = (YangHevcNaluType) header;
        m_videoFua2Data.nalu_type = (YangHevcNaluType) nal_type;
...
int32_t YangH265RtpEncode::encodeVideo(YangRtpPacket *pkt) {
    int err = 0;
    ...
    if (pkt->m_payload_type == YangRtspPacketPayloadTypeRaw) {
    	err=yang_encode_h264_raw(&m_buf,&m_videoRawData);
    	 ...
    	 err = yang_encode_h265_fua2(&m_buf,&m_videoFua2Data);

    } else if (pkt->m_payload_type == YangRtspPacketPayloadTypeSTAP) {
    	 err = yang_encode_h265_stap(&m_buf,&m_stapData);
    	 yang_reset_h2645_stap(&m_stapData);
    }
...
int32_t YangH265RtpEncode::encodeAudio(YangRtpPacket *pkt) {
    int err = 0;
    m_buf.init(m_rtpBuffer->getBuffer(), kRtpPacketSize);
...
int32_t YangH265RtpEncode::package_stap_a(YangStreamCapture *videoFrame) {
    int err = Yang_Ok;
    //uint8_t *buf = videoFrame->getVideoData();
//    int32_t vpsLen = *(buf + 12) + 1;
//    uint8_t *vps;
...
//    uint8_t *pps = buf + 13 + spsLen + 2;
    ...
    yang_decodeMetaH265(videoFrame->getVideoData(),videoFrame->getVideoLen(),vps_sample,sps_sample,pps_sample);
    yang_reset_h2645_stap(&m_stapData);
    uint8_t header = (uint8_t)sps_sample->bytes[0];
    m_stapData.nri =  header;
    if(m_stapData.nalus) m_stapData.nalus->push_back(vps_sample);
