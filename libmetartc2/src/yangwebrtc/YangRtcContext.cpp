...
YangRtcContext::YangRtcContext() {
	...
	audioSsrc=yang_generate_ssrc();
	videoSsrc=yang_generate_ssrc();
     yang_trace("\naudiossrc===%u,videossrc==%u..........",audioSsrc,videoSsrc);
	local_sdp=new YangSdp();
	remote_sdp=new YangSdp();
	state=0;
	memset(&srtp,0,sizeof(srtp));
	cer=(struct YangCertificate*)malloc(sizeof(struct YangCertificate));
	memset(cer,0,sizeof(struct YangCertificate));
	yang_init_certificate(cer);
