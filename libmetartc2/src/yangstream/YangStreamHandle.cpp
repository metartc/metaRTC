...
void YangStreamHandle::init(struct YangStreamConfig* pconf){
		memset(&m_conf,0,sizeof(struct YangStreamConfig));

		memset(m_conf.localIp,0,sizeof(m_conf.localIp));

		strcpy(m_conf.serverIp,pconf->serverIp);
		strcpy(m_conf.localIp,pconf->localIp);
		strcpy(m_conf.app,pconf->app);
		strcpy(m_conf.stream,pconf->stream);
