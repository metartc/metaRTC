...
int32_t YangIni::readStringValue(const char *section, const char *key,
...
int32_t YangIni::readStringValue1(const char *section, const char *key,
		char *val) {
	char sect[SECTION_MAX_LEN];
	if (section == NULL || key == NULL || val == NULL || m_file == NULL) {
		printf("%s: input parameter(s) is NULL!\n", __func__);
...
int32_t YangIni::readIntValue(const char *section, const char *key,
		int32_t p_defaultInt) {
