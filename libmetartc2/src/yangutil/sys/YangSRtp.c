#include <yangutil/sys/YangCSRtp.h>
...
int32_t yang_init_srtp(struct YangSRtp* srtp,char* recv_key,int precvkeylen, char* send_key,int psendkeylen)
{
    ...
    uint8_t *skey = (uint8_t *)malloc(psendkeylen);//new uint8_t[psendkeylen];
    ...
    uint8_t *rkey = (uint8_t *)malloc(precvkeylen);//new uint8_t[precvkeylen];
