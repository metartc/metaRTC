...
static void yang_initParam(YangAudioResampleContext* context) {
	if(context==NULL)
		return;
...
static void yang_initIn(YangAudioResampleContext* context,int32_t psample, int32_t pchannel) {
	if(context==NULL)
		return;
...
static void yang_initOut(YangAudioResampleContext* context,int32_t psample, int32_t pchannel) {
	if(context==NULL)
		return;
...
static void yang_init(YangAudioResampleContext* context,int32_t insample, int32_t inchannel, int32_t outsample,
	...
	yang_initParam(context);
}

static void yang_initTrans(YangAudioResampleContext* context) {
	if(context==NULL)
		return;
	...
	if(context->channelTrans){
		if (context->inBuf == NULL)
			context->inBuf =(uint8_t*) yang_calloc( context->inFrames<<2,1);
	}

	if(context->sampleTrans){
		if (context->outBuf == NULL)
			context->outBuf = (uint8_t*) yang_calloc( context->outFrames<<2,1);
...
static int32_t yang_resample(YangAudioResampleContext* context,YangFrame *audioFrame) {
	uint8_t *buf;
	uint8_t *buf_in;
    ...
    if (!context->isTrans||!audioFrame||!audioFrame->payload){
    	return Yang_Ok;
    }


	if (!context->isInitTrans){
		yang_initTrans(context);
        context->isInitTrans = 1;
	}

	buf = audioFrame->payload;
	buf_in=buf;
...
void yang_create_audioresample(YangAudioResample* res){
	...
	yang_initParam(res->context);
	res->init=yang_init;
	res->initIn=yang_initIn;
	res->initOut=yang_initOut;
