...
int32_t yang_get_sortBuffer_index(YangSortFrame** a, int64_t value,int32_t alen){
...
void yang_sortBuffer_putFrame(YangSortBufferSession* session,int64_t pts,YangFrame *pframe) {
	if (!pframe||session->vsize>=session->capacity)
		return;

	if(pts<=session->lastPts)
		return;
...
void yang_sortBuffer_removeFirst(YangSortBufferSession* session){
    if (session->vsize-session->cacheNum<1 || session->vsize<1){
    	return;
    }
...
YangFrame* yang_sortBuffer_getCurFrameRef(YangSortBufferSession* session) {
	if(session->vsize-session->cacheNum<1)
		return NULL;
