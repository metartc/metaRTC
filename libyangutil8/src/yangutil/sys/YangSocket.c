...
void yang_addr_copy(YangIpAddress* srcAddr,YangIpAddress* dstAddr,YangIpFamilyType familyType){
	if(dstAddr==NULL || srcAddr==NULL) return;

	dstAddr->familyType=srcAddr->familyType;

	if(familyType==Yang_IpFamilyType_IPV4){

		dstAddr->addr4.sin_family = srcAddr->addr4.sin_family;
		dstAddr->addr4.sin_port = srcAddr->addr4.sin_port;
#if  Yang_OS_WIN
		dstAddr->addr4.sin_addr.S_un.S_addr=srcAddr->addr4.sin_addr.S_un.S_addr;
#else
		dstAddr->addr4.sin_addr.s_addr = srcAddr->addr4.sin_addr.s_addr;
#endif
	}else{
		dstAddr->addr6.sin6_family = AF_INET6;
		dstAddr->addr6.sin6_port = srcAddr->addr6.sin6_port;
		yang_memcpy((char*)&dstAddr->addr6.sin6_addr,(char*)&srcAddr->addr6.sin6_addr,16);
	}
}

yangbool yang_addr_cmp(YangIpAddress* srcAddr,YangIpAddress* dstAddr){
	if(srcAddr->familyType==Yang_IpFamilyType_IPV4){

		if(srcAddr->addr4.sin_addr.s_addr==dstAddr->addr4.sin_addr.s_addr&&srcAddr->addr4.sin_port==dstAddr->addr4.sin_port)
			return yangtrue;
	}
	return yangfalse;
}
...
void yang_addr_updatePort(YangIpAddress* addr,int32_t port){
	if(addr->familyType==Yang_IpFamilyType_IPV6){
		addr->addr6.sin6_port = yang_htons(port);
		return;
	}

	addr->addr4.sin_port = yang_htons(port);
}
