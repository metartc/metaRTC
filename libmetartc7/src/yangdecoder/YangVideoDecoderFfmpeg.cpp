...
void YangVideoDecoderFfmpeg::parseRtmpHeader(uint8_t *p, int32_t pLen, int32_t *pwid,
	...
	if(m_encDecType==Yang_VED_H264){
		...
		yang_trace("\n>>>>>>>>>>>width==%d,height==%d",*pwid,*phei);
	}else if(m_encDecType==Yang_VED_H265) {
		yang_getH265RtmpHeader(p, headers, &headerLen);
		YangSample vps,sps2,pps;
		yang_decodeMetaH265(p,0,&vps,&sps2,&pps);
		yang_h265_decode_sps((uint8_t*)sps2.bytes,sps2.nb,(uint32_t*)pwid,(uint32_t*)phei);
	}
	m_width = *pwid;
	m_height= *phei ;
