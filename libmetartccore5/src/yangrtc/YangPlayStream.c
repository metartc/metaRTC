...
int32_t yang_rtcplay_on_rtp(YangRtcContext *context, YangRtcPlayStream *play,
	...
	audio_track = yang_rtcplay_get_audio_track(context, play,ssrc);
	video_track = yang_rtcplay_get_video_track(context,	play, ssrc);
	...
	if (audio_track) {
		...
		context->stats.recvStats.audio.rtpPacketCount++;
	} else if (video_track) {
		...
		context->stats.recvStats.video.rtpPacketCount++;
	} else {
