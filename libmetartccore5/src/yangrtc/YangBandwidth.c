...
int32_t yang_bandwidth_checkByRR(YangRemoteRecvStats* stats,YangBandwidth* bw){
	...
	int32_t lostCount=stats->lostVideoPacketCount-stats->preLostVideoPacketCount;
	int32_t sendCount=stats->sendVideoPacketCount-stats->preSendVideoPacketCount;
	if(sendCount>0){
		bw->lostRate=lostCount/sendCount;
	}

	stats->preLostVideoPacketCount=stats->lostVideoPacketCount;
	stats->preSendVideoPacketCount=stats->sendVideoPacketCount;
	return err;
}
...
void yang_insert_twcc_remote(YangTwcc* twcc,uint16_t sn,int64_t ts){
	if(twcc==NULL) return;
	int32_t ind=YANG_GET_RECV_BUFFER_INDEX(sn);
