...
int32_t yang_h265_encodeVideo(YangRtcSession *session, YangPushH265Rtp *rtp,
	...
	session->context.stats.on_videoRtp(&session->context.stats.sendStats,pkt,&rtp->buf);
	return yang_send_avpacket(session, pkt, &rtp->buf);

}
int32_t yang_h265_encodeAudio(YangRtcSession *session, YangPushH265Rtp *rtp,
		YangRtpPacket *pkt) {
	int err = 0;

	yang_init_buffer(&rtp->buf, yang_get_rtpBuffer(rtp->audioRtpBuffer),
			kRtpPacketSize);
	...
	session->context.stats.on_audioRtp(&session->context.stats.sendStats,pkt,&rtp->buf);
	return yang_send_avpacket(session, pkt, &rtp->buf);
...
int32_t yang_push_h265_audio(void *psession, YangPushH265Rtp *rtp,
...
void yang_create_pushH265(YangPushH265 *push, YangRtpBuffer* audioRtpBuffer,YangRtpBuffer* videoRtpBuffer) {
	if (push == NULL)		return;
	YangPushH265Rtp *rtp=(YangPushH265Rtp*)calloc(sizeof(YangPushH265Rtp),1);
	push->push=rtp;
	rtp->videoSsrc = 0;
	rtp->videoSeq = 0;
	rtp->audioSeq = 0;
	...
	push->on_audio =yang_push_h265_audio;
