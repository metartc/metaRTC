...
void g_yang_mt_init(void* pcontext,YangStreamConfig* stream,void* user){
    ...
    yang_create_rtcConnection(context->conn, context->avinfo);

	if (context->conn->session->context.streamConfig == NULL)	context->conn->session->context.streamConfig = context->streamconfig;
...
int32_t g_yang_mt_publishVideo(void* peer,YangFrame* videoFrame){
	...
	if(videoFrame->payload[0]==0x00&&videoFrame->payload[1]==0x00&&videoFrame->payload[2]==0x01){
		if((videoFrame->payload[3]& kNalTypeMask) == YangAvcNaluTypeIDR){
			...
			session->extradata.frametype=YANG_Frametype_Spspps;
			session->conn->publishVideo(session->conn->session,&session->extradata);
	...
	return session->conn->publishVideo(session->conn->session,
			videoFrame);
...
int32_t g_yang_mt_publishAudio(void* peer,YangFrame *audioFrame){
	...
	return  session->conn->publishAudio(session->conn->session,audioFrame);
}
...
void yang_destroy_metaConnection(YangMetaConnection* metaconn){
	if(metaconn==NULL) return;
	 if(((YangRtcSession*)metaconn->context)->context.state) {
		 ...
		 if(((YangMetaSession*)metaconn->context)->conn){
			yang_destroy_rtcConnection( ((YangMetaSession*)metaconn->context)->conn);
			yang_free(((YangMetaSession*)metaconn->context)->conn);
		 }
	 }
	 yang_free(((YangMetaSession*)metaconn->context)->streamconfig);
	 yang_free(((YangMetaSession*)metaconn->context)->avinfo);
