...
int32_t yang_playtrackAudio_on_rtp(YangRtcContext *context, YangPlayTrack* track,YangRtpPacket *ppkt) {
	...
	if (ppkt->payload) {
		...
		if(track&&track->sortBuffer){
			track->sortBuffer->putFrame(&track->sortBuffer->session,audioFrame.pts,&audioFrame);
			YangFrame* frame=track->sortBuffer->getCurFrameRef(&track->sortBuffer->session);
			if (frame&&context && context->streamConfig->recvCallback.receiveAudio){
...
void yang_create_playTrackAudio(YangRtcContext *context, YangPlayTrackAudio *track) {
	...
	if(track->session.hasAudioBuffer){
		track->session.sortBuffer=(YangSortBuffer*)calloc(sizeof(YangSortBuffer),1);
		yang_create_sortBuffer(track->session.sortBuffer,3);
		track->session.sortBuffer->initFrames(&track->session.sortBuffer->session,10,1500);
	}
