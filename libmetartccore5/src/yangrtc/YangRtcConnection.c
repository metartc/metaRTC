...
int32_t yang_rtcconn_requestStunServer(YangRtcSession *session){
	int32_t err=Yang_Ok;
	if(session->context.avinfo->rtc.iceCandidateType!=YangIceStun) return 1;
	if((err=session->ice.requestStun(&session->ice.server,session->context.streamConfig->localPort))!=Yang_Ok){
...
int32_t yang_rtcconn_turn_sendData(YangUdpSession *psession, char *data, int32_t nb) {
	...
	YangTurnConnection* turn=conn->session->ice.turn;

	if(turn&&turn->sendData) {
		return turn->sendData(&turn->session,conn->session->ice.uid,data,nb);
	}
	return  1;
}

int32_t yang_rtcconn_requestTurnServer(YangRtcSession *session){
	if(session->isTurnAllocated) return Yang_Ok;
	int32_t count=0;
	int32_t maxWaitCount=150;
	if(session->ice.turn==NULL){
		session->ice.turn=session->ice.getTurnConnection(&session->ice.server);
	}

	session->ice.turn->start(&session->ice.turn->session);
	session->isTurnAllocated=yangtrue;
	while(count<maxWaitCount&&!session->ice.turn->isAllocated(&session->ice.turn->session)){
		yang_usleep(20000);
		count++;
	}

    session->ice.server.relayIp = session->ice.turn->session.relayIp;
    session->ice.server.relayPort = session->ice.turn->session.relayPort;
...
int32_t yang_rtcconn_requestTurnServer2(YangRtcSession *session){
	if(session->ice.turn==NULL)
		yang_rtcconn_requestTurnServer(session);
	else if(!session->isTurnAllocated){
		return 1;
	}
	int32_t count=0;
	int32_t maxWaitCount=150;
	session->ice.turn->addPeer(&session->ice.turn->session,session->ice.uid,session,yang_rtcconn_turn_receive,session->context.streamConfig->remoteIp,session->remotePort);

	session->isTurnReady=yangtrue;
	while(count<maxWaitCount&&!session->ice.turn->isReady(&session->ice.turn->session)){
		yang_usleep(20000);
		count++;
	}
...
int32_t yang_rtcconn_createOffer(YangRtcSession *session, char **psdp){
	...
	if(session->context.avinfo->rtc.iceUsingLocalIp==0) {
			YangIceCandidateType candidateType=(YangIceCandidateType)session->context.avinfo->rtc.iceCandidateType;
			switch(candidateType){
				...
				case YangIceStun:
					...
					break;
				case YangIceTurn:
					if(yang_rtcconn_requestTurnServer(session)!=Yang_Ok) {
...
int32_t yang_rtcconn_startRtc(YangRtcSession* session,char* sdp){
	...
	//if(!session->remotePort==0&&session->context.avinfo->rtc.iceCandidateType==YangIceTurn)
	//	session->context.avinfo->rtc.iceCandidateType=YangIceStun;

	if(session->context.avinfo->sys.mediaServer==Yang_Server_P2p){

		if(session->context.avinfo->rtc.iceUsingLocalIp) goto ret;
		YangIceCandidateType candidateType=(YangIceCandidateType)session->context.avinfo->rtc.iceCandidateType;
		switch(candidateType){
			case YangIceHost:
				goto ret;
			...
			case YangIceStun:
				...
				break;
			case YangIceTurn:
				if(yang_rtcconn_requestTurnServer2(session)!=Yang_Ok) {
					yang_error("request turn server fail");
				}else{
...
void yang_create_rtcConnection(YangRtcConnection* conn,YangStreamConfig* streamconfig,YangAVInfo* avinfo){
	...
	yang_create_ice(&session->ice,streamconfig->uid,avinfo);
	...
	conn->requestTurnServer=yang_rtcconn_requestTurnServer;
