...
void g_startStunTimer(void *user) {
	if (user == NULL)		return;
	YangRtcConnection *conn = (YangRtcConnection*) user;
	conn->startStunTimer(conn->session);
...
int32_t yang_rtcconn_publishVideo(YangRtcSession *session, YangFrame *p) {
#if Yang_HaveDtls
...
int32_t yang_rtcconn_publishAudio(YangRtcSession *session, YangFrame *p) {
#if Yang_HaveDtls
	if (session==NULL||session->context.dtls->session.state!=YangDtlsStateClientDone)	return Yang_Ok;
#else
	if (session==NULL||!session->context.state)	return Yang_Ok;
...
int32_t yang_rtcconn_publishMsg(YangRtcSession *session, YangFrame *p) {
#if Yang_HaveDtls
#if Yang_HaveDatachannel
...
void yang_rtcconn_disconnect(YangRtcSession *session) {
	if (session == NULL)	return;
#if Yang_HaveDtls
...
void yang_create_rtcConnection(YangRtcConnection* conn,YangAVInfo* avinfo){
	...
	conn->disconnect=yang_rtcconn_disconnect;
	conn->init=yang_rtcconn_init;
	conn->publishVideo=yang_rtcconn_publishVideo;
	conn->publishAudio=yang_rtcconn_publishAudio;
	conn->publishMsg=yang_rtcconn_publishMsg;
	conn->notify=yang_rtcconn_notify;
	conn->isAlive=yang_rtcconn_isAlive;
	conn->startudp=yang_rtcconn_startudp;
	conn->receive=yang_rtcconn_receive;
	conn->startStunTimer=yang_rtcconn_startStunTimer;
}
