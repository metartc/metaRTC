...
bool yang_twcc_can_add_to_chunk(YangRecvTWCC* twcc,YangRtcpTWCCChunk* chunk, int32_t delta_size)
{
...
int32_t yang_twcc_encode_chunk_two_bit(YangRecvTWCC* twcc,YangRtcpTWCCChunk* chunk, size_t size, bool shift)
{
	uint32_t  i = 0;
...
int32_t yang_twcc_rtcp_decode(YangRtcpCommon* comm,YangBuffer *buffer){
    ...
    comm->payloadLen = (comm->header.length + 1) * 4 - sizeof(YangRtcpHeader) - 4;
...
void yang_create_rtcptwcc(YangRtcpCommon* comm){
	...
	comm->twcc=calloc(sizeof(YangRecvTWCC),1);
