...
int32_t yang_ice_turn_request(YangTurnConnection* turn,YangIceServer* server){

	if(turn->bindingTurnServer(&turn->session)!=Yang_Ok){
			return yang_error_wrap(ERROR_RTC_TURN,"turn bindingturnserver fail");
		}
	return Yang_Ok;
}
...
void yang_create_ice(YangIce* ice,YangAVInfo* avinfo){
	...
	ice->requestTurn=yang_ice_turn_request;
}

void yang_destroy_ice(YangIce* ice){
	yang_destroy_stun(&ice->stun);
	if(ice->turn){
		yang_destroy_turnConnection(ice->turn);
		yang_free(ice->turn);
	}
}
