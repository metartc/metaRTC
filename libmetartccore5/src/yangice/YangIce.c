...
void g_yang_ice_receive(char *data, int32_t size, void *user) {
	if (user == NULL)		return;
...
int32_t yang_ice_request(YangIceServer* server,int32_t localport){
	char tmp[1024]={0};
	int32_t err=Yang_Ok;
	...
	//yang_create_rtcudp(udp,"182.92.163.143",3478,16000);
	yang_create_stun(stun);
	yang_create_rtcudp(udp,localport);
	udp->updateRemoteAddress(&udp->session,server->serverIp,server->serverPort);

	udp->session.receive=g_yang_ice_receive;
	udp->session.user=&request;
	udp->start(&udp->session);
	int32_t waitTime=0;

	//YangStunPacket pkt={0};
	YangBuffer buf;
...
void yang_create_ice(YangIce* ice,YangAVInfo* avinfo){
	...
	ice->requestStun=yang_ice_request;
}
