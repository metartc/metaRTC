...
YangIceCandidateType yang_ice_handle(YangIceSession* session,void* rtcSession,yang_turn_receive receive,char* remoteIp,int32_t remotePort){

	switch(session->candidateType){
			case YangIceHost:
				break;
			case YangIceStun:
                if(session->server.stunPort==0&&yang_ice_requestStunServer(session)!=Yang_Ok) {
					yang_error("request stun server fail,change to host");
					session->candidateType=YangIceHost;
				}
				break;
			case YangIceTurn:
				if(yang_ice_requestTurnServer(session,rtcSession,receive,remoteIp,remotePort)!=Yang_Ok) {
                    yang_ice_closeTurn(session);
					yang_error("request turn server fail, change to stun");
					session->candidateType=YangIceStun;
					if(yang_ice_requestStunServer(session)!=Yang_Ok){
