...
void yang_turn_addFingerprint(YangTurnSession* session,YangBuffer* stream,yangbool addFingerprint){
    ...
    char keystr[128]={0};
    char key[128]={0};
    snprintf(keystr,sizeof(keystr),"%s:%s:%s",session->server->username,session->realm,session->server->password);
...
int32_t yang_turn_encode_request(YangTurnSession* session){
	...
	yang_init_buffer(&stream,tmp,1024);
	 ...
	 session->stun.encode_request(&stream,session->udp,session->server->username,session->server->password);
