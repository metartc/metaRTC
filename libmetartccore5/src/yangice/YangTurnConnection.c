...
void yang_turnconn_updateNonce(YangTurnSession *session){
	...
	if(session->response.nonceLen>0){
		...
		memcpy(session->nonce,session->response.nonce,session->response.nonceLen);
	}
}
	...
	case StunAllocateSuccessResponse:
		...
		if(session->relayPort==0) {
			...
			yang_trace("\nturn allocate ip=%u.%u.%u.%u,turn port=%d,lifetime=%d",
					ip[0],ip[1],ip[2],ip[3],session->relayPort,session->response.lifetime);
		}
	...
	case StunPermissionSuccessResponse:
		for (int32_t i = 0; i < session->peers.vec.vsize; i++) {
			if (session->peers.vec.payload[i]->state == YangTurnPermission) {
				...
				session->peers.vec.payload[i]->permissionOuttime=yang_get_system_time()/YANG_UTIME_SECONDS+Yang_Turn_Permission_Lifetime;
			}
		}
...
void yang_turnconn_refreshAllocate(YangTurnSession *session){
	...
	for(int32_t i=0;i<session->peers.vec.vsize;i++){
		peer=session->peers.vec.payload[i];
		if(yang_get_system_time()/YANG_UTIME_SECONDS>peer->permissionOuttime-60){
	...
	if(session->allocateOuttime && yang_get_system_time()/YANG_UTIME_SECONDS>=session->allocateOuttime-60){
		...
		yang_turn_encode_refresh(session);
		return;
	}
