...
#ifdef _WIN32
	uint32_t addr=be32toh(udp->session.local_addr.sin_addr.S_un.S_addr);
#else
	uint32_t addr=be32toh(udp->session.local_addr.sin_addr.s_addr);
#endif
   // uint32_t addr = be32toh(udp->session.local_addr.sin_addr.s_addr);//inet_addr(localIp);
	YangStunPacket packet;
	memset(&packet,0,sizeof(YangStunPacket));
