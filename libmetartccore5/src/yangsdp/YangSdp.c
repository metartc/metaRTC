...
void  yang_sdp_genLocalSdp_candidate(YangRtcSession *session,YangMediaDesc *media_desc,char* localIp,int32_t localport){
    ...
    if(session->context.avinfo->rtc.hasIceServer&&session->ice.server.stunPort>0){
    	strcpy(media_desc->candidates.payload[index].type, session->isTurn?"relay":"srflx");
    }else{
...
int32_t yang_sdp_genLocalSdp2(YangRtcSession *session, int32_t localport,char *dst, YangStreamOptType role) {
   ...
   if(mediaServer==Yang_Server_P2p&&(session->isServer||(session->context.avinfo->rtc.hasIceServer&&session->ice.server.stunPort>0))){
	    ...
	    yang_create_YangCandidateVector(&video_media_desc->candidates);
	   	...
	   	}else if(session->context.avinfo->rtc.hasIceServer&&session->ice.server.stunPort>0){
