...
void  yang_sdp_genLocalSdp_candidate(YangRtcSession *session,YangMediaDesc *media_desc,char* localIp,int32_t localport){
    ...
    if(session->context.avinfo->rtc.iceCandidateType){
    	strcpy(media_desc->candidates.payload[index].type, session->context.avinfo->rtc.iceCandidateType==YangIceTurn?"relay":"srflx");
    }else{
...
int32_t yang_sdp_genLocalSdp2(YangRtcSession *session, int32_t localport,char *dst, YangStreamOptType role) {
   ...
   if(mediaServer==Yang_Server_P2p){
	    ...
	    yang_create_YangCandidateVector(&video_media_desc->candidates);
	   	...
	   	}else if(session->context.avinfo->rtc.iceCandidateType==YangIceStun&&session->ice.server.stunPort>0){
	   		struct sockaddr_in addr;
	   		addr.sin_addr.s_addr= session->ice.server.stunIp;
	   		localip=inet_ntoa(addr.sin_addr);
	   		localport=session->ice.server.stunPort;
	   	}else if(session->context.avinfo->rtc.iceCandidateType==YangIceTurn&&(session->isTurnReady||session->isTurnAllocated)){
	   		struct sockaddr_in addr;
	   		addr.sin_addr.s_addr= session->ice.server.relayIp;
	   		localip=inet_ntoa(addr.sin_addr);
	   		localport=session->ice.server.relayPort;
	   	}else{
#ifdef _WIN32
	   		hasIplist=1;
#endif
		...
		for(int32_t i=0;i<sdp->media_descs.vsize;i++){
			...
			if(desc->candidates.vsize>0){
				for (k = 0; k < desc->candidates.vsize; k++) {
					...
					if(candidate){
						if(candidate->port>0) session->remotePort=candidate->port;
