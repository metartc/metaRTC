...
int32_t yang_sdp_genLocalSdp(YangRtcSession *session, int32_t localport,char **psdp, YangStreamOptType role) {
	...
	if(mediaServer==Yang_Server_P2p){
		...
		audio_media_desc->candidates.payload[0].port = session->context.streamConfig->localPort;
		...
		video_media_desc->candidates.payload[0].port = session->context.streamConfig->localPort;
		...
		if(data_media_desc) {
			...
			data_media_desc->candidates.payload[0].port = session->context.streamConfig->localPort;
