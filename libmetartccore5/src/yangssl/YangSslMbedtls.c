        ...
        const char* input, const int32_t input_length, char* output, unsigned int* output_length)
{
    ...
    if((err=mbedtls_md_hmac(mbedtls_md_info_from_type(MBEDTLS_MD_SHA1), (const uint8_t*)key, key_length, (const uint8_t*)input, input_length, ( uint8_t*)output))!=Yang_Ok){
...
void yang_dtls_genDateStr(time_t ts,char* str){
	     ...
	     sprintf(str,"%04d%02d%02d%02d%02d%02d",ntm->tm_year+1900,ntm->tm_mon+1,ntm->tm_mday,ntm->tm_hour,ntm->tm_min,ntm->tm_sec);
}
...
int32_t yang_create_certificate(YangCertificate* cer)
{
    ...
    uint8_t caSn[20];
    uint8_t* certBuf = (uint8_t*)calloc(4096,1);
    if(cer->dtls_cert==NULL) cer->dtls_cert=(mbedtls_x509_crt*)calloc(sizeof(mbedtls_x509_crt),1);
    if(cer->dtls_pkey==NULL) cer->dtls_pkey=(mbedtls_pk_context*)calloc(sizeof(mbedtls_pk_context),1);
    entropy = (mbedtls_entropy_context*) calloc(sizeof(mbedtls_entropy_context),1);
    ctrDrbg = (mbedtls_ctr_drbg_context*) calloc(sizeof(mbedtls_ctr_drbg_context),1);
    tmpCert = (mbedtls_x509write_cert*) calloc(sizeof(mbedtls_x509write_cert),1);
    ...
    if(mbedtls_x509write_crt_set_serial(tmpCert, &serial) != Yang_Ok || mbedtls_x509write_crt_set_validity(tmpCert, before, after) !=Yang_Ok ||
