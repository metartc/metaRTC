...
int32_t hmac_encode(const char *algo, const char *key, const int key_length,
	...
	*(output_length) = mbedtls_md_get_size(
...
void yang_dtls_genDateStr(time_t ts, char *str) {
	...
	sprintf(str, "%04d%02d%02d%02d%02d%02d", ntm->tm_year + 1900,
...
int32_t yang_create_certificate(YangCertificate *cer) {
	...
	if (cer->dtls_cert == NULL)
		cer->dtls_cert = (mbedtls_x509_crt*) calloc(sizeof(mbedtls_x509_crt),
				1);
	if (cer->dtls_pkey == NULL)
		cer->dtls_pkey = (mbedtls_pk_context*) calloc(
				sizeof(mbedtls_pk_context), 1);
	entropy = (mbedtls_entropy_context*) calloc(sizeof(mbedtls_entropy_context),
			1);
	ctrDrbg = (mbedtls_ctr_drbg_context*) calloc(
			sizeof(mbedtls_ctr_drbg_context), 1);
	tmpCert = (mbedtls_x509write_cert*) calloc(sizeof(mbedtls_x509write_cert),
			1);
	...
	if (mbedtls_x509write_crt_set_serial(tmpCert, &serial) != Yang_Ok
			|| mbedtls_x509write_crt_set_validity(tmpCert, before,
					after) !=Yang_Ok ||
