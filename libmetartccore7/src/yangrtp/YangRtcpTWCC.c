...
int32_t yang_twcc_encode_chunk_run_length(YangRecvTWCC* twcc,YangRtcpTWCCChunk* chunk)
{
	...
	encoded_chunk = (chunk->deltaSizes[0] << 13) | chunk->size;
...
int32_t yang_twcc_recv_encode(YangRecvTWCC* twcc,YangBuffer *buffer){
	...
	int16_t delta;
	uint16_t last_sn,lost_sn,packet_count,current_sn,tdelta,it;

	int32_t i,recv_delta_size;
	int32_t err = Yang_Ok;

	int64_t delta_us;
	int64_t ts,last_ts,current_ts;
	...
	ts =twcc->tss[YANG_GET_RECV_BUFFER_INDEX(twcc->baseSn)];

	twcc->referenceTime = (ts % kTwccFbReferenceTimeDivisor) / kTwccFbTimeMultiplier;
	last_ts = (int64_t)(twcc->referenceTime) * kTwccFbTimeMultiplier;
	...
	for(i=0; i<twcc->sns.vsize; i++) {
		...
		// calculate delta
		packet_count++;
		delta_us = yang_twcc_calculate_delta_us(current_ts, last_ts);
		delta = delta_us;
		if(delta != delta_us) {
		...
		if(current_sn > (last_sn + 1)) {
			// lost packet
			for(lost_sn = last_sn + 1; lost_sn < current_sn; ++lost_sn) {
				yang_twcc_process_pkt_chunk(twcc,&chunk, 0);
				packet_count++;
			}
		}

		recv_delta_size = (delta >= 0 && delta <= 0xff) ? 1 : 2;
	...
	for(i=0; i<twcc->pktDeltas.vsize; i++) {
		it=twcc->pktDeltas.payload[i];
		if(0 <= it && 0xFF >= it) {
			// small delta
			small_delta = it;
			yang_write_1bytes(buffer,small_delta);
		} else {
...
void yang_destroy_rtcptwcc(YangRtcpCommon* comm){
	YangRecvTWCC* twcc;
	if(comm==NULL||comm->twcc==NULL)
		return;

	twcc=(YangRecvTWCC*)comm->twcc;
