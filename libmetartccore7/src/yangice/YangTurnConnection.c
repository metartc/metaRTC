...
static int32_t yang_turnconn_receive(YangTurnSession *session, char *data, int32_t nb);

yang_vector_impl2(YangTurnPeer)

static void yang_turn_addFingerprint(YangTurnSession *session, YangBuffer *stream,
...
static int32_t yang_turn_encode_allocate(YangTurnSession *session, uint8_t protocol,
...
static int32_t yang_turn_encode_refresh(YangTurnSession *session, yangbool isClose) {
	uint32_t ret = 0;
	int32_t lifetime = 0;
...
static int32_t yang_turn_encode_permission(YangTurnSession *session,
		YangTurnPeer *peer) {
	uint32_t ret = 0;
...
static int32_t yang_turn_encode_bindChannel(YangTurnSession *session,
		YangTurnPeer *peer) {
	uint16_t channelNum;
	uint32_t ret = 0;
...
int32_t yang_turn_encode_sendIndication(YangTurnSession *session,
	...
	if(session==NULL || peer==NULL || data==NULL)
		return ERROR_RTC_TURN;
...
static int32_t yang_turn_decode(YangTurnSession *session, char *buf,
	...
	uint8_t stunClass,stunCode;
	uint16_t message_len,type,len;
...
static int32_t yang_turnconn_removePeer(YangTurnSession *session, int32_t uid) {
	int32_t i;
	YangTurnPeer *peer = NULL;
...
static yangbool yang_turnconn_isReady(YangTurnSession *session) {
	if(session==NULL)
		return yangfalse;
...
static yangbool yang_turnconn_isAllocated(YangTurnSession *session) {
	if(session==NULL)
		return yangfalse;
...
void yang_create_turnConnection(YangTurnConnection *conn, YangIceServer *server,
	...
	if(conn==NULL || server==NULL || sock==NULL)
		return;
