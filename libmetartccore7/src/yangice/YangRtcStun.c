...
uint32_t yang_stun_encode_mapped_address_ipv6(YangStunPacket* pkt,struct sockaddr_in6* addr,char** str)
{
	...
	p=(uint8_t*)(&addr->sin6_addr.s6_addr);
...
int32_t yang_stun_encode_binding_request(YangStunPacket* pkt, YangBuffer* stream,char* pwd,char* username,uint32_t username_len)
{
	...
	hmac_len = yang_stun_encode_hmac2(hmac_buf, hmac_buf_len, &hmac);
	...
	//Fingerprint
	crc32 = yang_crc32_ieee(stream->data, yang_buffer_pos(stream), 0)^ 0x5354554E;

	fingerprint_len = yang_stun_encode_fingerprint2(crc32,&fingerprint);
...
int32_t yang_stun_encode_binding_response(YangStunPacket* pkt,char* pwd, YangBuffer* stream)
{
	...
	uint32_t hmac_len,crc32;
	uint32_t hmac_buf_len = 0,fingerprint_len;
	uint32_t property_username_len,mapped_address_len;
	...
	property_username_len=yang_stun_encode_rtcusername(pkt,&property_username);
	mapped_address_len=yang_stun_encode_mapped_address_ipv4(pkt,&mapped_address);
...
int32_t yang_stun_encode_binding_request2(YangStunPacket* pkt, YangBuffer* stream,YangRtcSocket* udp,char* pwd,char* username,uint32_t username_len){
	int32_t err = Yang_Ok;
	uint32_t crc32,fingerprint_len;
	uint32_t hmac_buf_len = 0,hmac_len;
...
int32_t yang_stun_createRequestStunPacket(void* psession,char* ice_pwd){
	uint32_t addr,property_username_len;
	...
	if(yang_strlen(session->context.localIp)==0){
		yang_memset(session->context.localIp,0,sizeof(session->context.localIp));
		yang_getLocalInfo(session->context.peerInfo->familyType,session->context.localIp);
	}

	addr = yang_be32toh(yang_inet_addr(session->context.localIp));
	...
	packet.mapped_port=session->context.peerInfo->rtc.rtcLocalPort;//session->context.udp->session.local_addr.sin_port;
...
int32_t yang_stun_encode(YangStunMessageType stunType,YangBuffer* stream,void* pudp,char* username,char* ice_pwd){
	...
	addr = yang_be32toh(yang_inet_addr(localIp));
