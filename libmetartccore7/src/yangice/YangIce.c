...
static YangTurnConnection* g_yang_turnConnection=NULL;
static uint32_t g_yang_turnCount=0;

static YangTurnConnection* yang_ice_getTurn(YangIceServer* server,YangSocketProtocol protocol){
	int32_t port;
	YangRtcSocket* sock;
...
static void g_yang_ice_stun_receive(char *data, int32_t size, void *user) {
	uint8_t bt;
	int32_t err = 0;
...
static int32_t yang_ice_stun_request(YangIceServer* server,int32_t localport){
	int32_t err=Yang_Ok;
	int32_t waitTime=0;
...
static int32_t yang_ice_initTurn(YangIceSession *session){
	int32_t count=0;
	int32_t maxWaitCount=100;

	if(session->isTurnAllocated)
		return Yang_Ok;
...
static int32_t yang_ice_requestTurnServer(YangIceSession *session,void* rtcSession,yang_turn_receive receive,char* remoteIp,int32_t remotePort){
	int32_t count=0;
	int32_t maxWaitCount=100;
...
static void yang_ice_closeTurn(YangIceSession *session){
...
static int32_t yang_ice_initIce(YangIceSession *session){
	if(session==NULL)
...
static int32_t yang_ice_handle(YangIceSession* session,void* prtcSession,yang_turn_receive receive,char* remoteIp,int32_t remotePort){
	if(session==NULL || prtcSession==NULL ||receive==NULL || remoteIp==NULL)
		return ERROR_RTC_ICE;
	...
	if(session->iceState==YangIceFail)
		return ERROR_RTC_ICE;
...
void yang_create_ice(YangIce* ice,YangPeerInfo* peerInfo,YangPeerCallback* peerCallback){
	...
	session->callback.context=peerCallback->iceCallback.context;
	session->callback.onIceStateChange=peerCallback->iceCallback.onIceStateChange;
	session->callback.onConnectionStateChange=peerCallback->iceCallback.onConnectionStateChange;
