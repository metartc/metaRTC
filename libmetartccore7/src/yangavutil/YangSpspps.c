...
static uint32_t yang_sps_Ue(uint8_t *pBuff, uint32_t nLen, uint32_t *nStartBit)
{
...
static int32_t yang_sps_Se(uint8_t *pBuff, uint32_t nLen, uint32_t *nStartBit)
{
...
static yangdword yang_sps_u(uint32_t BitCount,uint8_t * buf,uint32_t *nStartBit)
{
	uint32_t i;
	yangdword dwRet = 0;
