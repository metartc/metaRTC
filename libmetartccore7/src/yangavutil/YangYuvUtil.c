...
void yang_plusYuy2(uint8_t *src, uint8_t *dest, int32_t model, int32_t srcWidth,
		int32_t srcHeight, int32_t destWidth, int32_t destHeight) {
	int32_t i,start2,start3,start4;
	uint8_t *temp=NULL;//, *srcTmp, *dstTmp;

	if (model == 4) {
		start4 = (srcHeight - destHeight + 1) * srcWidth * 2 - destWidth * 2;
		temp = src + start4;

	} else if (model == 3) {
		start3 = srcWidth * (srcHeight - destHeight);

		temp = src + start3;

	} else if (model == 2) {
		start2 = srcWidth - destWidth;
		temp = src + start2;
