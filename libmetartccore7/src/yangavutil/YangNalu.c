...
int32_t yang_getH264SpsppseNalu( YangFrame *videoFrame, uint8_t *pnaludata) {
	...
	spsLen = yang_get_be32(pnaludata);
	ppsLen = yang_get_be32(pnaludata + 4 + spsLen);

	if(spsLen+ppsLen+8>Yang_Nalu_SpsppsBufferLen)
		return 1;
...
int32_t yang_getH265SpsppseNalu( YangFrame *videoFrame, uint8_t *pnaludata) {
	...
	vpsLen = yang_get_be32(pnaludata);
	spsLen = yang_get_be32(pnaludata+ 4 + vpsLen);
	ppsLen = yang_get_be32(pnaludata + 4 + vpsLen+4+spsLen);

	if(vpsLen+spsLen+ppsLen+8>Yang_Nalu_SpsppsBufferLen)
		return 1;
	...
	vps = pnaludata + 4;
	sps = pnaludata + 4 + vpsLen + 4;
	pps=pnaludata + 4 + vpsLen + 4+spsLen+4;
