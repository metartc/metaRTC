...
void yang_pulltrack_receiveVideo(YangRtcContext *context,YangFrame *videoFrame) {
	...
	if(videoFrame==NULL||videoFrame->payload==NULL||videoFrame->nb>YANG_VIDEO_ENCODE_BUFFER_LEN)
		return;

    first=videoFrame->payload[0];
	temp = videoFrame->payload+1;
    videoLen = videoFrame->nb-1;
    ...
    if (first == 0x17) {
    	...
    	if(nalu.spsppsPos>-1&&nalu.keyframePos>-1){
    		...
    		if (context &&context->peerCallback->recvCallback.receiveVideo){
    			context->peerCallback->recvCallback.receiveVideo(context->peerCallback->recvCallback.context,videoFrame);
    		}
    	}
    ...
    return;
    sendevideo:
		videoFrame->uid=context->peerInfo->uid;
		if (context &&context->peerCallback->recvCallback.receiveVideo){
			context->peerCallback->recvCallback.receiveVideo(context->peerCallback->recvCallback.context,videoFrame);
		}
		return;
