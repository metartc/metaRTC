...
static int32_t yang_encodeAudio(YangRtcSession *session, YangPushAudioRtp *rtp,
		YangRtpPacket *pkt) {
	int err = 0;
...
static int32_t yang_on_rtp(void *psession, YangPushAudioRtp *rtp,
	...
	yang_memcpy(rtp->audioRawData.payload, audioFrame->payload,
            rtp->audioRawData.nb);
	if ((err = yang_encodeAudio(session, rtp, &rtp->audioRawPacket))
			!= Yang_Ok) {
...
void yang_create_pushAudio(YangPushAudio* push,YangRtpBuffer* audioRtpBuffer){
	YangPushAudioRtp *rtp;

	if (push == NULL)
		return;

	rtp=(YangPushAudioRtp*)yang_calloc(sizeof(YangPushAudioRtp),1);
	push->push=rtp;
