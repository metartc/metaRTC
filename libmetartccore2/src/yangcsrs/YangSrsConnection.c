...
int32_t yang_srs_doHandleSignal(YangRtcSession* session,SrsSdpResponseType* srs,int32_t localport, YangStreamOptType role) {
	int32_t err = Yang_Ok;
	char *srsSdp = (char*)calloc(1,1024*12);//new char[1024 * 12];
	char *tsdp=NULL;
	...
	//sprintf(apiurl, "http://%s:%d/rtc/v1/%s/", session->context.streamConf->serverIp.c_str(), session->context.streamConf->serverPort,roleStr);
