...
int32_t yang_srs_handlePlaySdp(YangRtcSession* session,SrsSdpResponseType* srs) {
	...
	for(int32_t i=0;i<sdp.media_descs.vlen;i++){
		...
		if(yang_strcmp(desc->type,"audio")==0){
			...
			for(int32_t k=0;k<desc->payload_types.vlen;k++){
			...
			for(int32_t k=0;k<desc->payload_types.vlen;k++){
				if(yang_strcmp(desc->payload_types.payload[0].encoding_name,"H264")==0){
					session->remote_video->encode=Yang_VED_264;
				}else if(yang_strcmp(desc->payload_types.payload[0].encoding_name,"H265")==0){
					session->remote_video->encode=Yang_VED_265;
				}
				session->remote_video->videoClock=90000;
			}
			for(int32_t k=0;k<desc->extmaps.vlen;k++){
