...
int32_t yang_sdp_querySrs(YangRtcSession* session,SrsSdpResponseType* srs,int32_t isplay,char* ip,int32_t port,char* purl, char* psdp)
{
	...
	yang_trace("\n***********************************\n");
	yang_trace("retcode===%d\n",srs->retcode);
	yang_trace("sessionid==%s\n",srs->sessionid);
	yang_trace("server==%s\n",srs->serverIp);
	yang_trace("\n***********************************\n");
...
int32_t yang_cstr_userupindex(char* p,char c,int32_t n){
...
int32_t yang_srs_handlePlaySdp(YangRtcSession* session,SrsSdpResponseType* srs) {
	...
	if(session->remote_audio==NULL) session->remote_audio=(YangAudioParam*)calloc(sizeof(YangAudioParam),1);//new YangAudioParam();
	if(session->remote_video==NULL) session->remote_video=(YangVideoParam*)calloc(sizeof(YangVideoParam),1);//new YangVideoParam();
