...
int32_t yang_sdp_genLocalSdp(YangRtcSession *session, int32_t localport,
	...
	yang_init_rtcsdp(local_sdp);
		...
		yang_init_strVector(&local_sdp->msids);
		...
		yang_insert_strVector(&local_sdp->msids,streamnames);
...
//media pay
		...
		audiotype->clock_rate=48000;
		strcpy(audiotype->encoding_param,"2");
		strcpy(audiotype->format_specific_param,"minptime=10;useinbandfec=1");
		yang_init_strVector(&audiotype->rtcp_fb);
		yang_insert_strVector(&audiotype->rtcp_fb,"transport-cc");
		...
		yang_init_strVector(&videotype->rtcp_fb);
		yang_insert_strVector(&videotype->rtcp_fb,"transport-cc");
		yang_insert_strVector(&videotype->rtcp_fb,"nack");
		yang_insert_strVector(&videotype->rtcp_fb,"nack pli");
