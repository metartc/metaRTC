...
int32_t yang_sdp_genLocalSdp(YangRtcSession *session, int32_t localport,
	...
	char randstr[50];
	char *ss;
	if (session->context.avcontext->video->videoEncoderType == Yang_VED_264) {
		ss = (char*) local_h264_sdp_str;
	}
#if	Yang_H265_Encoding
	if (session->context.avcontext->video->videoEncoderType == Yang_VED_265) {
	...
	if (session->context.avcontext->audio->audioEncoderType == Yang_AED_OPUS) {
		char opus[64];
		memset(opus, 0, sizeof(opus));
		sprintf(opus, "opus/%d/%d", session->context.avcontext->audio->sample,
				session->context.avcontext->audio->channel);
