#include <yangwebrtc/YangCRtcPlayStream.h>
#include <math.h>
...
void yang_init_rtcplay(YangRtcContext *context, YangRtcPlayStream* play,	YangRtpBuffer *rtpBuffer){
	...
	yang_init_rtpPacket(&play->rtp);
		...
		play->rtpBuffer = rtpBuffer;
			...
			play->audio_track=(YangRecvTrack*)calloc(1,sizeof(YangRecvTrack));//new YangRtcAudioRecvTrack( conf,play->session,  rtpBuffer);
			yang_init_recvaudioTrack(context,play->audio_track,rtpBuffer);
			play->video_track=	(YangVideoRecvTrack*)calloc(1,sizeof(YangVideoRecvTrack));//new YangVideoRecvTrack(conf,play->session,  rtpBuffer);
		...
		/**for (int32_t i = 0; i < (int) streaplay->desc->video_track_descs_.size(); ++i) {
			YangRtcTrack *desc = streaplay->desc->video_track_descs_.at(i);
			twcc_id = desc->twcc_id;//desc->get_rtp_extension_id(kTWCCExt);
...
int32_t yang_rtcplay_send_rtcp_rr(YangRtcContext *context, YangRtcPlayStream* play) {
	if(context==NULL||play==NULL) return 1;
	int32_t err = Yang_Ok;

	//for (int32_t i = 0; i < (int) play->video_tracks.size(); ++i) {
		//YangVideoRecvTrack *track = play->video_tracks.at(i);
		//if ((err = play->video_tracks->send_rtcp_rr()) != Yang_Ok) {
		if ((err = yang_recvtrack_send_rtcp_rr(context,&play->video_track->recv)) != Yang_Ok) {
			return yang_error_wrap(err, "track=%u",
	...
	//	YangRtcAudioRecvTrack *track = play->audio_tracks.at(i);
		if ((err = yang_recvtrack_send_rtcp_rr(context,play->audio_track)) != Yang_Ok) {
			return yang_error_wrap(err, "track=%u",
					play->audio_track->ssrc_);
		}
	//}
...
int32_t yang_rtcplay_send_rtcp_xr_rrtr(YangRtcContext *context, YangRtcPlayStream* play) {
	...
	//for (int32_t i = 0; i < (int) play->video_tracks.size(); ++i) {
		//YangVideoRecvTrack *track = play->video_tracks.at(i);
		//if ((err = play->video_tracks->send_rtcp_xr_rrtr()) != Yang_Ok) {
		if ((err = yang_recvtrack_send_rtcp_xr_rrtr(context,&play->video_track->recv)) != Yang_Ok) {
			return yang_error_wrap(err, "track=%u",
					play->video_track->recv.ssrc_);
		}
	//}
	...
	//for (int32_t i = 0; i < (int) play->audio_tracks.size(); ++i) {
		//YangRtcAudioRecvTrack *track = play->audio_tracks.at(i);
		if ((err = yang_recvtrack_send_rtcp_xr_rrtr(context,play->audio_track)) != Yang_Ok) {
			return yang_error_wrap(err, "track=%u",
					play->audio_track->ssrc_);
		}
	//}
...
int32_t yang_rtcplay_on_rtp(YangRtcContext *context, YangRtcPlayStream* play,char *data, int32_t nb_data) {
	...
	if ((err = yang_dec_rtp(&context->srtp,plaintext, &nb_plaintext))!= Yang_Ok) {
		if(err==srtp_err_status_replay_fail) return Yang_Ok;
		 YangBuffer b; yang_init_buffer(&b,data, nb_data);
		 YangRtpHeader h; memset(&h,0,sizeof(YangRtpHeader));h.ignore_padding=true;
		 ...
		 yang_decode_rtpHeader(&b,&h);
		    return yang_error_wrap(err, "marker=%u, pt=%u, seq=%u, ts=%u, ssrc=%u, pad=%u, payload=%uB", h.marker, h.payload_type,
		            h.sequence, h.timestamp, h.ssrc, h.padding_length, nb_data - yang_buffer_pos(&b));
	}

	yang_reset_rtpPacket(&play->rtp);//play->rtp.reset();
	//play->buf.init(play->rtp.wrap(play->rtpBuffer, plaintext, nb_plaintext), nb_plaintext);
	...
	//yang_trace("\n");
	//for(int i=0;i<50;i++) yang_trace("%02x,",play->rtp.payload[i]);
...
int32_t yang_rtcplay_check_send_nacks(YangRtcContext *context, YangRtcPlayStream* play) {
	...
	//if (!play->nack_enabled) {
	//	return err;
	//}

	//for (int32_t i = 0; i < (int) play->video_tracks.size(); ++i) {
	//	YangVideoRecvTrack *track = play->video_tracks.at(i);
		if ((err = yang_recvvideoTrack_check_send_nacks(context,play->video_track)) != Yang_Ok) {
			return yang_error_wrap(err, "video track=%d",play->video_track->recv.ssrc_);
		}
	//}

	//for (int32_t i = 0; i < (int) play->audio_tracks.size(); ++i) {
	//	YangRtcAudioRecvTrack *track = play->audio_tracks.at(i);
		if ((err = yang_recvaudioTrack_check_send_nacks(context,play->audio_track)) != Yang_Ok) {
			return yang_error_wrap(err, "audio track=%d",
					play->audio_track->ssrc_);
		}
//	}
	...
	yang_read_1bytes(&stream);
		...
		if (length > rtcp->nb_data) {
			return yang_error_wrap(ERROR_RTC_RTCP_CHECK,
					"invalid XR packet, length=%u, nb_buf=%d", length, rtcp->nb_data);
		}
...
void yang_rtcplay_simulate_nack_drop(YangRtcContext *context, YangRtcPlayStream* play,int32_t nn) {
	if(context==NULL||play==NULL) return ;
	play->nn_simulate_nack_drop = nn;
}

void yang_rtcplay_simulate_drop_packet(YangRtcContext *context, YangRtcPlayStream* play,YangRtpHeader *h,
		int32_t nn_bytes) {
	if(context==NULL||play==NULL) return ;
