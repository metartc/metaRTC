...
void g_yang_doTask(int32_t taskId, void *user) {
	...
	if (session->isSendStun && taskId == 1) {
		if (session->context && session->context->stun.data
				&& session->context->stun.nb > 0) {
...
void yang_init_rtcsession(YangRtcSession *session, YangStreamOptType role) {
	...
	session->codec =
			(YangVideoCodec) session->context->context->video->videoEncoderType;
	session->isSendDtls = 0;
	session->twcc_id_ = 1;
	session->h264Encoder = NULL;

	if (session->codec == Yang_VED_264) {
		session->h264Encoder = (YangH264RtpEncode*) calloc(1,
				sizeof(YangH264RtpEncode));			//new YangH264RtpEncode();
	}
#if	Yang_H265_Encoding
	session->h265Encoder = NULL;
	if (session->codec == Yang_VED_265) {
		session->h265Encoder = (YangH265RtpEncode*) calloc(1,
				sizeof(YangH265RtpEncode));			//new YangH265RtpEncode();
	}
#endif
	...
	if (role == Yang_Stream_Play) {
		if (session->context->playStream == NULL) {
			session->context->playStream = (YangRtcPlayStream*) calloc(1,
					sizeof(YangRtcPlayStream));
			yang_init_rtcplay(session->context, session->context->playStream,
					session->rtpBuffer);
			//session->play->session->twcc_enabled
		}
	} else {
		if (session->context->pubStream == NULL) {
			session->context->pubStream = (YangRtcPublishStream*) calloc(1,
					sizeof(YangRtcPublishStream));//new YangRtcPublishStream(this);
			yang_init_rtcpublish(session->context->pubStream,
					session->context->audioSsrc, session->context->videoSsrc);
...
void yang_rtcsession_startTimers(YangRtcSession *session) {
	...
	if (session->context->context && session->context->context->rtc->sendTwcc
...
int32_t yang_rtcsession_on_rtcp(YangRtcSession *session, char *data,
	...
	for (int i = 0; i < rtcp_compound.vlen; i++) {
		rtcp = (YangRtcpCommon*) (rtcp_compound.rtcps
				+ sizeof(YangRtcpCommon) * i);
...
void yang_rtcsession_receive(YangRtcSession *session, char *data, int32_t size) {
	...
	if (is_rtp_or_rtcp && !is_rtcp) {
		session->startRecv = 1;
		if (session->context && session->context->playStream)
			yang_rtcplay_on_rtp(session->context, session->context->playStream,
					data, size);
		return;
	...
	if (size >= 13 && yang_is_dtls((uint8_t*) data, size)) {
		...
		if (session->context->dtls) {
			if (yang_decodeHandshake(session->context->dtls, data,
					size) == Yang_Ok) {
				...
				if (session->context->dtls->state == YangDtlsStateClientDone) {
					...
					if (session->context && session->context->context)
						...
						session->context->context->yang_sendRequest(
								session->context->streamConf->uid, 0,
								Yang_Req_Connected);
				}
			}
		}
...
int32_t yang_rtcsession_send_video_meta(YangRtcSession *session, YangFrame *p) {
	...
	if (session->h264Encoder)
		return yang_pub_h264_package_stap_a(session->context,
				session->h264Encoder, p);
#if	Yang_H265_Encoding
	if (session->h265Encoder)
		return yang_pub_h265_package_stap_a(session->context,
				session->h265Encoder, p);
#endif
	return Yang_Ok;
}
...
int32_t yang_rtcsession_publishVideo(YangRtcSession *session, YangFrame *p) {
	...
	if (session->h264Encoder)
		return yang_pub_h264_video(session->context, session->h264Encoder, p);
#if	Yang_H265_Encoding
	if (session->h265Encoder)
		return yang_pub_h265_video(session->context, session->h265Encoder, p);
#endif
	return Yang_Ok;
...
int32_t yang_rtcsession_publishAudio(YangRtcSession *session, YangFrame *p) {
	...
	if (session->h264Encoder)
		return yang_pub_h264_audio(session->context, session->h264Encoder, p);
#if	Yang_H265_Encoding
	if (session->h265Encoder)
		return yang_pub_h265_audio(session->context, session->h265Encoder, p);
#endif
...
void yang_rtcsession_do_request_keyframe(YangRtcSession *session, uint32_t ssrc) {
	if (session->context && session->context->context)
		session->context->context->yang_sendRequest(
				session->context->streamConf->uid, ssrc, Yang_Req_Sendkeyframe);
