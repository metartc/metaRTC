...
#include <yangwebrtc/YangCH264RtpEncode.h>
...
void yang_init_rtcsession_context(YangRtcSession* session,YangAVContext* avcontext,void* user){
	...
	session->context.avcontext->audio=avcontext->audio;
	session->context.avcontext->video=avcontext->video;
	session->context.avcontext->sys=avcontext->sys;
	session->context.avcontext->rtc=avcontext->rtc;
	session->context.avcontext->yang_sendRequest=avcontext->yang_sendRequest;
...
void yang_init_rtcsession(YangRtcSession *session, YangStreamOptType role) {
	...
	session->codec =
			(YangVideoCodec) session->context.avcontext->video->videoEncoderType;
	...
	if (role == Yang_Stream_Play) {
		if (session->playStream == NULL) {
			session->playStream = (YangRtcPlayStream*) calloc(1,
					sizeof(YangRtcPlayStream));
			yang_init_rtcplay(&session->context, session->playStream,
					session->rtpBuffer);

		}
	} else {
		if (session->pubStream == NULL) {
			session->pubStream = (YangRtcPublishStream*) calloc(1,
					sizeof(YangRtcPublishStream));//new YangRtcPublishStream(this);
			yang_init_rtcpublish(session->pubStream,
...
void yang_rtcsession_startTimers(YangRtcSession *session) {
	...
	if (session->context.avcontext && session->context.avcontext->rtc->sendTwcc
			...
			if (yang_decodeHandshake(session->context.dtls, data,size) == Yang_Ok) {
				...
				if (session->context.dtls->state == YangDtlsStateClientDone) {
					...
					if (session->context.avcontext)
						...
						session->context.avcontext->yang_sendRequest(
...
void yang_rtcsession_do_request_keyframe(YangRtcSession *session, uint32_t ssrc) {
	if (session->context.avcontext)
		session->context.avcontext->yang_sendRequest(
				session->context.streamConf->uid, ssrc, Yang_Req_Sendkeyframe,session->context.user);
