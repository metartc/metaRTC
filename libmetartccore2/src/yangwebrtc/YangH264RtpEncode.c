...
int32_t yang_h264_encodeAudio(YangRtcContext* context,YangH264RtpEncode* rtp,YangRtpPacket *pkt);
int32_t yang_h264_encodeVideo(YangRtcContext* context,YangH264RtpEncode* rtp,YangRtpPacket *pkt);
int32_t yang_h264_package_fu_a(YangRtcContext* context,YangH264RtpEncode* rtp,YangFrame *videoFrame,
                                        int32_t fu_payload_size);
int32_t yang_h264_package_single_nalu(YangRtcContext* context,YangH264RtpEncode* rtp,char *p, int32_t plen,
                                               int64_t timestamp);
int32_t yang_h264_package_single_nalu2(YangRtcContext* context,YangH264RtpEncode* rtp,YangFrame *videoFrame);
...
void yang_init_h264RtpEncode(YangH264RtpEncode* rtp,YangRtpBuffer* rtpBuffer){
	if(rtp==NULL) return;
	   rtp->videoSsrc = 0;
	    ...
	    rtp->rtpBuffer = NULL;
	   // rtp->session = NULL;
	    rtp->videoBuf = (char*)malloc(kRtpPacketSize);//new char[kRtpPacketSize];
	    memset(&rtp->stapData,0,sizeof(YangRtpSTAPData));
	    yang_inti_sampleVector(&rtp->stapData.nalus);
...
void yang_destroy_h264RtpEncode(YangH264RtpEncode* rtp){
    ...
    yang_destroy_sampleVector(&rtp->stapData.nalus);
...
int32_t yang_pub_h264_audio(YangRtcContext* context,YangH264RtpEncode* rtp,YangFrame* audioFrame){
	 int err = 0;
	    ...
	    rtp->audioRawPacket.header.timestamp=audioFrame->timestamp;//getAudioTimestamp();
	    ...
	    memcpy(rtp->audioRawData.payload, audioFrame->payload,rtp->audioRawData.nb);//getAudioData(), rtp->audioRawData.nb);
	    if ((err = yang_h264_encodeAudio(context,rtp,&rtp->audioRawPacket)) != Yang_Ok) {
...
int32_t yang_pub_h264_video(YangRtcContext* context,YangH264RtpEncode* rtp,YangFrame* videoFrame){
	int32_t err = Yang_Ok;
	    ...
	    if (videoFrame->nb <= kRtpMaxPayloadSize) {
	        if ((err = yang_h264_package_single_nalu2(context,rtp,videoFrame)) != Yang_Ok) {
	            return yang_error_wrap(err, "package single nalu");
	        }
	    } else {
	        if ((err = yang_h264_package_fu_a(context,rtp,videoFrame, kRtpMaxPayloadSize)) != Yang_Ok) {
...
int32_t yang_pub_h264_package_stap_a(YangRtcContext* context,YangH264RtpEncode* rtp,YangFrame* videoFrame){
	...
	//    uint8_t *pps = buf + 13 + spsLen + 2;
	    ...
	    rtp->videoStapPacket.header.timestamp=0;
	  ...
	  //  vps_sample->rtp->bytes = (char*) vps;
	   // vps_sample->rtp->size = vpsLen;
	   // rtp->stap.nalus.push_back(vps_sample);

	    yang_push_sampleVector(&rtp->stapData.nalus,&sps_sample);
	    yang_push_sampleVector(&rtp->stapData.nalus,&pps_sample);


	    if ((err = yang_h264_encodeVideo(context,rtp,&rtp->videoStapPacket)) != Yang_Ok) {
	        return yang_error_wrap(err, "encode packet");
	    }
	    return err;
}
...
int32_t yang_h264_package_single_nalu2(YangRtcContext* context,YangH264RtpEncode* rtp,YangFrame *videoFrame) {
    ...
    rtp->videoRawPacket.header.timestamp=videoFrame->timestamp;
    ...
    memcpy(rtp->videoRawData.payload, videoFrame->payload,  rtp->videoRawData.nb);
    if ((err = yang_h264_encodeVideo(context,rtp,&rtp->videoRawPacket)) != Yang_Ok) {
        return yang_error_wrap(err, "encode packet");
    }
    return err;
}
...
int32_t yang_h264_package_fu_a(YangRtcContext* context,YangH264RtpEncode* rtp,YangFrame *videoFrame,
    ...
    for (int32_t i = 0; i < num_of_packet; ++i) {
        ...
        rtp->videoFuaPacket.header.timestamp=videoFrame->timestamp;
        ...
        if ((err = yang_h264_encodeVideo(context,rtp,&rtp->videoFuaPacket)) != Yang_Ok) {
            return yang_error_wrap(err, "encode packet");
        }
...
int32_t yang_h264_encodeVideo(YangRtcContext* context,YangH264RtpEncode* rtp,YangRtpPacket *pkt) {
    int err = 0;
   // rtp->buf.init(yang_get_rtpBuffer(rtp->rtpBuffer), kRtpPacketSize);
    yang_init_buffer(&rtp->buf,yang_get_rtpBuffer(rtp->rtpBuffer), kRtpPacketSize);
   ...
   // if (rtp->session)   return rtp->session->send_avpacket(pkt, &rtp->buf);
    return yang_send_avpacket(context,pkt, &rtp->buf);
    //return err;
}
int32_t yang_h264_encodeAudio(YangRtcContext* context,YangH264RtpEncode* rtp,YangRtpPacket *pkt) {
    int err = 0;
