...
int32_t yang_h264_encodeAudio(YangRtcContext *context, YangH264RtpEncode *rtp,
		YangRtpPacket *pkt);
int32_t yang_h264_encodeVideo(YangRtcContext *context, YangH264RtpEncode *rtp,
		YangRtpPacket *pkt);
int32_t yang_h264_package_fu_a(YangRtcContext *context, YangH264RtpEncode *rtp,
		YangFrame *videoFrame, int32_t fu_payload_size);
int32_t yang_h264_package_single_nalu(YangRtcContext *context,
		YangH264RtpEncode *rtp, char *p, int32_t plen, int64_t timestamp);
int32_t yang_h264_package_single_nalu2(YangRtcContext *context,
...
int32_t yang_pub_h264_audio(YangRtcContext *context, YangH264RtpEncode *rtp,
	...
	if ((err = yang_h264_encodeAudio(context, rtp, &rtp->audioRawPacket))
			!= Yang_Ok) {
...
int32_t yang_pub_h264_video(YangRtcContext *context, YangH264RtpEncode *rtp,
		YangFrame *videoFrame) {
	int32_t err = Yang_Ok;

	if (videoFrame->nb <= kRtpMaxPayloadSize) {
		if ((err = yang_h264_package_single_nalu2(context, rtp, videoFrame))
				!= Yang_Ok) {
			return yang_error_wrap(err, "package single nalu");
		}
	} else {
		if ((err = yang_h264_package_fu_a(context, rtp, videoFrame,
...
int32_t yang_pub_h264_package_stap_a(YangRtcContext *context,
		YangH264RtpEncode *rtp, YangFrame *videoFrame) {
	int err = Yang_Ok;
	...
	yang_decodeMetaH264(videoFrame->payload, videoFrame->nb, &sps_sample,
			&pps_sample);
	...
	if ((err = yang_h264_encodeVideo(context, rtp, &rtp->videoStapPacket))
			!= Yang_Ok) {
		return yang_error_wrap(err, "encode packet");
	}
	return err;
...
int32_t yang_h264_package_single_nalu2(YangRtcContext *context,
	...
	memcpy(rtp->videoRawData.payload, videoFrame->payload,
			rtp->videoRawData.nb);
	if ((err = yang_h264_encodeVideo(context, rtp, &rtp->videoRawPacket))
			!= Yang_Ok) {
		return yang_error_wrap(err, "encode packet");
	}
	return err;
...
int32_t yang_h264_package_fu_a(YangRtcContext *context, YangH264RtpEncode *rtp,
	...
	for (int32_t i = 0; i < num_of_packet; ++i) {
		...
		if ((err = yang_h264_encodeVideo(context, rtp, &rtp->videoFuaPacket))
				!= Yang_Ok) {
			return yang_error_wrap(err, "encode packet");
		}
