...
int32_t yang_send_avpacket(YangRtcContext *context, YangRtpPacket *pkt,
		YangBuffer *pbuf) {
	int32_t err = Yang_Ok;
	int32_t nn_encrypt = yang_buffer_pos(pbuf);
	//if(m_publish) m_publish->cache_nack(pkt,pbuf->data,nn_encrypt);
	if (context->pubStream)
		yang_rtcpublish_cache_nack(context->pubStream, pkt, pbuf->data,
				nn_encrypt);
...
void yang_do_request_keyframe(YangRtcContext *context, uint32_t ssrc) {
	if (context && context->context && context->context->yang_sendRequest)
		context->context->yang_sendRequest(context->streamConf->uid, ssrc,
