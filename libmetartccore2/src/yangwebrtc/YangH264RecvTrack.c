...
void yang_h264recv_copy(YangH264RecvTrack* recv,YangRtpPacket *src, YangH264PacketCache *pkt);
int32_t yang_h264recv_packet_video(YangRtcContext* context,YangH264RecvTrack* recv,const uint16_t start,
		const uint16_t end);
int32_t yang_h264recv_is_keyframe(YangRtpPacket *pkt);
int32_t yang_h264recv_is_keyframe2(YangH264PacketCache *pkt);
void yang_h264recv_copy(YangH264RecvTrack* recv,YangRtpPacket *src, YangH264PacketCache *pkt) ;
int32_t yang_h264recv_put_frame_video(YangRtcContext* context,YangH264RecvTrack* recv,char *p, int64_t timestamp,
		int32_t nb);
int32_t yang_h264recv_find_next_lost_sn(YangH264RecvTrack* recv,uint16_t current_sn,
		uint16_t *end_sn);
int32_t yang_h264recv_check_frame_complete(YangH264RecvTrack* recv,const uint16_t start,
		const uint16_t end) ;
...
void yang_init_h264recv(YangRtcContext* context,YangH264RecvTrack* recv,YangRtpBuffer *rtpBuffer){
	...
	memset(recv->cache_video_pkts, 0, Yang_Rtc_Recv_Buffer*sizeof(YangH264PacketCache));
...
int32_t yang_h264recv_packet_video(YangRtcContext* context,YangH264RecvTrack* recv,const uint16_t start,
	...
	YangH264PacketCache *header = &recv->cache_video_pkts[yang_cache_index(start)];
	...
	*p++=0x00;
	...
	for (uint16_t i = 0; i < cnt; ++i) {
		...
		if (pkt->nalu_type==kStapA) {
            ...
            if(recv->hasReceiveStap>-1&&recv->hasReceiveStap==pkt->sn){
				...
				for (int32_t j = 0; j < recv->stapData.nalus.vlen; ++j) {
					YangSample *sample = yang_get_sampleVector(&recv->stapData.nalus,j);//->at(j);
					if (sample->nb > 0) {
		...
		memset(&recv->cache_video_pkts[i],0,sizeof(struct YangH264PacketCache));
	}

	if ((err = yang_h264recv_put_frame_video(context,recv,recv->video_buffer, timestamp, p-recv->video_buffer)) != Yang_Ok) {
		yang_warn("fail to pack video frame");
	}

	recv->header_sn = end + 1;
	uint16_t tail_sn = 0;
	int32_t sn = yang_h264recv_find_next_lost_sn(recv,recv->header_sn, &tail_sn);
	if (-1 == sn) {
		if (yang_h264recv_check_frame_complete(recv,recv->header_sn, tail_sn)) {
			err = yang_h264recv_packet_video(context,recv,recv->header_sn, tail_sn);
		}
	} else if (-2 == sn) {
...
int32_t yang_h264recv_packet_video_key_frame(YangRtcContext* context,YangH264RecvTrack* recv,YangRtpPacket *pkt) {
	int32_t err = Yang_Ok;
	...
	if (pkt->nalu_type == kStapA) {
		...
		yang_reset_h2645_stap(&recv->stapData);
		yang_decode_h264_stap(&stap_buf, &recv->stapData);
		YangSample *sps = yang_get_h264_stap_sps(&recv->stapData); //recv->stap.get_sps();
		YangSample *pps = yang_get_h264_stap_pps(&recv->stapData); //recv->stap.get_pps();
		if (NULL == sps || NULL == pps) {
			return yang_error_wrap(ERROR_RTC_RTP_MUXER,
					"no sps or pps in stap-a rtp. sps: %p, pps:%p", sps, pps);
		} else {
			int32_t metaLen=0;
			yang_getConfig_Meta_H264(sps,pps,(uint8_t*)recv->video_buffer,&metaLen);

			recv->hasReceiveStap = pkt->header.sequence;
			yang_reset_h2645_stap(&recv->stapData);
			if(metaLen>0){
				if ((err = yang_h264recv_put_frame_video(context,recv,recv->video_buffer, 0, metaLen))
						!= Yang_Ok) {
					return err;
				}
			}
	...
	if (-1 == sn) {
		if (yang_h264recv_check_frame_complete(recv,recv->header_sn, tail_sn)) {
			...
			if ((err = yang_h264recv_packet_video(context,recv,recv->header_sn, tail_sn)) != Yang_Ok) {
...
//    }
	...
	if (recv->lost_sn == src->header.sequence) {
		uint16_t tail_sn = 0;
		int32_t sn = yang_h264recv_find_next_lost_sn(recv,recv->lost_sn, &tail_sn);
		if (-1 == sn) {
			if (yang_h264recv_check_frame_complete(recv,recv->header_sn, tail_sn)) {
				if ((err = yang_h264recv_packet_video(context,recv,recv->header_sn, tail_sn)) != Yang_Ok) {
...
int32_t yang_h264recv_packet_video_Raw(YangRtcContext* context,YangH264RecvTrack* recv,YangRtpPacket* pkt){
    int err=Yang_Ok;
    char* p=recv->video_buffer;
