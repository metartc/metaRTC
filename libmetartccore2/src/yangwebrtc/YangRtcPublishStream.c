#include <yangwebrtc/YangCRtcPublishStream.h>
...
//	pub->nack_enabled = 0;
		...
		pub->audio_queue = (YangPublishNackBuffer*)calloc(1,sizeof(YangPublishNackBuffer));//new YangPublishNackBuffer(100);
		yang_init_pubNackbuffer(pub->audio_queue,100);

		pub->video_queue = (YangPublishNackBuffer*)calloc(1,sizeof(YangPublishNackBuffer));//new YangPublishNackBuffer(1000);
		yang_init_pubNackbuffer(pub->video_queue,1000);

	pub->audioSsrc = audioSsrc;
	pub->videoSsrc = videoSsrc;
	//pub->nack_enabled = true;
}
...
int32_t yang_rtcpublish_on_rtcp_nack(YangRtcContext* context,YangRtcPublishStream* pub,YangRtcpCommon *rtcp) {
	...
	if (que&&(err = yang_rtcpublish_on_recv_nack(context,pub,que, rtcp->nack)) != Yang_Ok) {
		return yang_error_wrap(err, "track response nack.  ssrc=%u", ssrc);
	}
	return err;
}
...
YangSendNackBuffer* yang_rtcpublish_fetch_rtp_packet(YangRtcContext* context,YangRtcPublishStream* pub,YangPublishNackBuffer *que,
		uint16_t seq) {
	if(context==NULL||pub==NULL) return NULL;
...
int32_t yang_rtcpublish_on_recv_nack(YangRtcContext* context,YangRtcPublishStream* pub,YangPublishNackBuffer *que,YangRtcpNack* nack) {
	...
	for (int32_t i = 0; i < nack->vlen; ++i) {
		...
		YangSendNackBuffer *pkt = yang_rtcpublish_fetch_rtp_packet(context,pub,que, seq);
		if (pkt == NULL) {
			continue;
		}
        yang_trace("\nsendlostsn=%hu,",seq);

		//if ((err = pub->session->send_nackpacket(pkt->payload,pkt->nb)) != Yang_Ok) {
...
int32_t yang_rtcpublish_do_request_keyframe(YangRtcContext* context,uint32_t ssrc) {
	if(context==NULL) return 1;
	int32_t err = Yang_Ok;

	//pub->session->do_request_keyframe(ssrc);
...
int32_t yang_rtcpublish_on_rtcp_ps_feedback(YangRtcContext* context,YangRtcPublishStream* pub,YangRtcpCommon *rtcp) {
	...
	switch (fmt) {
	case kPLI: {
		uint32_t ssrc = pub->videoSsrc; //get_video_publish_ssrc(rtcp->get_media_ssrc());
		if (ssrc) {
