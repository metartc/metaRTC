...
//	pub->nack_enabled = 0;
	...
	pub->audio_queue = (YangPublishNackBuffer*) calloc(1,
			sizeof(YangPublishNackBuffer)); //new YangPublishNackBuffer(100);
	yang_init_pubNackbuffer(pub->audio_queue, 100);

	pub->video_queue = (YangPublishNackBuffer*) calloc(1,
			sizeof(YangPublishNackBuffer)); //new YangPublishNackBuffer(1000);
...
int32_t yang_rtcpublish_on_rtcp(YangRtcContext *context,
	...
	if (context == NULL || pub == NULL)
		return 1;
	yang_trace("\npublish rtcp size==%d,nb==%" PRId64, rtcp->nb_data,
			yang_get_rtcpCommon_nb_bytes(rtcp));
