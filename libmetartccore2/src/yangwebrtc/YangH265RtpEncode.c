#include <yangwebrtc/YangCH265RtpEncode.h>
...
int32_t yang_h265_encodeAudio(YangRtcContext* context,YangH265RtpEncode* rtp,YangRtpPacket *pkt);
int32_t yang_h265_encodeVideo(YangRtcContext* context,YangH265RtpEncode* rtp,YangRtpPacket *pkt);
int32_t yang_h265_package_fu_a(YangRtcContext* context,YangH265RtpEncode* rtp,YangFrame *videoFrame,
                                        int32_t fu_payload_size);
int32_t yang_h265_package_single_nalu(YangRtcContext* context,YangH265RtpEncode* rtp,char *p, int32_t plen,
                                               int64_t timestamp);
int32_t yang_h265_package_single_nalu2(YangRtcContext* context,YangH265RtpEncode* rtp,YangFrame *videoFrame);
...
void yang_destroy_h265RtpEncode(YangH265RtpEncode* rtp){
    ...
    //yang_delete(rtp->stapData.nalus);
...
int32_t yang_pub_h265_audio(YangRtcContext* context,YangH265RtpEncode* rtp,YangFrame* audioFrame){
   ...
   // rtp->audioRawPacket.reset();
    ...
    rtp->audioRawPacket.header.timestamp=audioFrame->timestamp;
    ...
    memcpy(rtp->audioRawData.payload, audioFrame->payload, rtp->audioRawData.nb);
    if ((err = yang_h265_encodeAudio(context,rtp,&rtp->audioRawPacket)) != Yang_Ok) {
...
int32_t yang_pub_h265_video(YangRtcContext* context,YangH265RtpEncode* rtp,YangFrame* videoFrame){
	int32_t err = Yang_Ok;
	    ...
	    if (videoFrame->nb <= kRtpMaxPayloadSize) {
	        if ((err = yang_h265_package_single_nalu2(context,rtp,videoFrame)) != Yang_Ok) {
	            return yang_error_wrap(err, "package single nalu");
	        }
	    } else {
	        if ((err = yang_h265_package_fu_a(context,rtp,videoFrame, kRtpMaxPayloadSize)) != Yang_Ok) {
...
int32_t yang_pub_h265_package_stap_a(YangRtcContext* context,YangH265RtpEncode* rtp,YangFrame* videoFrame){
	 int err = Yang_Ok;
	    ...
	    yang_push_sampleVector(&rtp->stapData.nalus,&vps_sample);
	    yang_push_sampleVector(&rtp->stapData.nalus,&sps_sample);
	    yang_push_sampleVector(&rtp->stapData.nalus,&pps_sample);



	    if ((err = yang_h265_encodeVideo(context,rtp,&rtp->videoStapPacket)) != Yang_Ok) {
	        return yang_error_wrap(err, "encode packet");
	    }
	    return err;
}
...
int32_t yang_h265_package_single_nalu2(YangRtcContext* context,YangH265RtpEncode* rtp,YangFrame *videoFrame) {
   ...
   // rtp->videoRawPacket.reset();
    ...
    if ((err = yang_h265_encodeVideo(context,rtp,&rtp->videoRawPacket)) != Yang_Ok) {
        return yang_error_wrap(err, "encode packet");
    }
    return err;
}
...
int32_t yang_h265_package_fu_a(YangRtcContext* context,YangH265RtpEncode* rtp,YangFrame *videoFrame,
    ...
    for (int32_t i = 0; i < num_of_packet; ++i) {
       ...
       // rtp->videoFuaPacket.reset();
        ...
        if ((err = yang_h265_encodeVideo(context,rtp,&rtp->videoFuaPacket)) != Yang_Ok) {
            return yang_error_wrap(err, "encode packet");
        }
