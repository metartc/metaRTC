...
void ssl_on_info(const SSL* dtls, int32_t where, int32_t ret)
{
    ...
    if (where & SSL_CB_LOOP) {
       ...
       // dtls_impl->on_dtls_alert(alert_type, alert_desc);
        yang_info("dtls alert ..type==%s,desc==%s",SSL_alert_type_string_long(ret),SSL_alert_desc_string(ret));
    }
}
			...
			int32_t err = Yang_Ok;
			   ...
			   int32_t arq_max_retry = 12 * 2;
			    for (int32_t i = 0; arq_count < arq_max_retry && i < max_loop; i++) {
			        ...
			        int32_t r0 = 0; struct timeval to = {0};
			        ...
			        r0 = BIO_reset(dtls->bioOut); int32_t r1 = SSL_get_error(dtls->ssl, r0);
			        if (r0 != 1) {
			        	return yang_error_wrap(ERROR_OpenSslBIOReset,"OpenSslBIORese BIO_reset r0=%d, r1=%d", r0, r1);
			        }

			       r0 = DTLSv1_handle_timeout(dtls->ssl); r1 = SSL_get_error(dtls->ssl, r0);
			        if (r0 == 0) {
...
SSL_CTX* yang_build_dtls_ctx(YangRtcDtls *dtls, char* role) {
 ...
 if (dtls->version == YangDtlsVersion1_2) {
	...
	if (true) {
        ...
        if(SSL_CTX_use_certificate(dtls_ctx,dtls->cer->dtls_cert) != 1) yang_error("SSL_CTX_use_certificate error");
        //yang_assert(SSL_CTX_use_PrivateKey(dtls_ctx,	m_cer->get_public_key()) == 1);
        if(SSL_CTX_use_PrivateKey(dtls_ctx,	dtls->cer->dtls_pkey) != 1)  yang_error("SSL_CTX_use_PrivateKey error");
   ...
   // stop();
    ...
    char send_key[50];
     ...
     if ((err = yang_get_srtp_key(dtls,recv_key, &recvKeyLen,send_key,&sendKeyLen)) != Yang_Ok) {
      ...
      if ((err = yang_init_srtp(dtls->srtp,recv_key,recvKeyLen, send_key,sendKeyLen)) != Yang_Ok) {
...
int32_t yang_startHandShake(YangRtcDtls* dtls){
	if(yang_doHandshake(dtls)) return 1;
			...
			for (int32_t i = 0; i < 1024 && BIO_ctrl_pending(dtls->bioIn) > 0; i++) {
				...
				int32_t r1 = SSL_get_error(dtls->ssl, r0);
				   if (r0 <= 0) {
					...
					uint8_t *data = NULL;
					int32_t size = BIO_get_mem_data(dtls->bioOut, (char** )&data);
					if(size>0&&dtls->udp) yang_rtc_sendData(dtls->udp,(char*)data,size);
					continue;
				}
...
int32_t yang_get_srtp_key(YangRtcDtls* dtls,char* recv_key, int *precvkeylen,char* send_key,int *psendkeylen){
	int32_t err = Yang_Ok;
		    ...
		    if (!SSL_export_keying_material(dtls->ssl, material, sizeof(material), dtls_srtp_lable, strlen(dtls_srtp_lable), NULL, 0, 0)) {
...
/**
YangRtcDtls::YangRtcDtls(struct YangRtcContext* pcontext) {
	m_context=pcontext;
	m_sslctx = NULL;
	m_ssl = NULL;
	m_bioIn = NULL;
...
int32_t YangRtcDtls::decodeHandshake() {
   ...
   int32_t arq_max_retry = 12 * 2;
    for (int32_t i = 0; arq_count < arq_max_retry && i < max_loop; i++) {
       ...
       r0 = DTLSv1_handle_timeout(m_ssl); r1 = SSL_get_error(m_ssl, r0);
        ...
        arq_count++;
        if (size > 0 && (err = yang_rtc_sendData(m_context->udp,(char*)data, size)) != Yang_Ok) {
             return yang_error_wrap(ERROR_RTC_DTLS,"error dtls send size=%u", size);
           }
    }
