#include <yangrtp/YangCRtpHeader.h>
...
int32_t yang_decode_rtpHeader(YangBuffer* buf,YangRtpHeader* header){
	int32_t err = Yang_Ok;
		...
		if (extension) {
			if(header->extensions==NULL) header->extensions=(YangRtpExtensions*)calloc(1,sizeof(YangRtpExtensions));// YangRtpExtensions();
