...
void yang_init_stap(YangRtpSTAPData* stap){
	if(stap==NULL) return;
	yang_init_YangSampleVector(&stap->nalus);
}
...
void yang_reset_h2645_stap(YangRtpSTAPData* data){
	if(data==NULL||data->nalus.vlen==0) return;
	data->nb=0;
	data->nri=0;
	data->payload=NULL;
...
YangSample* yang_get_h265_stap_sps(YangRtpSTAPData* data){
    if(data==NULL||data->nalus.vlen==0) return NULL;
    int32_t nn_nalus = (int) data->nalus.vlen;
    for (int32_t i = 0; i < nn_nalus; i++) {
        YangSample *p = &data->nalus.payload[i];
        if (!p || !p->nb) {
