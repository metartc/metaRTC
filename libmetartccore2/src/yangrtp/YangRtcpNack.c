...
void yang_rtcpNack_addSn(YangRtcpNack* nack,uint16_t sn){
	...
	if(nack->vsize>=nack->capacity-1){
		uint16_t* tmp=(uint16_t*)malloc(nack->vsize*sizeof(uint16_t));
		...
		memcpy(nack->nacks,tmp,nack->vsize*sizeof(uint16_t));
		yang_free(tmp);
		nack->capacity+=50;
	}
...
int32_t yang_decode_rtcpNack(YangRtcpCommon* comm,YangBuffer *buffer){
	...
	for (int32_t i = 0; i < (comm->header.length - 2); i++) {
		...
		if (vsize == 0) {
			...
			yang_insert_uint16(tmp, pid, &vsize);
		}
		memset(bitmask, 0, 20);
	...
	comm->nack->vsize = vsize;
	if (vsize)	memcpy(comm->nack->nacks, tmp, vsize * sizeof(uint16_t));
