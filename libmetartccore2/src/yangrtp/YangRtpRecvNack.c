#include <yangrtp/YangCRtpRecvNack.h>
...
{
	if(nack==NULL) return;
    //nack->queue.erase(seq);
	yang_nackinfo_remove(nack->queue.nackinfos,seq,&nack->queue.vlen);
}

YangRtpNackInfo* yang_recvnack_find(YangRtpRecvNack* nack,uint16_t seq)
{
	for(int i=0;i<nack->queue.vlen;i++){
...
{
    if (nack->queue.vlen >= nack->max_queue_size) {
    	yang_nackbuffer_notify_nack_list_full(nack->rtp);
        //nack->rtp->notify_nack_list_full();
    	for(int32_t i=0;i<nack->queue.vlen;i++){
...
bool yang_recvnack_isEmpty(YangRtpRecvNack* nack){
	return nack->queue.vlen==0?1:0;
}
void yang_recvnack_get_nack_seqs(YangRtpRecvNack* recvnack,YangRtcpNack* nack, uint32_t* timeout_nacks)
{
  ...
  // std::map<uint16_t, YangRtpNackInfo>::iterator iter = nack->queue.begin();
   int32_t vlen=0;
   uint16_t tmp[recvnack->queue.vlen];
   // while (iter != recvnack->queue.end()) {
   for(int i=0;i<recvnack->queue.vlen;i++){
        const uint16_t seq = recvnack->queue.nackinfos[i]->sn;//iter->first;
        YangRtpNackInfo* nack_info = recvnack->queue.nackinfos[i];//iter->second;
        ...
        if (now - nack_info->pre_req_nack_time_ >= nack_interval ) {
            ...
            yang_insert_uint16(tmp, seq, &vlen);
        }


    }
    yang_rtcpNack_addSns(nack,tmp,vlen);
    //if(vlen>0) memcpy(nack->nacks,tmp,vlen);
}
