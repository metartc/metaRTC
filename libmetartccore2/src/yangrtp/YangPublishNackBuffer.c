#include <yangrtp/YangCPublishNackBuffer.h>
...
void yang_init_pubNackbuffer(YangPublishNackBuffer* nackbuf,int32_t capacity){
	...
	nackbuf->queue =(YangSendNackBuffer**) malloc(sizeof(YangSendNackBuffer*)*nackbuf->capacity);//new YangSendNackBuffer*[nackbuf->capacity];
	memset(nackbuf->queue, 0, sizeof(YangSendNackBuffer*) * capacity);
}
