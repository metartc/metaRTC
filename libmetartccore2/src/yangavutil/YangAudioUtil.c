...
void yang_audiores_initParam(YangAudioResampleContext* context) {
	...
	context->sampleTrans = (context->inSample == context->outSample ? false : true);
	context->channelTrans = (context->inChannel == context->outChannel ? false : true);
	context->isTrans = (context->sampleTrans || context->channelTrans) ? true : false;
}
void yang_audiores_initIn(void* pcontext,int32_t psample, int32_t pchannel) {
	if(pcontext==NULL) return;
	YangAudioResampleContext* context=(YangAudioResampleContext*)pcontext;
...
void yang_audiores_initOut(void* pcontext,int32_t psample, int32_t pchannel) {
	if(pcontext==NULL) return;
	YangAudioResampleContext* context=(YangAudioResampleContext*)pcontext;
...
void yang_init_audioresample(YangAudioResample* res){
	...
	res->context->sampleTrans = false;
	res->context->channelTrans = false;
	res->context->isInitTrans = false;
	res->context->isTrans = true;
