...
void yang_createH264Meta(struct YangVideoMeta* pvmd,struct YangFrame *videoFrame){
	memset(pvmd,0,sizeof(struct YangVideoMeta));
...
void yang_getConfig_Flv_H264(struct YangH2645Conf *p_264, uint8_t *configBuf,
		int32_t *p_configLen) {
 ...
 *
 * */
void yang_getConfig_Meta_H264(struct YangSample* psps, struct YangSample* ppps,uint8_t *configBuf,int32_t *p_configLen){
...
void yang_getConfig_Meta_H265(struct YangSample* pvps,struct YangSample* psps, struct YangSample* ppps, uint8_t *configBuf,int32_t *p_configLen){
	int32_t i = 0;
...
void yang_decodeMetaH264(uint8_t *buf,int32_t p_configLen,struct YangSample* sps,struct YangSample* pps){
	sps->nb= *(buf + 12) + 1;
	sps->bytes = (char*)buf + 13;
	pps->nb = *(sps->bytes + sps->nb + 1) + 1;
	pps->bytes= (char*)buf + 13 + sps->nb + 2;
}
void yang_decodeMetaH265(uint8_t *meta,int32_t p_configLen,struct YangSample* vps,struct YangSample* sps,struct YangSample* pps){
	vps->nb=*(meta+32);
