...
#ifdef _WIN32
	packet.mapped_address=session->context.sock->session.remote_addr.sin_addr.S_un.S_addr;
#else
	packet.mapped_address=session->context.sock->session.remote_addr.sin_addr.s_addr;
#endif

	packet.mapped_port=htons(session->context.sock->session.remote_addr.sin_port);
