...
void* yang_run_rtcudp_thread(void *obj) {
	...
	YangIpAddress src;
	memset(&src, 0, sizeof(YangIpAddress));
	...
	while (sock->isLoop) {
		...
		if ((len = yang_socket_recvfrom(sock->fd, buffer, 2048,  &src)) > 0) {
			...
			if(sock->notRemoteInit){
				memcpy((char*)&sock->remote_addr,(char*)&src,sizeof(YangIpAddress));
...
int32_t yang_create_rtcsocket_srs(YangRtcSocket* psock,YangSocketProtocol protocol){
	...
	if(psock->session.buffer==NULL) psock->session.buffer=(char*)calloc(2,1);
	return Yang_Ok;
}
