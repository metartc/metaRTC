...
int32_t yang_rtc_sendData(YangRtcSocketSession *session, char *data, int32_t nb) {
	...
	return yang_socket_sendto(session->fd, data, nb, 0, (struct sockaddr*) &session->remote_addr,sizeof(struct sockaddr)) > 0 ? Yang_Ok : 1;
}

int32_t yang_rtc_sendData2(YangRtcSocketSession *session, struct sockaddr* remote_addr,char *data, int32_t nb) {
	if (session == NULL || !session->isStart || session->notRemoteInit || data==NULL)	return ERROR_RTC_UDP;
	return yang_socket_sendto(session->fd, data, nb, 0, remote_addr,sizeof(struct sockaddr)) > 0 ? Yang_Ok : 1;
}
...
void* yang_run_rtcudp_thread(void *obj) {
	...
	if (yang_socket_bind(sock->fd, (struct sockaddr*) &sock->local_addr,
	...
	while (sock->isLoop) {
		...
		if ((len = yang_socket_recvfrom(sock->fd, buffer, 2048, 0,	(struct sockaddr*) &src, &src_len)) > 0) {
...
void* yang_run_rtctcp_thread(void *obj) {
	...
	if (yang_socket_connect(sock->fd, (struct sockaddr*) &sock->remote_addr,
