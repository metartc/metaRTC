...
int32_t yang_rtc_turn_sendData(YangRtcSocketSession *session, char *data, int32_t nb) {
	...
	return yang_socket_sendto(session->fd, session->turnBuffer, nb + 4, &session->remote_addr) > 0 ? Yang_Ok : ERROR_RTC_TURN;
}
...
int32_t yang_turnconn_start(YangTurnSession *session) {
	if (session->isStart == 1)
		return Yang_Ok;
	int32_t err = Yang_Ok;
	if (session->sock->session.turnBuffer == NULL)
		session->sock->session.turnBuffer = (char*) calloc(1500, 1);
...
void yang_destroy_turnConnection(YangTurnConnection *conn) {
	...
	yang_destroy_rtcudp(session->sock);
