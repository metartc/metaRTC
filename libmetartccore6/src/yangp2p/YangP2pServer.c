...
void* yang_run_p2pserver_thread(void *obj) {
	...
	if (yang_socket_bind(p2p->serverfd, (struct sockaddr*) &p2p->local_addr,sizeof(struct sockaddr_in)) < 0) {
	...
	while (p2p->isLoop) {
		...
		int connfd = yang_socket_accept(p2p->serverfd, (struct sockaddr*) &src, &src_len);
		if(connfd>-1){
			yang_thread_t th;
			p2p->connFd=connfd;
