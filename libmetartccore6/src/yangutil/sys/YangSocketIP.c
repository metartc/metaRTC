...
void yang_getIp( char* domain, char* ip)
{
	...
	for (int i = 0; host->h_addr_list[i]; i++)
	{
		...
		yang_strcpy(ip,inet_ntoa(*(struct in_addr*)host->h_addr_list[i]));
		break;
	}
...
int32_t yang_getLocalInfo(char* ip){
	...
	for(int32_t i=0;;i++)
	{
		...
		if(yang_strstr(localIP,"127.0.0")||yang_memcmp(localIP,"169.",4)==0||yang_strstr(localIP,"192.168.56.")) continue;
		yang_strcpy(ip,localIP);
...
int32_t yang_getLocalInfoList(YangStringVector* vecs){
	...
	{
		char* localIP = inet_ntoa(*(IN_ADDR*)ph->h_addr_list[i]);
		if(yang_strstr(localIP,"127.0.0")) continue;
