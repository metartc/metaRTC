...
void Yang_Logs1(int32_t level, const char *format, ...) {
	...
	yang_vsnprintf(str, 2048 - 1, format, args);
	va_end(args);
}
#ifdef _WIN32
...
AMF_EncodeString(char *output, char *outend, const struct AVal *bv) {
	...
	yang_memcpy(output, bv->av_val, bv->av_len);
	output += bv->av_len;
...
AMF_EncodeNamedString(char *output, char *outend, const struct AVal *strName,
	...
	yang_memcpy(output, strName->av_val, strName->av_len);
...
AMFProp_Encode(AMFObjectProperty *prop, char *pBuffer, char *pBufEnd) {
	...
	if (prop->p_type != AMF_NULL && prop->p_name.av_len) {
		...
		*pBuffer++ = prop->p_name.av_len & 0xff;
		yang_memcpy(pBuffer, prop->p_name.av_val, prop->p_name.av_len);
...
void AMFProp_Dump(AMFObjectProperty *prop) {
	...
	yang_snprintf(strRes, 255, "Name: %18.*s, ", name.av_len, name.av_val);
	...
	switch (prop->p_type) {
	case AMF_NUMBER:
		yang_snprintf(str, 255, "NUMBER:\t%.2f", prop->p_vu.p_number);
		break;
	case AMF_BOOLEAN:
		yang_snprintf(str, 255, "BOOLEAN:\t%s",
				prop->p_vu.p_number != 0.0 ? "TRUE" : "FALSE");
		break;
	case AMF_STRING:
		yang_snprintf(str, 255, "STRING:\t%.*s", prop->p_vu.p_aval.av_len,
				prop->p_vu.p_aval.av_val);
		break;
	case AMF_DATE:
		yang_snprintf(str, 255, "DATE:\ttimestamp: %.2f, UTC offset: %d",
				prop->p_vu.p_number, prop->p_UTCoffset);
		break;
	default:
		yang_snprintf(str, 255, "INVALID TYPE 0x%02x", (uint8_t) prop->p_type);
	}
...
void AMF_AddProp(AMFObject *obj, const AMFObjectProperty *prop) {
	if (!(obj->o_num & 0x0f))
		obj->o_props = (struct AMFObjectProperty*) yang_realloc(obj->o_props,
				(obj->o_num + 16) * sizeof(AMFObjectProperty));
	yang_memcpy(&obj->o_props[obj->o_num++], prop, sizeof(AMFObjectProperty));
}
...
void AMF3CD_AddProp(AMF3ClassDef *cd, struct AVal *prop) {
	if (!(cd->cd_num & 0x0f))
		cd->cd_props = (struct AVal*) yang_realloc(cd->cd_props,
