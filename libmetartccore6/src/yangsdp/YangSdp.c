...
void  yang_sdp_genLocalSdp_candidate(YangRtcSession *session,YangMediaDesc *media_desc,char* localIp,int32_t localport){
    ...
    if(session->ice.session.candidateType>YangIceHost){
    	...
    	//strcpy(media_desc->candidates.payload[index].type, session->ice.session.candidateType==YangIceTurn?"host":"srflx");
    }else{
...
void yang_sdp_init_direction(YangMediaDesc* media_desc,YangStreamOptType role){
	if (role == Yang_Stream_Play) {
		media_desc->recvonly = true;


	} else if (role == Yang_Stream_Publish) {
		media_desc->sendonly = true;

	} else if (role == Yang_Stream_Both) {
		media_desc->sendrecv = true;
...
int32_t yang_sdp_genLocalSdp2(YangRtcSession *session, int32_t localport,char *dst, YangStreamOptType role) {
	...
	sprintf(audio_media_desc->mid,"%d",midNum++);
	...
	sprintf(video_media_desc->mid,"%d",midNum++);
	...
	if(session->context.avinfo->rtc.enableDatachannel){
		...
		sprintf(data_media_desc->mid,"%d",midNum++);

	}
#endif
	//char group_policy[32]={0};
	sprintf(local_sdp->group_policy,"%s","BUNDLE");
	for(int a=0;a<midNum;a++){
		sprintf(local_sdp->group_policy,"%s %d",local_sdp->group_policy,a);
	}
   ...
   if(mediaServer==Yang_Server_P2p){
	    ...
	    yang_create_YangCandidateVector(&video_media_desc->candidates);
	   	...
	   	}else if(localip&&strlen(localip)>0){
#if Yang_Enable_RTC_Audio
		...
		for(int32_t i=0;i<sdp->media_descs.vsize;i++){
			...
			if(yang_strcmp(desc->type,"audio")==0){
				...
				for(k=0;k<desc->payload_types.vsize;k++){
					...
					}else if(yang_strcmp(payload->encoding_name,Yang_AV1_Name)==0){
						session->context.codec=Yang_VED_AV1;
						session->remote_video->encode=Yang_VED_AV1;
					}else if(yang_strcmp(payload->encoding_name,"MJPEG")==0){
