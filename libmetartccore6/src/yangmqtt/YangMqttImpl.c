...
YangmqttErrors yang_mqtt_sync(yang_mqtt_client *client) {
   // Recover from any errors
    YangmqttErrors err;
    int32_t reconnecting = 0;
    yang_thread_mutex_lock(&client->mutex);
    if (client->error != MQTT_ERROR_RECONNECTING && client->error != MQTT_OK && client->reconnect_callback != NULL) {
        ...
        // yang_mqtt_reconnect will have queued the disconnect packet - that needs to be sent and then call reconnect =
        if (client->error == MQTT_ERROR_RECONNECTING) {
            reconnecting = 1;
            client->error = MQTT_OK;
        }
        yang_thread_mutex_unlock(&client->mutex);
    }


    err = (YangmqttErrors)__yang_mqtt_recv(client);
    if (err != MQTT_OK) return err;

    err = (YangmqttErrors)__yang_mqtt_send(client);
...
uint16_t __yang_mqtt_next_pid(yang_mqtt_client *client) {
    int32_t pid_exists = 0;
    if (client->pid_lfsr == 0) {
        client->pid_lfsr = 163u;
    }

    do {
        yang_mqtt_queued_message *curr;
       ...
       // check that the PID is unique
        pid_exists = 0;
        for(curr = yang_mqtt_mq_get(&(client->mq), 0); curr >= client->mq.queue_tail; --curr) {
               ...
               void (*publish_response_callback)(void* state,yang_mqtt_response_publish *publish))
{
    ...
    yang_thread_mutex_init(&client->mutex,NULL);
    yang_thread_mutex_lock(&client->mutex);

    client->socketfd = sockfd;

    yang_mqtt_mq_init(&client->mq, sendbuf, sendbufsz);
                         ...
                         void (*publish_response_callback)(void* state, yang_mqtt_response_publish *publish))
{
    ...
    client->socketfd = (yang_socket_t) -1;

    yang_mqtt_mq_init(&client->mq, NULL, 0);
...
#define MQTT_CLIENT_TRY_PACK(tmp, msg, client, pack_call, release)  \
    if (client->error < 0) {                                        \
        if (release) yang_thread_mutex_unlock(&client->mutex);         \
        return client->error;                                       \
    }                                                               \
    tmp = pack_call;                                                \
    if (tmp < 0) {                                                  \
        client->error = (YangmqttErrors)tmp;                                        \
        if (release) yang_thread_mutex_unlock(&client->mutex);         \
        return (YangmqttErrors)tmp;                                                 \
    } else if (tmp == 0) {                                          \
        yang_mqtt_mq_clean(&client->mq);                                 \
        tmp = pack_call;                                            \
        if (tmp < 0) {                                              \
            client->error = (YangmqttErrors)tmp;                                    \
            if (release) yang_thread_mutex_unlock(&client->mutex);     \
            ...
            if (release) yang_thread_mutex_unlock(&client->mutex);     \
            return (YangmqttErrors)MQTT_ERROR_SEND_BUFFER_IS_FULL;                  \
        }                                                           \
    }                                                               \
    msg = yang_mqtt_mq_register(&client->mq, (size_t)tmp);                       \
...
//Note: Current thread already has mutex locked.
    ...
    MQTT_CLIENT_TRY_PACK(rv, msg, client, 
        yang_mqtt_pack_connection_request(
                     ...
                     uint8_t publish_flags)
{
    ...
    uint16_t packet_id;
    yang_thread_mutex_lock(&client->mutex);
    packet_id = __yang_mqtt_next_pid(client);



    MQTT_CLIENT_TRY_PACK(
        rv, msg, client, 
        yang_mqtt_pack_publish_request(
            client->mq.curr, client->mq.curr_sz,
            topic_name,
            packet_id,
            application_message,
                       ...
                       int32_t max_qos_level)
{
    ...
    MQTT_CLIENT_TRY_PACK(
        rv, msg, client, 
        yang_mqtt_pack_subscribe_request(
            client->mq.curr, client->mq.curr_sz,
            packet_id,
            topic_name,
            max_qos_level,
...
YangmqttErrors yang_mqtt_unsubscribe(yang_mqtt_client *client,
                         const char* topic_name)
{
    uint16_t packet_id = __yang_mqtt_next_pid(client);
    ssize_t rv;
    yang_mqtt_queued_message *msg;
    yang_thread_mutex_lock(&client->mutex);
    ...
    MQTT_CLIENT_TRY_PACK(
        rv, msg, client, 
        yang_mqtt_pack_unsubscribe_request(
            client->mq.curr, client->mq.curr_sz,
            packet_id,
            topic_name,
            (const char*)NULL
...
{
    YangmqttErrors err = yang_mqtt_disconnect(client);
...
ssize_t __yang_mqtt_send(yang_mqtt_client *client)
{
   ...
   //loop through all messages in the queue
    len = yang_mqtt_mq_length(&client->mq);
    for(; i < len; ++i) {
        yang_mqtt_queued_message *msg = yang_mqtt_mq_get(&client->mq, i);
        int32_t resend = 0;
        if (msg->state == MQTT_QUEUED_UNSENT) {
           //message has not been sent to lets send it
            ...
            /* check for timeout */
            if (yang_pal_time() > msg->time_sent + client->response_timeout) {
                resend = 1;
       ...
       //we're sending the message
        {
          ssize_t tmp = yang_mqtt_sendmsg(client->socketfd, msg->start + client->send_offset, msg->size - client->send_offset);
          if (tmp < 0) {
            client->error = (YangmqttErrors)tmp;
            yang_thread_mutex_unlock(&client->mutex);
            return tmp;
      ...
      // update timeout watcher
        client->time_of_last_send = yang_pal_time();
  ...
  // check for keep-alive
    {
    	yang_time_t keep_alive_timeout = client->time_of_last_send + (yang_time_t)((float)(client->keep_alive));
        if (yang_pal_time() > keep_alive_timeout) {
          ssize_t rv = __yang_mqtt_ping(client);
          if (rv != MQTT_OK) {
            client->error = (YangmqttErrors)rv;
            yang_thread_mutex_unlock(&client->mutex);
            return rv;
          }
        }
...
{
    yang_mqtt_response response;
    ssize_t yang_mqtt_recv_ret = MQTT_OK;
    yang_thread_mutex_lock(&client->mutex);

   //read until there is nothing left to read, or there was an error
    while(yang_mqtt_recv_ret == MQTT_OK) {

        ssize_t rv, consumed;
        yang_mqtt_queued_message *msg = NULL;

        rv = yang_mqtt_recvmsg(client->socketfd, client->recv_buffer.curr, client->recv_buffer.curr_sz, 0);
        if (rv < 0) {
            /* an error occurred */
            client->error = (YangmqttErrors)rv;
            yang_thread_mutex_unlock(&client->mutex);
        ...
        /* attempt to parse */
        consumed = yang_mqtt_unpack_response(&response, client->recv_buffer.mem_start, (size_t) (client->recv_buffer.curr - client->recv_buffer.mem_start));

        if (consumed < 0) {
            client->error = (YangmqttErrors)consumed;
            yang_thread_mutex_unlock(&client->mutex);
            return consumed;
        } else if (consumed == 0) {
        ...
        */
        switch (response.fixed_header.control_type) {
            case MQTT_CONTROL_CONNACK:
                /* release associated CONNECT */
                msg = yang_mqtt_mq_find(&client->mq, MQTT_CONTROL_CONNECT, NULL);
                if (msg == NULL) {
                    client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                    yang_mqtt_recv_ret = MQTT_ERROR_ACK_OF_UNKNOWN;
                    break;
                }
                msg->state = MQTT_QUEUED_COMPLETE;
              // initialize typical response time
                client->typical_response_time = (double) (yang_pal_time() - msg->time_sent);
               //check that connection was successful
                if (response.decoded.connack.return_code != MQTT_CONNACK_ACCEPTED) {
                    if (response.decoded.connack.return_code == MQTT_CONNACK_REFUSED_IDENTIFIER_REJECTED) {
                        ...
                        yang_mqtt_recv_ret = MQTT_ERROR_CONNECT_CLIENT_ID_REFUSED;
                    } else {
                        client->error = MQTT_ERROR_CONNECTION_REFUSED;
                        yang_mqtt_recv_ret = MQTT_ERROR_CONNECTION_REFUSED;
                    }
                    break;
                }
                break;
            case MQTT_CONTROL_PUBLISH:
               // stage response, none if qos==0, PUBACK if qos==1, PUBREC if qos==2
                if (response.decoded.publish.qos_level == 1) {
                    rv = __yang_mqtt_puback(client, response.decoded.publish.packet_id);
                    if (rv != MQTT_OK) {
                        client->error = (YangmqttErrors)rv;
                        yang_mqtt_recv_ret = rv;
                        break;
                    }
                } else if (response.decoded.publish.qos_level == 2) {
                   //check if this is a duplicate
                    if (yang_mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBREC, &response.decoded.publish.packet_id) != NULL) {
                        break;
                    }

                    rv = __yang_mqtt_pubrec(client, response.decoded.publish.packet_id);
                    if (rv != MQTT_OK) {
                        client->error = (YangmqttErrors)rv;
                        yang_mqtt_recv_ret = rv;
                        break;
            ...
            case MQTT_CONTROL_PUBACK:
                //release associated PUBLISH
                msg = yang_mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBLISH, &response.decoded.puback.packet_id);
                if (msg == NULL) {
                    client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                    yang_mqtt_recv_ret = MQTT_ERROR_ACK_OF_UNKNOWN;
                    break;
                ...
                client->typical_response_time = 0.875 * (client->typical_response_time) + 0.125 * (double) (yang_pal_time() - msg->time_sent);
                break;
            case MQTT_CONTROL_PUBREC:
                //check if this is a duplicate
                if (yang_mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBREL, &response.decoded.pubrec.packet_id) != NULL) {
                    break;
                }
               //release associated PUBLISH
                msg = yang_mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBLISH, &response.decoded.pubrec.packet_id);
                if (msg == NULL) {
                    client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                    yang_mqtt_recv_ret = MQTT_ERROR_ACK_OF_UNKNOWN;
                    break;
               ...
               // update response time
                client->typical_response_time = 0.875 * (client->typical_response_time) + 0.125 * (double) (yang_pal_time() - msg->time_sent);
                // stage PUBREL
                rv = __yang_mqtt_pubrel(client, response.decoded.pubrec.packet_id);
                if (rv != MQTT_OK) {
                    client->error = (YangmqttErrors)rv;
                    yang_mqtt_recv_ret = rv;
                    break;
            ...
            case MQTT_CONTROL_PUBREL:
               // release associated PUBREC
                msg = yang_mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBREC, &response.decoded.pubrel.packet_id);
                if (msg == NULL) {
                    client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                    yang_mqtt_recv_ret = MQTT_ERROR_ACK_OF_UNKNOWN;
                    break;
              ...
              // update response time
                client->typical_response_time = 0.875 * (client->typical_response_time) + 0.125 * (double) (yang_pal_time() - msg->time_sent);
                // stage PUBCOMP
                rv = __yang_mqtt_pubcomp(client, response.decoded.pubrec.packet_id);
                if (rv != MQTT_OK) {
                    client->error = (YangmqttErrors)rv;
                    yang_mqtt_recv_ret = rv;
                    break;
            ...
            case MQTT_CONTROL_PUBCOMP:
                ...
                msg = yang_mqtt_mq_find(&client->mq, MQTT_CONTROL_PUBREL, &response.decoded.pubcomp.packet_id);
                if (msg == NULL) {
                    client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                    yang_mqtt_recv_ret = MQTT_ERROR_ACK_OF_UNKNOWN;
                    break;
            ...
            case MQTT_CONTROL_SUBACK:
                ...
                msg = yang_mqtt_mq_find(&client->mq, MQTT_CONTROL_SUBSCRIBE, &response.decoded.suback.packet_id);
                if (msg == NULL) {
                    client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                    yang_mqtt_recv_ret = MQTT_ERROR_ACK_OF_UNKNOWN;
                    break;
               ...
               // update response time
                ...
                if (response.decoded.suback.return_codes[0] == MQTT_SUBACK_FAILURE) {
                    ...
                    yang_mqtt_recv_ret = MQTT_ERROR_SUBSCRIBE_FAILED;
                    break;
                }
                break;
            case MQTT_CONTROL_UNSUBACK:
               // release associated UNSUBSCRIBE
                msg = yang_mqtt_mq_find(&client->mq, MQTT_CONTROL_UNSUBSCRIBE, &response.decoded.unsuback.packet_id);
                if (msg == NULL) {
                    client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                    yang_mqtt_recv_ret = MQTT_ERROR_ACK_OF_UNKNOWN;
                    break;
            ...
            case MQTT_CONTROL_PINGRESP:
                ...
                msg = yang_mqtt_mq_find(&client->mq, MQTT_CONTROL_PINGREQ, NULL);
                if (msg == NULL) {
                    client->error = MQTT_ERROR_ACK_OF_UNKNOWN;
                    yang_mqtt_recv_ret = MQTT_ERROR_ACK_OF_UNKNOWN;
                    break;
            ...
            default:
                ...
                yang_mqtt_recv_ret = MQTT_ERROR_MALFORMED_RESPONSE;
                break;
        }
        {
...
} yang_mqtt_fixed_header_rules_s;
yang_mqtt_fixed_header_rules_s yang_mqtt_fixed_header_rules ={
...
static ssize_t yang_mqtt_fixed_header_rule_violation(const yang_mqtt_fixed_header *fixed_header) {
    ...
    required_flags = yang_mqtt_fixed_header_rules.required_flags[control_type];
    mask_required_flags = yang_mqtt_fixed_header_rules.mask_required_flags[control_type];

   //check for valid type
    if (!yang_mqtt_fixed_header_rules.control_type_is_valid[control_type]) {
...
ssize_t yang_mqtt_unpack_fixed_header(yang_mqtt_response *response, const uint8_t *buf, size_t bufsz) {
    ...
    errcode = yang_mqtt_fixed_header_rule_violation(fixed_header);
    if (errcode) {
        return errcode;
    }
...
{ 
    ...
    // yang_mqtt_string length is strlen + 2
    remaining_length += __yang_mqtt_packed_cstrlen(client_id);

    if (will_topic != NULL) {
        uint8_t temp;
        // there is a will
        connect_flags |= MQTT_CONNECT_WILL_FLAG;
        remaining_length += __yang_mqtt_packed_cstrlen(will_topic);
    ...
    if (user_name != NULL) {
        ...
        remaining_length += __yang_mqtt_packed_cstrlen(user_name);
    } else {
    ...
    if (password != NULL) {
        ...
        remaining_length += __yang_mqtt_packed_cstrlen(password);
    } else {
    ...
    rv = yang_mqtt_pack_fixed_header(buf, bufsz, &fixed_header);
    if (rv <= 0) {
    ...
    *buf++ = (uint8_t) 'T';
    *buf++ = (uint8_t) 'T';
    *buf++ = YANG_MQTT_PROTOCOL_LEVEL;
    *buf++ = connect_flags;
    buf += __yang_mqtt_pack_uint16(buf, keep_alive);

    // pack the payload
    buf += __yang_mqtt_pack_str(buf, client_id);
    if (connect_flags & MQTT_CONNECT_WILL_FLAG) {
        buf += __yang_mqtt_pack_str(buf, will_topic);
        buf += __yang_mqtt_pack_uint16(buf, (uint16_t)will_message_size);
        yang_memcpy(buf, will_message, will_message_size);
        buf += will_message_size;
    }
    if (connect_flags & MQTT_CONNECT_USER_NAME) {
        buf += __yang_mqtt_pack_str(buf, user_name);
    }
    if (connect_flags & MQTT_CONNECT_PASSWORD) {
        buf += __yang_mqtt_pack_str(buf, password);
    }
...
ssize_t yang_mqtt_unpack_connack_response(yang_mqtt_response *yang_mqtt_response, const uint8_t *buf) {
    const uint8_t *const start = buf;
    yang_mqtt_response_connack *response;

    // check that remaining length is 2
    if (yang_mqtt_response->fixed_header.remaining_length != 2) {
        return MQTT_ERROR_MALFORMED_RESPONSE;
    }
    
    response = &(yang_mqtt_response->decoded.connack);
    // unpack
    if (*buf & 0xFE) {
    ...
    if (*buf > 5u) {
        ...
        response->return_code = (MQTTConnackReturnCode) *buf++;
    }
    return buf - start;
}
...
ssize_t yang_mqtt_pack_disconnect(uint8_t *buf, size_t bufsz) {
    ...
    fixed_header.remaining_length = 0;
    return yang_mqtt_pack_fixed_header(buf, bufsz, &fixed_header);
}
                                  ...
                                  uint8_t publish_flags)
{
    ...
    remaining_length = (uint32_t)__yang_mqtt_packed_cstrlen(topic_name);
    if (inspected_qos > 0) {
        remaining_length += 2;
    }
    ...
    buf += __yang_mqtt_pack_str(buf, topic_name);
    if (inspected_qos > 0) {
        buf += __yang_mqtt_pack_uint16(buf, packet_id);
    }

    // pack payload
    yang_memcpy(buf, application_message, application_message_size);
...
{    
    const uint8_t *const start = buf;
    yang_mqtt_fixed_header *fixed_header;
    yang_mqtt_response_publish *response;
    
    fixed_header = &(yang_mqtt_response->fixed_header);
    response = &(yang_mqtt_response->decoded.publish);
    ...
    if (yang_mqtt_response->fixed_header.remaining_length < 4) {
        return MQTT_ERROR_MALFORMED_RESPONSE;
    }

    // parse variable header
    response->topic_name_size = __yang_mqtt_unpack_uint16(buf);
    buf += 2;
    response->topic_name = buf;
    buf += response->topic_name_size;

    if (response->qos_level > 0) {
        response->packet_id = __yang_mqtt_unpack_uint16(buf);
        buf += 2;
    }
...
ssize_t yang_mqtt_unpack_pubxxx_response(yang_mqtt_response *yang_mqtt_response, const uint8_t *buf)
{
    ...
    // parse packet_id
    packet_id = __yang_mqtt_unpack_uint16(buf);
    buf += 2;

    if (yang_mqtt_response->fixed_header.control_type == MQTT_CONTROL_PUBACK) {
        yang_mqtt_response->decoded.puback.packet_id = packet_id;
    } else if (yang_mqtt_response->fixed_header.control_type == MQTT_CONTROL_PUBREC) {
        yang_mqtt_response->decoded.pubrec.packet_id = packet_id;
    } else if (yang_mqtt_response->fixed_header.control_type == MQTT_CONTROL_PUBREL) {
        yang_mqtt_response->decoded.pubrel.packet_id = packet_id;
    } else {
        yang_mqtt_response->decoded.pubcomp.packet_id = packet_id;
    }
...
ssize_t yang_mqtt_unpack_suback_response (yang_mqtt_response *yang_mqtt_response, const uint8_t *buf) {
    const uint8_t *const start = buf;
    uint32_t remaining_length = yang_mqtt_response->fixed_header.remaining_length;
    ...
    // unpack packet_id
    yang_mqtt_response->decoded.suback.packet_id = __yang_mqtt_unpack_uint16(buf);
    buf += 2;
    remaining_length -= 2;

    // unpack return codes
    yang_mqtt_response->decoded.suback.num_return_codes = (size_t) remaining_length;
    yang_mqtt_response->decoded.suback.return_codes = buf;
...
// SUBSCRIBE
ssize_t yang_mqtt_pack_subscribe_request(uint8_t *buf, size_t bufsz, uint32_t packet_id, ...) {
    va_list args;
    const uint8_t *const start = buf;
    ssize_t rv;
    yang_mqtt_fixed_header fixed_header;
    ...
    for(i = 0; i < num_subs; ++i) {
        ...
        fixed_header.remaining_length += __yang_mqtt_packed_cstrlen(topic[i]) + 1;
    }
    ...
    // pack variable header
    buf += __yang_mqtt_pack_uint16(buf, (uint16_t)packet_id);


    // pack payload
    for(i = 0; i < num_subs; ++i) {
        buf += __yang_mqtt_pack_str(buf, topic[i]);
        *buf++ = max_qos[i];
    }
...
ssize_t yang_mqtt_unpack_unsuback_response(yang_mqtt_response *yang_mqtt_response, const uint8_t *buf)
{
    ...
    // parse packet_id
    yang_mqtt_response->decoded.unsuback.packet_id = __yang_mqtt_unpack_uint16(buf);
    buf += 2;
...
// UNSUBSCRIBE
ssize_t yang_mqtt_pack_unsubscribe_request(uint8_t *buf, size_t bufsz, uint32_t packet_id, ...) {
    va_list args;
    const uint8_t *const start = buf;
    ssize_t rv;
    yang_mqtt_fixed_header fixed_header;
    ...
    for(i = 0; i < num_subs; ++i) {
        ...
        fixed_header.remaining_length += __yang_mqtt_packed_cstrlen(topic[i]);
    }
...
{  
    if(buf != NULL)
    {
        ...
        mq->curr = (uint8_t *)buf;
        mq->queue_tail = (yang_mqtt_queued_message *)mq->mem_end;
        mq->curr_sz = (size_t) (yang_mqtt_mq_currsz(mq));
    }
}
...
void yang_mqtt_mq_clean(yang_mqtt_message_queue *mq) {
    yang_mqtt_queued_message *new_head;

    for(new_head = yang_mqtt_mq_get(mq, 0); new_head >= mq->queue_tail; --new_head) {
    ...
    if (new_head < mq->queue_tail) {
        ...
        mq->curr_sz = (size_t) (yang_mqtt_mq_currsz(mq));
        return;
    } else if (new_head == yang_mqtt_mq_get(mq, 0)) {
        // do nothing
        return;
    }
    ...
    // move buffered data
    {
        ...
        yang_memmove(mq->mem_start, new_head->start, n);
        ...
        // move queue
        {
            ...
            yang_memmove(yang_mqtt_mq_get(mq, new_tail_idx), mq->queue_tail, sizeof(yang_mqtt_queued_message) * (size_t) ((new_tail_idx + 1)));
            mq->queue_tail = yang_mqtt_mq_get(mq, new_tail_idx);
          
            {
                // bump back start's
                ssize_t i = 0;
                for(; i < new_tail_idx + 1; ++i) {
                    yang_mqtt_mq_get(mq, i)->start -= removing;
                }
            }
        }
    }
...
{
    yang_mqtt_queued_message *curr;
    for(curr = yang_mqtt_mq_get(mq, 0); curr >= mq->queue_tail; --curr) {
...
ssize_t yang_mqtt_unpack_response(yang_mqtt_response* response, const uint8_t *buf, size_t bufsz) {
    const uint8_t *const start = buf;
    ssize_t rv = yang_mqtt_unpack_fixed_header(response, buf, bufsz);
    if (rv <= 0) return rv;
    else buf += rv;
    switch(response->fixed_header.control_type) {
        case MQTT_CONTROL_CONNACK:
            rv = yang_mqtt_unpack_connack_response(response, buf);
            break;
        case MQTT_CONTROL_PUBLISH:
            rv = yang_mqtt_unpack_publish_response(response, buf);
            break;
        case MQTT_CONTROL_PUBACK:
            rv = yang_mqtt_unpack_pubxxx_response(response, buf);
            break;
        case MQTT_CONTROL_PUBREC:
            rv = yang_mqtt_unpack_pubxxx_response(response, buf);
            break;
        ...
        case MQTT_CONTROL_PUBCOMP:
            ...
            break;
        case MQTT_CONTROL_SUBACK:
            rv = yang_mqtt_unpack_suback_response(response, buf);
            break;
        case MQTT_CONTROL_UNSUBACK:
            rv = yang_mqtt_unpack_unsuback_response(response, buf);
            break;
        case MQTT_CONTROL_PINGRESP:
            return rv;
        default:
...
{
  uint16_t integer_htons = yang_htons(integer);
  yang_memcpy(buf, &integer_htons, 2);
  return 2;
}
...
uint16_t __yang_mqtt_unpack_uint16(const uint8_t *buf)
{
  ...
  yang_memcpy(&integer_htons, buf, 2);
  return yang_ntohs(integer_htons);
}

ssize_t __yang_mqtt_pack_str(uint8_t *buf, const char* str) {
    uint16_t length = (uint16_t)yang_strlen(str);
    int32_t i = 0;
     // pack string length
    buf += __yang_mqtt_pack_uint16(buf, length);
...
const char* yang_mqtt_error_str(YangmqttErrors error) {
    int32_t offset = error - MQTT_ERROR_UNKNOWN;
    if (offset >= 0) {
