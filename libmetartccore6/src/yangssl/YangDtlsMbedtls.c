...
int32_t yang_get_srtp_key(YangDtlsSession *session, char *precv_key, int *precvkeylen,
	...
	memcpy(send_key, &keyingMaterialBuffer[offset], SRTP_MASTER_KEY_KEY_LEN);
	offset += SRTP_MASTER_KEY_KEY_LEN;

	memcpy(recv_key, &keyingMaterialBuffer[offset], SRTP_MASTER_KEY_KEY_LEN);
	offset += SRTP_MASTER_KEY_KEY_LEN;

	memcpy(send_key + SRTP_MASTER_KEY_KEY_LEN, &keyingMaterialBuffer[offset], SRTP_MASTER_KEY_SALT_LEN);
...
int32_t yang_mbed_sendCallback(void* user, const unsigned char* data, size_t len)
{
	...
	if (len > 0&& dtls->udp->sendData(&dtls->udp->session,(char*)data, len)!= Yang_Ok) {
		return yang_error_wrap(ERROR_RTC_DTLS, "error dtls send size=%u",len);
	}
...
int32_t yang_create_rtcdtls(YangRtcDtls *dtls,int32_t isServer) {
	...
	session->ssl=(mbedtls_ssl_context*)calloc(sizeof(mbedtls_ssl_context),1);
