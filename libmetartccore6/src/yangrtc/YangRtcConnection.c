...
void g_yang_doTask(int32_t taskId, void *user) {
	...
	if (!session->isServer && session->isSendStun && taskId == 1) {
		...
		if (session->context.stun.data&& session->context.stun.nb > 0) {
			if(session->context.sock->sendData(&session->context.sock->session,session->context.stun.data, session->context.stun.nb)!=Yang_Ok){
...
void yang_rtcconn_init(YangRtcSession *session, YangStreamOptType role) {
    ...
    if (role == Yang_Stream_Publish || role==Yang_Stream_Both)  {
    	if(session->pushAudio==NULL){
    		session->pushAudio=(YangPushAudio*) calloc(1,sizeof(YangPushAudio));
    	}
    	session->pushH264 = NULL;
        ...
        session->pushAV1 = NULL;
        if (session->codec == Yang_VED_AV1) {
            session->pushAV1 = (YangPushAV1*) calloc(1,sizeof(YangPushAV1));
        }
#endif
#if	Yang_Enable_MJpeg_Encoding
        session->pushMjpeg = NULL;
        if (session->codec == Yang_VED_MJPEG) {
            session->pushMjpeg = (YangPushMjpeg*) calloc(1,sizeof(YangPushMjpeg));
        }
#endif
    }
	//  session->20ms
...
#if	Yang_Enable_AV1_Encoding
		if (session->pushAV1) {
			yang_create_pushAV1(session->pushAV1,  session->pushVideoRtpBuffer);
		}
#endif
#if	Yang_Enable_MJpeg_Encoding
		if (session->pushMjpeg) {
			yang_create_pushMjpeg(session->pushMjpeg,  session->pushVideoRtpBuffer);
		}
#endif
...
#if	Yang_Enable_AV1_Encoding
	if (session->pushAV1)
		return session->pushAV1->on_spspps(session, session->pushAV1->push, p);
#endif
	return Yang_Ok;
}
...
#if	Yang_Enable_AV1_Encoding
	if (session->pushAV1)
		return session->pushAV1->on_video(session, session->pushAV1->push, p);
#endif
#if	Yang_Enable_MJpeg_Encoding
	if (session->pushMjpeg)
		return session->pushMjpeg->on_video(session, session->pushMjpeg->push, p);
#endif
	return Yang_Ok;
...
int32_t yang_rtcconn_startRtc(YangRtcSession* session,char* sdp){
	...
	if(session->context.avinfo->sys.mediaServer==Yang_Server_P2p){
		if(session->context.avinfo->rtc.iceUsingLocalIp==0 &&
			...
			yang_create_rtcudp(session->context.sock,Yang_Socket_Protocol_Udp,session->context.streamConfig->localPort);
			session->context.sock->sendData=yang_rtcconn_turn_sendData;
