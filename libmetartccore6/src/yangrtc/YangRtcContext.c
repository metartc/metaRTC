...
void yang_create_rtcContext(YangRtcContext *context) {
	...
	context->state = 0;
	context->sock = ( YangRtcSocket*) calloc(sizeof(YangRtcSocket),1);

	memset(context->sock, 0, sizeof(YangRtcSocket));

#if Yang_Enable_Dtls
	memset(&context->srtp, 0, sizeof(context->srtp));
	if(g_yang_certificate==NULL){
		g_yang_certificate = (YangCertificate*) calloc(sizeof(YangCertificate),1);
...
void yang_destroy_rtcContext(YangRtcContext *context) {
	...
	yang_destroy_rtcudp(context->sock);
