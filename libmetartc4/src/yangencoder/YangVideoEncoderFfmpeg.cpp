...
int32_t YangVideoEncoderFfmpeg::init(YangContext* pcontext) {
	...
	setVideoPara(&pcontext->avinfo.video,&pcontext->avinfo.enc);
	usingVaapi=1;
	yLen =m_videoInfo.outWidth * m_videoInfo.outHeight;
		uLen = yLen / 4;
		allLen = yLen * 3 / 2;
	...
	initParam(m_codecCtx,&pcontext->avinfo.video,&pcontext->avinfo.enc);
