...
int32_t YangDecoderOpenh264::decode(YangFrame* videoFrame,YangYuvType yuvtype,YangDecoderCallback* pcallback){
	...
	//int result = m_svcDecoder->DecodeFrame2(videoFrame->payload, videoFrame->nb, pData, &m_dstBufInfo);
	...
	if(result == 0 && m_dstBufInfo.iBufferStatus == 1){
		...
		//result = m_svcDecoder->DecodeFrame2(NULL, 0, pData, &m_dstBufInfo);
		//if(result == 0 && m_dstBufInfo.iBufferStatus == 1) writeData(videoFrame,pData,pcallback);
	}else{
