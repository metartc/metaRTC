...
void g_pushstream_sendData(void* context,char* data,int32_t nb){
	YangRtcPublish* push=(YangRtcPublish*)context;
	push->publishMsg(data, nb);
}
...
int32_t YangRtcPublish::publishMsg(char* data,int32_t nb){
    YangFrame frame;
    memset(&frame,0,sizeof(YangFrame));
    frame.nb=nb;
    frame.payload=(uint8_t*)data;
    if(m_pushs.size()>0) return m_pushs.front()->publishMsg(m_pushs.front()->context,&frame);
	return 1;
}
