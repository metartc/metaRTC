...
int32_t YangRtcReceive::publishMsg(YangFrame* msgFrame){
    if(m_recv) {
    	return m_recv->publishMsg(m_recv->context,msgFrame);
    }
	return 1;
}
...
int32_t YangRtcReceive::init(int32_t puid, char* localIp,
	...
	strcpy(m_recv->streamconfig.remoteIp,server);
	m_recv->streamconfig.remotePort = pport;
	strcpy(m_recv->streamconfig.app,app);
	strcpy(m_recv->streamconfig.stream,stream);
	m_recv->streamconfig.uid = puid;
	m_recv->streamconfig.streamOptType = Yang_Stream_Play;

	m_recv->streamconfig.recvCallback.context=this;
	m_recv->streamconfig.recvCallback.receiveAudio=g_rtcrecv_receiveAudio;
	m_recv->streamconfig.recvCallback.receiveVideo=g_rtcrecv_receiveVideo;
	m_recv->streamconfig.recvCallback.receiveMsg=g_rtcrecv_receiveMsg;

	m_recv->streamconfig.recvCallback.context=this;

	memcpy(&m_recv->streamconfig.rtcCallback,&m_context->rtcCallback,sizeof(YangRtcCallback));
	yang_create_rtcstream_handle(m_recv, &m_context->avinfo);


    m_recv->init(m_recv->context);
	return Yang_Ok;
