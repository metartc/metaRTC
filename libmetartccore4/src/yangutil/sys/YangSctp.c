...
void yang_sctp_sendData(YangSctp* sctp,char* data,int32_t nb,int32_t isBinary){
	...
	if(sctp->ordered){
		...
		if (sctp->maxPacketLifeTime != 0)
		{
			...
			//sctp->spa.sendv_prinfo.pr_value  = maxPacketLifeTime;
		}
		else if (sctp->maxRetransmits != 0)
		{
			sctp->spa.sendv_prinfo.pr_policy = SCTP_PR_SCTP_RTX;
			//sctp->spa.sendv_prinfo.pr_value  = maxRetransmits;
		}
	}
	yang_put_be32(&sctp->spa.sendv_sndinfo.snd_ppid,isBinary ? DATA_CHANNEL_PPID_BINARY : DATA_CHANNEL_PPID_STRING);
	int32_t ret = usrsctp_sendv(sctp->local_sock, data,nb, NULL, 0, &sctp->spa, sizeof(sctp->spa), SCTP_SENDV_SPA, 0);
	if(ret<0){
