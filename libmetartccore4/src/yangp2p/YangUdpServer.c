...
//
#include <yangp2p/YangUdpServer.h>
...
#endif
int32_t yang_create_udpserver(YangUdpServer *udp, char *pserverIp,int32_t listenPort,
		int32_t plocalPort) {
	if (udp == NULL)
		return 1;
	udp->lclPort = plocalPort;
	udp->serverfd = -1;
	udp->serverPort = listenPort;
...
void yang_destroy_udpserver(YangUdpServer *udp) {
    if (udp == NULL)
        return;
    yang_stop_udpserver(udp);
    if (udp->serverfd > 0) {
        closesocket(udp->serverfd);
        udp->serverfd = -1;
    }
}
int32_t yang_rtc_sendData(YangUdpServer *udp, char *data, int32_t nb) {
...
void* yang_run_udpserver_thread(void *obj) {
#ifdef _WIN32
...
#endif
	YangUdpServer *udp = (YangUdpServer*) obj;
	udp->isStart = 1;
#ifdef _WIN32
    int32_t timeout=200;
...
void yang_start_udpserver(YangUdpServer *udp) {
	if (udp == NULL)
		return;
	if (pthread_create(&udp->threadId, 0, yang_run_udpserver_thread, udp)) {
