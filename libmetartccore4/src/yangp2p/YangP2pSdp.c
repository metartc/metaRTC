...
int32_t yang_p2p_doHandleSignal(YangRtcSession* session,P2pSdpResponseType* p2p,int32_t localport, YangStreamOptType role) {
	...
	err=yang_sdp_queryP2p(session,p2p,role==Yang_Stream_Play?1:0,(char*)session->context.streamConf->remoteIp,session->context.streamConf->remotePort,apiurl, tsdp);
