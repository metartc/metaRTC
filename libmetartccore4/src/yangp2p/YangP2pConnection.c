...
int32_t yang_p2p_startRtc(YangRtcSession* session,char* sdp){
	...
	session->isServer=session->context.streamConf->isServer;

	yang_p2p_getRemoteSdp(session,sdp);
	yang_stun_createRequestStunPacket(session,session->remoteIcePwd);

	yang_create_rtcudp(session->context.udp,session->context.streamConf->remoteIp,session->isServer?0:session->listenPort, session->context.streamConf->localPort);
...
int32_t yang_p2p_getRemoteSdp(YangRtcSession* session,char* sdpstr) {
	...
	for(int32_t i=0;i<sdp.media_descs.vsize;i++){
		...
		if(yang_strcmp(desc->type,"audio")==0){
			memset(session->remote_fingerprint,0,sizeof(session->remote_fingerprint));
			memcpy(session->remote_fingerprint,desc->session_info.fingerprint,sizeof(desc->session_info.fingerprint));
		...
		if(yang_strcmp(desc->type,"application")==0) session->usingDatachannel=1;
		...
		if(!session->isServer&&desc->candidates.vsize>0){
