...
#endif
int32_t yang_create_rtcudp(YangUdpHandle *udp, char *remoteIp,int32_t remotePort,
		int32_t plocalPort) {
	if (udp == NULL)		return ERROR_RTC_UDP;
...
void yang_destroy_rtcudp(YangUdpHandle *udp) {
    ...
    pthread_mutex_destroy(&udp->lock);
}
int32_t yang_rtc_sendData(YangUdpHandle *udp, char *data, int32_t nb) {
	if (udp == NULL || !udp->isStart || udp->notRemoteInit || data==NULL)	return ERROR_RTC_UDP;
	return sendto(udp->fd, data, nb, 0, (struct sockaddr*) &udp->remote_addr,sizeof(struct sockaddr)) > 0 ? Yang_Ok : 1;
}
