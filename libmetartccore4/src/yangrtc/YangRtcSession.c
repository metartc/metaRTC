...
void g_yang_doTask(int32_t taskId, void *user) {
	...
	if (session->startRecv&&session->playStream) {
	...
	if(session->pubStream){
		if (taskId == 1) {
			if (yang_rtcpublish_check_rrt(&session->context,session->pubStream))		yang_error("check rtt Error ");
			if (yang_rtcpublish_check_twcc(&session->context,session->pubStream))		yang_error("check twcc Error ");
		}
	}

}

void yang_create_rtcsession(YangRtcSession* session,YangAVInfo* avinfo,void* user){
...
void yang_rtcsession_startTimers(YangRtcSession *session) {
	YangStreamOptType opt=session->context.streamConf->streamOptType;
...
void yang_rtcsession_receive(YangRtcSession *session, char *data, int32_t size) {
	...
	if (session->context.streamConf&&session->context.streamConf->rtcCallback.sendRequest)
		session->context.streamConf->rtcCallback.sendRequest(session->context.streamConf->rtcCallback.context,session->context.streamConf->uid, 0,Yang_Req_Connected);
	return;
}
...
void yang_rtcsession_do_request_keyframe(YangRtcSession *session, uint32_t ssrc) {
	if (session->context.streamConf&&session->context.streamConf->rtcCallback.sendRequest)
		session->context.streamConf->rtcCallback.sendRequest(session->context.streamConf->rtcCallback.context,session->context.streamConf->uid, ssrc, Yang_Req_Sendkeyframe);
...
int32_t yang_recvvideo_notify(YangRtcSession *session, YangRtcMessageType mess) {
	if(session==NULL||session->playStream==NULL||session->playStream->videoTrack==NULL) return 1;
	YangVideoRecvTrack *videorecv=session->playStream->videoTrack;
