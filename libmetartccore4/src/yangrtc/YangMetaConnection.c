...
void g_yang_mt_init(void* pcontext,YangStreamConfig* stream,void* user){
    ...
    yang_create_rtcsession(context->session, context->avinfo);

	if (context->session->context.streamConfig == NULL)	context->session->context.streamConfig = context->streamconfig;
...
int32_t g_yang_mt_connectServer(void* peer,int32_t pmediaServer){
	...
	if(pmediaServer==Yang_Server_Zlm) return yang_zlm_connectRtcServer(session);
	if(pmediaServer==Yang_Server_P2p) return yang_p2p_connectRtcServer(session);
