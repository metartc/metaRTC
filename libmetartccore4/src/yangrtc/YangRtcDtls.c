...
int32_t yang_process_dtls_data(void* user,YangRtcDtls *dtls, char *data, int32_t nb_data) {
	   /*uint16_t size = 0;
	    if (nb_data >= 13) {
...
int32_t yang_dtls_sendSctpData(YangRtcDtls* dtls,uint8_t* pdata, int32_t nb){
	...
	if(err<0){
		return yang_error_wrap(ERROR_RTC_DTLS,"send sctp dtls error!");
	}
	uint8_t *data = NULL;
	int32_t size = BIO_get_mem_data(dtls->bioOut, (char** )&data);
...
int32_t yang_dtls_receiveSctpData(YangRtcDtls* dtls,uint8_t* src, int32_t src_nb,uint8_t* dst,int32_t* dst_nb){
	int32_t err = Yang_Ok;
	err = BIO_write(dtls->bioIn, src, src_nb);

	if (err != src_nb)
	{
		yang_error("receive sctp dtls error(%d)!size=%d", err,src_nb);
	}
	int32_t r0 = SSL_read(dtls->ssl, dst, *dst_nb);
	int32_t r1 = SSL_get_error(dtls->ssl, r0);
	if(r0<=0){
		if (r1 != SSL_ERROR_WANT_READ && r1 != SSL_ERROR_WANT_WRITE) {
			return 1;
