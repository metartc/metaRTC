...
void yang_create_h265RtpEncode(YangH265RtpEncode *rtp, YangRtpBuffer* audioRtpBuffer,YangRtpBuffer* videoRtpBuffer) {
	if (rtp == NULL)
		return;
	rtp->videoSsrc = 0;
	rtp->videoSeq = 0;
...
int32_t yang_h265_package_fu_a(YangRtcSession *session, YangH265RtpEncode *rtp,
	...
	for (int32_t i = 0; i < num_of_packet; ++i) {
		...
		if(i==0){
			...
			yang_insert_twcc(&session->twcc,rtp->rtpExtension.twcc.sn);
		}
#endif
