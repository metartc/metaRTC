...
void yang_create_rtcplay(YangRtcContext *context, YangRtcPlayStream *play,
	...
	yang_create_recvaudioTrack(context, play->audio_track);
	play->video_track = (YangVideoRecvTrack*) calloc(1,	sizeof(YangVideoRecvTrack));
	yang_create_recvvideoTrack(context, play->video_track);
	...
	int32_t twcc_id = context->twcc_id;
		if (twcc_id > 0) {
			if(play->twcc==NULL) play->twcc=(YangRtcpTWCC*)calloc(1,sizeof(YangRtcpTWCC));
			play->twcc_id = twcc_id;
			play->twcc->media_ssrc=play->video_track->recv.ssrc;
...
void yang_destroy_rtcplay(YangRtcPlayStream *play) {
	...
	if(play->twcc){
		yang_destroy_YangRtpTwccVector(&play->twcc->twccs);
		yang_free(play->twcc);
	}
}
...
int32_t yang_rtcplay_send_rtcp_rr(YangRtcContext *context,
	...
	if ((err = yang_recvtrack_send_rtcp_rr(context, &play->video_track->recv))
			!= Yang_Ok) {
		return yang_error_wrap(err, "track=%u", play->video_track->recv.ssrc);
	}
	//}
	...
	if ((err = yang_recvtrack_send_rtcp_rr(context, play->audio_track))
			!= Yang_Ok) {
		return yang_error_wrap(err, "track=%u", play->audio_track->ssrc);
	}
...
#if Yang_Using_TWCC
	YangRtpTwcc twcc;
	twcc.ts=yang_get_system_time();
	twcc.sn=sn;
	if(play->twcc) yang_insert_YangRtpTwccVecotr(play->twcc,&twcc);
...
int32_t yang_rtcplay_on_rtp(YangRtcContext *context, YangRtcPlayStream *play,
	...
	int32_t err = Yang_Ok;
	 ...
	 if (play->twcc_enabled&&play->twcc) {
		 uint16_t twcc_sn = 0;
...
int32_t yang_rtcplay_check_send_nacks(YangRtcContext *context,
	...
	if ((err = yang_recvvideoTrack_check_send_nacks(context, play->video_track))
		...
		return yang_error_wrap(err, "video track=%d",
				play->video_track->recv.ssrc);
	}

	if ((err = yang_recvaudioTrack_check_send_nacks(context, play->audio_track))
			!= Yang_Ok) {
		return yang_error_wrap(err, "audio track=%d", play->audio_track->ssrc);
	}
...
int32_t yang_rtcplay_send_periodic_twcc(YangRtcContext *context,
	...
	for (int32_t i = 0; i < 1024 && play->twcc->twccs.vsize>0; ++i) {

		play->twcc->fb_pkt_count=play->twcc_fb_count;
		play->twcc_fb_count++;

		yang_send_rtcp_fb_twcc(context,play->twcc,play->twcc_fb_count);
...
int32_t yang_rtcplay_on_rtcp(YangRtcContext *context, YangRtcPlayStream *play,
	...
	if (YangRtcpType_sr == rtcp->header.type) {
		...
		if(context->streamConf->streamOptType==Yang_Stream_Both) return Yang_Ok;
		return yang_error_wrap(ERROR_RTC_RTCP_CHECK, "unknown play rtcp type=%u",rtcp->header.type);
	}
}
