...
int32_t yang_rtcpublish_on_rtcp(YangRtcContext *context,
	...
	if (YangRtcpType_rr == rtcp->header.type) {
		...
		//return yang_error_wrap(ERROR_RTC_RTCP_CHECK, "unknown publish rtcp type=%u",rtcp->header.type);
	//}
	return Yang_Ok;
}
...
int32_t yang_rtcpublish_check_rrt(YangRtcContext* context,YangRtcPublishStream* pub){
	...
	YangBandwidth* bw=&context->bandwidth;
		YangRtcCallback *rtcCallback=&context->streamConf->rtcCallback;
		...
		if(bw->lostRate>kDefaultHighLossThreshold){
			...
			if(rtcCallback->sendRequest){
                rtcCallback->sendRequest(rtcCallback->context,context->streamConf->uid,pub->videoSsrc,Yang_Req_HighLostPacketRate);
			}
            ...
            rtcCallback->sendRequest(rtcCallback->context,context->streamConf->uid,pub->videoSsrc,Yang_Req_LowLostPacketRate);
