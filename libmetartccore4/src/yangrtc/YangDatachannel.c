...
void yang_datachannel_send_message(void *pcontext,char* data,int32_t nb,int32_t isBinary){
	YangDatachannelContext* context=(YangDatachannelContext*)pcontext;

	yang_sctp_sendData(&context->sctp,data,nb,0);
}
...
void yang_datachannel_receive_msg(void* user,uint16_t streamId, uint16_t ssn, uint32_t ppid, int flags,  uint8_t* data, size_t len){
	...
	frame.payload=data;
	frame.nb=len;
	frame.uid=context->streamConf->uid;
    context->streamConf->recvcb.receiveMsg(context->streamConf->recvcb.context,&frame);
}
...
void yang_datachannelsend_dtls_msg(void* user,char* data,int32_t nb){
	...
	YangRtcContext* context=(YangRtcContext*)user;
	if(context->dtls->state!=YangDtlsStateClientDone||context->dtls->isStop==1||context->state!=1)	return;

	yang_dtls_sendSctpData(context->dtls,(uint8_t*)data,nb);
}
