...
int32_t yang_sdp_genLocalSdp(YangRtcSession *session, int32_t localport,char **psdp, YangStreamOptType role) {
	...
	if(mediaServer==Yang_Server_P2p){
		...
		strcpy(audio_media_desc->candidates.payload[0].ip,	session->context.avinfo->sys.localIp);
		...
		strcpy(video_media_desc->candidates.payload[0].ip,	session->context.avinfo->sys.localIp);
		...
		if(data_media_desc) {
			...
			strcpy(data_media_desc->candidates.payload[0].ip,	session->context.avinfo->sys.localIp);
...
int32_t yang_sdp_parseRemoteSdp(YangRtcSession* session,YangSdp* sdp){
	int32_t k=0;
		...
		if(session->context.streamConf&&session->context.streamConf->rtcCallback.setMediaConfig){
			session->context.streamConf->rtcCallback.setMediaConfig(session->context.streamConf->rtcCallback.context,session->context.streamConf->uid,session->remote_audio,session->remote_video);
		}
		return Yang_Ok;
}
