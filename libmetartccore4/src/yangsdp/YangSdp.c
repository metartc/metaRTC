...
void  yang_sdp_genLocalSdp_candidate(YangRtcSession *session,YangMediaDesc *media_desc,char* localIp){
    //candidate   
    ...
    media_desc->candidates.payload[index].port = session->context.streamConfig->localPort;
...
int32_t yang_sdp_genLocalSdp(YangRtcSession *session, int32_t localport,char **psdp, YangStreamOptType role) {
	...
	if(session->context.avinfo->rtc.usingDatachannel){
		yang_insert_YangMediaDescVector(&local_sdp->media_descs, NULL);
   ...
   if(mediaServer==Yang_Server_P2p){
        ...
        int32_t j=0;
        for(j=0;j<iplists.vsize;j++)  yang_sdp_genLocalSdp_candidate(session,audio_media_desc,iplists.payload[j]);
        for(j=0;j<iplists.vsize;j++)  yang_sdp_genLocalSdp_candidate(session,video_media_desc,iplists.payload[j]);
#if Yang_HaveDatachannel
         yang_create_YangCandidateVector(&data_media_desc->candidates);
        for(j=0;j<iplists.vsize;j++)  yang_sdp_genLocalSdp_candidate(session,data_media_desc,iplists.payload[j]);
#endif
        yang_destroy_stringVector(&iplists);
#else
        yang_sdp_genLocalSdp_candidate(session,audio_media_desc,session->context.avinfo->sys.localIp);
        yang_sdp_genLocalSdp_candidate(session,video_media_desc,session->context.avinfo->sys.localIp);
#if Yang_HaveDatachannel
        yang_create_YangCandidateVector(&data_media_desc->candidates);
        yang_sdp_genLocalSdp_candidate(session,data_media_desc,session->context.avinfo->sys.localIp);
#endif
#endif
    }
