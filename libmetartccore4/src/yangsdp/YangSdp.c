...
int32_t yang_sdp_genLocalSdp(YangRtcSession *session, int32_t localport,
	...
	//if(data_media_desc) strcpy(data_media_desc->session_info.ice_ufrag, session->isServer?session->remote_ufrag:randstr);
	...
	//if(data_media_desc) strcpy(data_media_desc->session_info.ice_pwd, session->isServer?session->remoteIcePwd:randstr);
	...
	//if(data_media_desc) strcpy(data_media_desc->session_info.fingerprint,session->isServer?session->remote_fingerprint:session->context.cer->fingerprint);
