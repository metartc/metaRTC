...
int32_t yang_get_nackinfo_index(YangRtpNackInfo* a, uint16_t value,int32_t alen){
...
int32_t yang_nackinfo_insert(YangRtpNackInfoList* vec,uint16_t value){
	...
	if(alen>=vec->capacity){
		tmp=(YangRtpNackInfo*)yang_calloc(1,sizeof(YangRtpNackInfo)*(vec->capacity+50));
...
YangRtpNackInfo* yang_recvnack_find(YangRtpRecvNack* nack,uint16_t seq)
{
	...
	for(i=0;i<nack->queue.vsize;i++){
		if(nack->queue.nackinfos[i].sn==seq)
			return &nack->queue.nackinfos[i];
	}
	return NULL;
...
void yang_recvnack_get_nack_seqs(YangRtpRecvNack* recvnack,YangRtcpNack* nack, uint32_t* timeout_nacks)
{
	...
	for(i=0;i<recvnack->queue.vsize;i++){
		seq = recvnack->queue.nackinfos[i].sn;
		nack_info = &recvnack->queue.nackinfos[i];

		alive_time = now - nack_info->generateTime;
		...
		nack_interval = yang_max(recvnack->opts.minNackInterval, recvnack->opts.nackInterval / 3);
