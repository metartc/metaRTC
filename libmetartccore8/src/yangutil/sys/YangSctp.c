...
int32_t g_sctp_conn_output(void *addr, void *buf, size_t length, uint8_t tos, uint8_t set_df)
{
	...
	if(sctp->send_dtls_msg)
		sctp->send_dtls_msg(sctp->user,buf,length);
...
void yang_sctp_sendData(YangSctp* sctp,YangFrame* msgFrame){
	...
	err = usrsctp_sendv(sctp->local_sock, msgFrame->payload,msgFrame->nb, NULL, 0, &sctp->spa, sizeof(sctp->spa), SCTP_SENDV_SPA, 0);

	if(err<0){
		yang_error("sctp send error(%d)",err);
	}
...
void yang_create_sctp(YangSctp* sctp){
	...
	err=usrsctp_set_non_blocking(sctp->local_sock, 1);
	if(err<0)
		yang_error("usrsctp_set_non_blocking() failed: %s", strerror(errno));


	linger_opt.l_onoff = 1;
	linger_opt.l_linger = 0;
	err=usrsctp_setsockopt(sctp->local_sock, SOL_SOCKET, SO_LINGER, &linger_opt, sizeof(linger_opt));
	if(err<0)
		yang_error("usrsctp_setsockopt(SO_LINGER) failed: %s", strerror(errno));


	av.assoc_value =
			SCTP_ENABLE_RESET_STREAM_REQ | SCTP_ENABLE_RESET_ASSOC_REQ | SCTP_ENABLE_CHANGE_ASSOC_REQ;

	err=usrsctp_setsockopt(sctp->local_sock, IPPROTO_SCTP, SCTP_ENABLE_STREAM_RESET, &av, sizeof(av));

	if(err<0)
		yang_error("usrsctp_setsockopt(SCTP_ENABLE_STREAM_RESET) failed: %s", strerror(errno));


	err=usrsctp_setsockopt(sctp->local_sock, IPPROTO_SCTP, SCTP_NODELAY, &on, sizeof(on));

	if(err<0)
		yang_error("usrsctp_setsockopt(SCTP_NODELAY) failed: %s", strerror(errno));
	...
	for (i = 0; i < sizeof(event_types) / sizeof(uint16_t); i++) {
		...
		err=usrsctp_setsockopt(sctp->local_sock, IPPROTO_SCTP, SCTP_EVENT, &event, sizeof(struct sctp_event));

		if(err<0)
			yang_error("usrsctp_setsockopt(SCTP_EVENT) failed: %s", strerror(errno));
	}
	...
	err=usrsctp_setsockopt(sctp->local_sock, IPPROTO_SCTP, SCTP_INITMSG, &initmsg, sizeof(struct sctp_initmsg));
	if(err<0)
		yang_error("usrsctp_setsockopt(SCTP_INITMSG) failed: %s", strerror(errno));

	err=usrsctp_bind(sctp->local_sock, (struct sockaddr*) &localConn, sizeof(localConn));
	if(err<0)
		yang_error("usrsctp_bind() failed: %s", strerror(errno));
	...
	err=usrsctp_setsockopt(sctp->local_sock, IPPROTO_SCTP, SCTP_PEER_ADDR_PARAMS, &peerAddrParams, sizeof(peerAddrParams));
	if(err<0)
		yang_error("usrsctp_setsockopt(SCTP_PEER_ADDR_PARAMS) failed: %s", strerror(errno));
