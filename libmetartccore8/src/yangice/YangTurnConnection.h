...
typedef struct{
	...
	yangbool (*isAllReady)(YangTurnSession* session);
	yangbool (*isAllocated)(YangTurnSession* session);
	int32_t (*sendData)(YangTurnSession *psession, int32_t uid,char *data, int32_t nb);
	int32_t (*addPeer)(YangTurnSession *session,int32_t uid,void* rtcSession,yang_turn_receive receive,YangIpAddress* addr);
...
void yang_create_turnConnection(YangTurnConnection* conn,void* iceSession,YangRtcSocket* sock,int32_t localport);
