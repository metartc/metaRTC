...
typedef struct{
	...
	int32_t (*encodeStunServer)(YangBuffer* stream,char* username,char* ice_pwd);
	...
	int32_t (*createResponseStunPacket)(YangStunPacket* request,YangIpAddress* address,YangIpFamilyType familyType,void* session);
	int32_t (*createRequestStunPacket)(void* psession,char* ice_pwd);
	int32_t (*getRequestStunPacket)(void* psession,char* tid,char* ice_pwd,char** requestStr,int32_t* requestLen);
	int32_t (*getResponseStunPacket)(void* psession,char* tid,YangIpAddress* remoteAddr,char** reponseStr,int32_t* responseLen);
}YangRtcStun;
...
typedef struct{
	...
	uint32_t (*encode_peer_address_ipv4)(YangIpAddress* addr,char** str);
	...
	int32_t (*encode_request)(YangBuffer* stream,char* username,char* ice_pwd);
}YangStunLib;
