...
int32_t yang_turn_encode_permission(YangTurnSession *session,
	...
	char *hostStr = NULL;
	ret = session->stun.encode_peer_address_ipv4(&peer->address,&hostStr);
...
static int32_t yang_turn_encode_request(YangTurnSession *session) {
	char tmp[1024] = { 0 };
	YangBuffer stream;
	if(session==NULL)
		return ERROR_RTC_TURN;
	...
	session->stun.encode_request(&stream, session->server->username, session->server->password);
	return session->sock->write(&session->sock->session, stream.data,
			yang_buffer_pos(&stream));

}

static int32_t yang_turn_decode(YangTurnSession *session, char *buf,
		const int32_t nb_buf) {
	uint16_t message_len;
	int32_t err = Yang_Ok;
	YangBuffer stream;
	char magic_cookie[4];
	char transcation_id[13]={0};
	...
	yang_read_bytes(&stream,transcation_id,12);
	...
	while (yang_buffer_left(&stream) >= 4) {
		...
		case StunXorRelayedAddress: {
			if(*(val+1)==1){
				session->response.address.familyType=Yang_IpFamilyType_IPV4;
				session->response.address.addr4.sin_port=yang_get_be16((uint8_t*)val+2)^ ((uint16_t)(kStunMagicCookie >> 16));
				session->response.address.addr4.sin_port=yang_ntohs(session->response.address.addr4.sin_port);
				session->response.address.addr4.sin_addr.s_addr=yang_get_be32((uint8_t*)val+4)^ kStunMagicCookie;
				session->response.address.addr4.sin_addr.s_addr=yang_ntohl(session->response.address.addr4.sin_addr.s_addr);
			}
			break;
		}
		case StunLifetime: {
		...
		case StunData: {
			yang_trace("recv turn data size=%d,", len);
			break;
		}
		default: {
			break;
...
static void g_yang_turn_receive_data(char *data, int32_t size, void *user) {
...
static int32_t yang_turnconn_sendData(YangTurnSession *psession, int32_t uid,
	...
	for (i = 0; i < psession->peers.vec.vsize; i++) {
...
static void yang_turnconn_updateNonce(YangTurnSession *session) {
	if(session==NULL)
		return ;
...
static int32_t yang_turnconn_handleError(YangTurnSession *session, char *data,
		int32_t nb) {
	uint16_t stuntype;
	...
	case StunAllocateErrorResponse:{
		if (session->state == YangTurnAllocate){
			...
			session->errorCode=YangTurnAllocateError;
			break;
		}
		if (session->state < YangTurnAllocate)
	...
	case Yang_Turn_Error_BadRequest:
		yang_warn("Yang_Turn_Error_Bad_Request");
		break;
	case Yang_Turn_Error_Unauthoorized:
		yang_turnconn_updateNonce(session);
		yang_warn("Yang_Turn_Error_Unauthoorized");
		break;
	case Yang_Turn_Error_Nonce:
		yang_turnconn_updateNonce(session);
		yang_warn("Yang_Turn_Error_Nonce");
		break;
	}
...
static int32_t yang_turnconn_handleRecv(YangTurnSession *session, char *data,
		int32_t nb) {
	uint16_t stuntype;
	int32_t i;
	int32_t err = Yang_Ok;
	...
	case StunAllocateSuccessResponse:
		...
		yang_addr_copy(&session->response.address,&session->server->turnAddress,session->sock->session.familyType);
		yang_addr_copy(&session->response.address,&session->relayAddress,session->sock->session.familyType);
		ip=(char*)yang_calloc(64,1);
		yang_addr_getIPStr(&session->relayAddress,ip,64);
		yang_trace("\nturn allocate ip=%s,turn port=%hu,lifetime=%d",ip, yang_addr_getPort(&session->relayAddress),
	...
	case StunPermissionSuccessResponse:
		for (i = 0; i < session->peers.vec.vsize; i++) {
...
static void yang_turnconn_refreshAllocate(YangTurnSession *session) {
	int32_t i;
	...
	for (i = 0; i < session->peers.vec.vsize; i++) {
		...
		if (yang_get_system_time() / YANG_UTIME_SECONDS
			...
			session->waitTime = session->turnReqInterval;
			return;
		}
	}
...
static int32_t yang_run_turnconn_app(YangTurnSession *session) {
	int32_t i;
	int32_t err = Yang_Ok;
	if(session==NULL)
		return ERROR_RTC_TURN;
...
static int32_t yang_turnconn_start(YangTurnSession *session) {
	int32_t err = Yang_Ok;
	if(session==NULL)
		return ERROR_RTC_TURN;
	if (session->isStart)
...
static void yang_turnconn_stop(YangTurnSession *session) {
	if (session == NULL)
		return;
...
static int32_t yang_turnconn_addPeer(YangTurnSession *session, int32_t uid,
		void *rtcSession, yang_turn_receive receive, YangIpAddress* addr) {
	int32_t i;
	YangTurnPeer *peer;
	for(i=0;i<session->peers.vec.vsize;i++){
		peer=session->peers.vec.payload[i];
		if(peer&&yang_addr_cmp(addr,&peer->address))
			return Yang_Ok;
	}
	peer = (YangTurnPeer*) yang_calloc(sizeof(YangTurnPeer), 1);
	yang_addr_copy(addr,&peer->address,addr->familyType);
...
yangbool yang_turnconn_isAllReady(YangTurnSession *session) {
	int32_t i;
	YangTurnPeer *peer = NULL;
	if(session==NULL||session->peers.vec.vsize==0)
		return yangfalse;

	for (i = 0; i < session->peers.vec.vsize; i++) {
		peer = session->peers.vec.payload[i];
		if (peer->state != YangTurnReady) {
			return yangfalse;
		}
	}
	return  yangtrue;
...
void yang_create_turnConnection(YangTurnConnection *conn,void* piceSession,
		YangRtcSocket *sock, int32_t localport) {
	YangTurnSession *session;
	YangIceSession* iceSession;
	YangRtcSession* rtcSession;
	char serverIp[64] = { 0 };
	if(conn==NULL || piceSession==NULL || sock==NULL)
		return;

	session = &conn->session;
	iceSession=(YangIceSession*)piceSession;
	rtcSession=(YangRtcSession*)iceSession->rtcSession;
	session->iceSession=piceSession;
	session->sock = sock;
	session->server = &iceSession->server;
	...
	session->turnReqInterval=rtcSession->context.peerInfo->rtc.turnReqInterval;
	session->waitTime = session->turnReqInterval;
	...
	conn->isAllReady = yang_turnconn_isAllReady;
