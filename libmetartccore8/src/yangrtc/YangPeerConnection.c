...
static int32_t yang_pc_onVideo(YangPeer* peer,YangPushData *videoData){
	YangRtcConnection *conn = NULL;
	if(peer==NULL||peer->conn==NULL||videoData==NULL||videoData->pushData==NULL)
		return  ERROR_RTC_PEERCONNECTION;
	conn = (YangRtcConnection*) peer->conn;
	return  conn->on_video(conn->session,videoData);
}

static int32_t yang_pc_onAudio(YangPeer* peer,YangPushData *audioData){
	YangRtcConnection *conn = NULL;
	if(peer==NULL||peer->conn==NULL||audioData==NULL||audioData->pushData==NULL)
		return  ERROR_RTC_PEERCONNECTION;
	conn = (YangRtcConnection*) peer->conn;
	return  conn->on_audio(conn->session,audioData);
}
