...
yangbool g_yang_isUser_destroy_certificate = yangfalse;

void yang_create_rtc_certificate(){
...
void yang_user_setCertificate(){
	g_yang_isUser_destroy_certificate = yangtrue;
}
void yang_destroy_user_certificate(){
...
void yang_destroy_global_certificate(){
	if(g_yang_isUser_destroy_certificate) return;
	g_yang_cerCount--;
...
void yang_create_rtcContext(YangRtcContext *context) {
	...
	context->audioEncodeCodec = Yang_AED_OPUS;
	context->videoEncodeCodec = Yang_VED_H264;
	context->audioDecodeCodec = Yang_AED_OPUS;
	context->videoDecodeCodec = Yang_VED_H264;

	context->enableAudioTrack=yangfalse;
	context->enableVideoTrack=yangfalse;

	context->audioDirection=YangInactive;
	context->videoDirection=YangInactive;

	context->audioSsrc = yang_generate_ssrc();
	context->videoSsrc = yang_generate_ssrc();

	context->h264PayloadType = YangH264PayloadType;
	context->h265PayloadType = YangH265PayloadType;
	context->audioPayloadType = YangOpusPayloadType;
	context->opusPayloadType = YangOpusPayloadType;
	context->pcmaPayloadType = YangPcmaPayloadType;
	context->pcmuPayloadType = YangPcmuPayloadType;
	...
	context->cert=(YangCertificate*) yang_get_global_certificate();
	...
	context->dtls->session.cert = context->cert;
#endif
