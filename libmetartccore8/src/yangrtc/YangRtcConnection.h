...
typedef struct{
	...
	int32_t (*addIceCandidate)(YangRtcSession *session,char* candidateStr);

	void (*close)(YangRtcSession *session);

	int32_t (*on_video)(YangRtcSession* session,YangPushData *videoData);
	int32_t (*on_audio)(YangRtcSession* session,YangPushData *audioData);
	...
	int32_t (*sendRequestPli)(YangRtcSession *session);
