...
int32_t yang_create_certificate(YangCertificate *cer) {
	...
	if (cer->cert == NULL)
		cer->cert = (mbedtls_x509_crt*) yang_calloc(sizeof(mbedtls_x509_crt),
				1);
	if (cer->pkey == NULL)
		cer->pkey = (mbedtls_pk_context*) yang_calloc(
	...
	mbedtls_x509_crt_init(cer->cert);
	mbedtls_pk_init(cer->pkey);
	ca_inited = yangtrue;
	...
	if (cer->ecdsa_mode) {
		...
		if (mbedtls_pk_setup(cer->pkey,
				...
				|| mbedtls_rsa_gen_key(mbedtls_pk_rsa(*cer->pkey),
	...
	mbedtls_x509write_crt_set_subject_key(tmpCert, cer->pkey);
	mbedtls_x509write_crt_set_issuer_key(tmpCert, cer->pkey);
	...
	if ((err = mbedtls_x509_crt_parse_der(cer->cert,
	...
	uint8_t md[128] = { 0 };
#if Yang_Mbedtls_3
	if ((err = mbedtls_sha256(cer->cert->raw.p, cer->cert->raw.len,md, 0)) != Yang_Ok) {
#else
	if ((err = mbedtls_sha256_ret(cer->cert->raw.p, cer->cert->raw.len,md, 0)) != Yang_Ok) {
#endif
	...
	ret: if (ca_inited) {
		...
		if (err) {
			mbedtls_x509_crt_free(cer->cert);
			mbedtls_pk_free(cer->pkey);
		}
	}
