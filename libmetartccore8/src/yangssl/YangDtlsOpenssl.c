...
int32_t yang_dtls_verify_callback(int32_t preverify_ok, X509_STORE_CTX *ctx) {

	return 1;
}
void dtls_ssl_on_info(const SSL *ssl, int32_t where, int32_t ret) {
...
int32_t yang_dtls_filter_data(YangDtlsSession *dtls, uint8_t *data, int32_t size) {
	int32_t err = Yang_Ok;
...
SSL_CTX* yang_build_dtls_ctx(YangDtlsSession *dtls, char *role) {
	...
	if (dtls->cert->ecdsa_mode) { // By ECDSA, https://stackoverflow.com/a/6006898
#if OPENSSL_VERSION_NUMBER >= 0x10002000L // v1.0.2
...
int32_t yang_dtls_on_handshake_done(YangDtlsSession *dtls) {
	int32_t err = Yang_Ok;
	...
	int sendKeyLen = 30;
	int recvKeyLen = 30;
	if ((err = yang_dtls_get_srtp_key(dtls, recv_key, &recvKeyLen, send_key,	&sendKeyLen)) != Yang_Ok) {
		return err;
	}
...
int32_t yang_dtls_doHandshake(YangDtlsSession *dtls) {
	if (dtls->handshake_done) {
		return Yang_Ok;
	}
	...
	yang_dtls_filter_data(dtls, data, size);
	...
	if (dtls->handshake_done) {
		yang_dtls_on_handshake_done(dtls);

	}

	return Yang_Ok;
}
int32_t yang_dtls_startHandShake(YangDtlsSession *dtls) {
	if (yang_dtls_doHandshake(dtls))
		return ERROR_RTC_DTLS;
...
int32_t yang_process_dtls_data(void* user,YangDtlsSession *dtls, char *data, int32_t nb_data) {
	...
	if ((err = yang_dtls_doHandshake(dtls) != Yang_Ok)) {
		return ERROR_RTC_DTLS;
	}
...
int32_t yang_create_rtcdtls(YangRtcDtls *pdtls,int32_t isControlled) {
	...
	dtls->isControlled = isControlled;
	...
	SSL_set_info_callback(dtls->ssl, dtls_ssl_on_info);
	...
	pdtls->doHandshake=yang_dtls_doHandshake;
	pdtls->startHandShake=yang_dtls_startHandShake;

	pdtls->processData=yang_process_dtls_data;

	pdtls->sendDtlsAlert=yang_sendDtlsAlert;
	pdtls->filterData=yang_dtls_filter_data;
	pdtls->getSrtpKey=yang_dtls_get_srtp_key;
