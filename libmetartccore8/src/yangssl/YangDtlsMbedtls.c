	...
	while (flag && dtls->bufferLen>0) {
		...
		if (ret > 0) {
			...
			if(ret == MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY){
				...
				if(!dtls->isSendAlert) dtls->sslCallback->sslAlert(dtls->sslCallback->context,dtls->uid,"warning","CN");
			}
			flag = yangfalse;
...
int32_t yang_dtls_mbed_sendCallback(void* user, const unsigned char* data, size_t len)
{
...
int32_t yang_dtls_mbed_receiveCallback(void* user, unsigned char* buf, size_t len)
{
	YangDtlsSession* dtls = (YangDtlsSession*) user;
	...
	if(dtls->bufferLen<0)
		dtls->bufferLen=0;
	if(dtls->bufferLen>0)
		yang_memmove(dtls->buffer,dtls->buffer+dataLen,dtls->bufferLen);
	return dataLen;
}
...
void yang_dtls_mbed_initDtls(YangDtlsSession* session){
	yang_trace("\n dtls is mbedtls");
	...
	// init ssl config
	if(mbedtls_ssl_config_defaults(session->sslConfig, session->isControlled ? MBEDTLS_SSL_IS_SERVER : MBEDTLS_SSL_IS_CLIENT,
	...
	if(mbedtls_ssl_conf_own_cert(session->sslConfig, session->cert->cert, session->cert->pkey)!=Yang_Ok){
	...
	mbedtls_ssl_set_bio(session->ssl, session, yang_dtls_mbed_sendCallback, yang_dtls_mbed_receiveCallback,NULL);
...
int32_t yang_create_rtcdtls(YangRtcDtls *dtls,int32_t isControlled) {
	...
	session->isControlled=isControlled;
	...
	yang_dtls_mbed_initDtls(session);
