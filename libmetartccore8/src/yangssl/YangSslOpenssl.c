//
// Copyright (c) 2019-2026 yanggaofeng guoai
//
//
...
int32_t yang_hmac_encode(const char *algo, const char *key,
		const int32_t key_length, const char *input, const int32_t input_length,
...
void yang_destroy_certificate(YangCertificate *cert) {
	if (cert == NULL)
		return;
	if (cert->eckey) {
...
static int32_t yang_ssl_gen_certificate(YangCertificate *cert) {
	cert->ecdsa_mode = 1;
	cert->pkey = EVP_PKEY_new();

	if (!cert->ecdsa_mode) { // By RSA
		RSA *rsa = RSA_new();
		if (rsa == NULL)
			return yang_error_wrap(ERROR_SSL, "Certificate rsa create fail");

		// Initialize the big-number for private key.
		BIGNUM *exponent = BN_new();
		if (exponent == NULL)
			return yang_error_wrap(ERROR_SSL, "Certificate BIGNUM create fail");
		BN_set_word(exponent, RSA_F4);
		int32_t key_bits = 1024;
		RSA_generate_key_ex(rsa, key_bits, exponent, NULL);
		if (EVP_PKEY_set1_RSA(cert->pkey, rsa) != 1)
			yang_error("EVP_PKEY_set1_RSA error");

		RSA_free(rsa);
		BN_free(exponent);
	}
	if (cert->ecdsa_mode) { // By ECDSA, https://stackoverflow.com/a/6006898
		cert->eckey = EC_KEY_new();
		...
		if (EC_KEY_set_group(cert->eckey, ecgroup) != 1)
			return yang_error_wrap(ERROR_SSL, "EC_KEY_set_group error");
		if (EC_KEY_generate_key(cert->eckey) != 1)
			return yang_error_wrap(ERROR_SSL, "EC_KEY_generate_key error");
		if (EVP_PKEY_set1_EC_KEY(cert->pkey, cert->eckey) != 1)
			return yang_error_wrap(ERROR_SSL, "EVP_PKEY_set1_EC_KEY error");
	...
	ASN1_INTEGER_set(X509_get_serialNumber(cert->cert), serial);

	X509_NAME_add_entry_by_txt(subject, "CN", MBSTRING_ASC,
			(const uint8_t*) "metaRTC", (int) yang_strlen("metaRTC"), -1, 0);

	X509_set_issuer_name(cert->cert, subject);
	X509_set_subject_name(cert->cert, subject);

	int32_t expire_day = 365;
	const long cert_duration = 60 * 60 * 24 * expire_day;

	X509_gmtime_adj(X509_get_notBefore(cert->cert), 0);
	X509_gmtime_adj(X509_get_notAfter(cert->cert), cert_duration);

	X509_set_version(cert->cert, 2);
	if (X509_set_pubkey(cert->cert, cert->pkey) != 1)
		yang_error("EVP_PKEY_set1_EC_KEY error");
	if (X509_sign(cert->cert, cert->pkey, EVP_sha1()) == 0)
		yang_error("EVP_PKEY_set1_EC_KEY error");
...
int32_t yang_create_certificate(YangCertificate *cert) {
	if (cert == NULL)
		return 1;
	int32_t err = Yang_Ok;
	cert->ecdsa_mode = yangtrue;
	// Initialize once.
	if (cert->cert) {
		return err;
	}
	...
	err = yang_ssl_gen_certificate(cert);
	...
	X509_digest(cert->cert, EVP_sha256(), md, &n);
	...
	yang_memset(cert->fingerprint, 0, sizeof(cert->fingerprint));
	yang_memcpy(cert->fingerprint, fp, yang_strlen(fp));
