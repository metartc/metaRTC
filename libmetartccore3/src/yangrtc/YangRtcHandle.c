...
void yang_rtcstream_handle_init(void* handle,void* pcontext) {
	...
	if(context->isInit==0){
        ...
        if((context->streamconfig->streamOptType==Yang_Stream_Play||context->streamconfig->streamOptType==Yang_Stream_Both)&&context->stream){
        	...
        	context->stream->setRtcMessageNotify(context->stream->context,context->streamconfig->uid,notify);
        }
