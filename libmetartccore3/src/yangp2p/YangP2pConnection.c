...
extern int32_t g_localport;
int32_t yang_p2p_getRemoteSdp(YangRtcSession* session,char* p2p);


int32_t yang_p2p_startRtc(YangRtcSession* session,char* sdp){
	g_localport=8000;
	yang_create_rtcudp(session->context.udp,session->context.streamConf->remoteIp,0, g_localport);
	session->isServer=1;
	yang_p2p_getRemoteSdp(session,sdp);
	yang_stun_createStunPacket(session,session->remoteIcePwd);
...
int32_t yang_p2p_connectRtcServer(YangRtcSession* session){
	...
	if ((err=yang_p2p_doHandleSignal(session,&sdp,g_localport,session->context.streamConf->streamOptType))  == Yang_Ok) {
