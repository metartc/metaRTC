...
int32_t yang_nackinfo_insert(YangRtpNackInfoList* vec,uint16_t value){
	...
	for(int i=0;i<alen;i++){
		if(i==alen-1){
			if(value==nacks[i]->sn) return 1;
			//if(value<a[i]){
			nackinfo=(YangRtpNackInfo*)calloc(1,sizeof(YangRtpNackInfo));
			yang_create_nackinfo(nackinfo,value);
			if(yang_rtp_seq_distance(value,nacks[i]->sn)>0){
				nacks[alen]=nacks[i];
				nacks[i]=nackinfo;
			}else{
				nacks[alen]=nackinfo;
		...
		if(nacks[i]->sn==value||nacks[i+1]->sn==value) return 1;

		nackinfo=(YangRtpNackInfo*)calloc(1,sizeof(YangRtpNackInfo));
		yang_create_nackinfo(nackinfo,value);
		if(yang_rtp_seq_distance(nacks[i]->sn,value)>0&&yang_rtp_seq_distance(value,nacks[i+1]->sn)>0){
			int left=alen-i;
			memmove((char*)nacks+(i+2)*sizeof(YangRtpNackInfo*),(char*)nacks+(i+1)*sizeof(YangRtpNackInfo*),sizeof(YangRtpNackInfo*)*left);
			nacks[i+1]=nackinfo;
