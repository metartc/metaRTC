...
int32_t yang_sdp_genLocalSdp(YangRtcSession *session, int32_t localport,
	...
	sprintf(streamnames, "%s/%s", session->context.streamConf->app,
			session->context.streamConf->stream);
	yang_insert_stringVector(&local_sdp->msids, streamnames);

	strcpy(local_sdp->group_policy,mediaServer==Yang_Server_Zlm?"BUNDLE 0 1": "BUNDLE");
...
//session info
	...
	strcpy(audio_media_desc->session_info.fingerprint,
			session->context.cer->fingerprint);
	strcpy(video_media_desc->session_info.fingerprint,
			session->context.cer->fingerprint);
...
//media pay
	...
	yang_insert_YangMediaPayloadTypeVector(&video_media_desc->payload_types,
			NULL);
	YangMediaPayloadType *videotype =
			&video_media_desc->payload_types.payload[0];
	if (session->context.avcontext->avinfo->video.videoEncoderType
			== Yang_VED_264) {
		videotype->payload_type = 125;
		strcpy(videotype->encoding_name, "H264");
	} else if (session->context.avcontext->avinfo->video.videoEncoderType
			== Yang_VED_265) {
		videotype->payload_type = 126;
		strcpy(videotype->encoding_name, "H265");
	}
	videotype->clock_rate = 90000;
	strcpy(videotype->format_specific_param,
	...
	//candidate
		...
		strcpy(audio_media_desc->candidates.payload[0].ip_,	session->context.streamConf->remoteIp);
		...
		strcpy(video_media_desc->candidates.payload[0].ip_,
				session->context.streamConf->remoteIp);
