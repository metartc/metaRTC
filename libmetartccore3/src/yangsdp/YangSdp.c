...
//session info
	...
	strcpy(video_media_desc->session_info.fingerprint,
			session->context.cer->fingerprint);
	strcpy(audio_media_desc->session_info.setup, role==Yang_Stream_Both?"passive":"actpass");
	strcpy(video_media_desc->session_info.setup, role==Yang_Stream_Both?"passive":"actpass");
