...
int32_t yang_srs_startRtc(YangRtcSession* session,char* sdp){
	yang_create_rtcudp(session->context.udp,session->context.streamConf->remoteIp,session->listenPort, g_localport);
...
int32_t yang_srs_connectRtcServer(YangRtcSession* session){
	...
	if ((err=yang_srs_getSignal(session,&srs,g_localport,session->context.streamConf->streamOptType))  == Yang_Ok) {
