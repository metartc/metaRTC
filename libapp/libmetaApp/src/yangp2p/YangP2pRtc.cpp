...
YangP2pRtc::YangP2pRtc(YangContext *pcontext) {
	...
	yang_create_YangPeerConnectionVector2(&m_pushs);
	yang_create_yangint32Vector(&m_removeList);
...
YangP2pRtc::~YangP2pRtc() {
	...
	for(int32_t i=0;i<m_pushs.vec.vsize;i++){
		m_pushs.vec.payload[i]->close(&m_pushs.vec.payload[i]->peer);
		yang_destroy_peerConnection(m_pushs.vec.payload[i]);
		yang_free(m_pushs.vec.payload[i]);
	}
	m_pushs.clear(&m_pushs.vec);

	yang_destroy_YangPeerConnectionVector2(&m_pushs);
	yang_destroy_yangint32Vector(&m_removeList);
...
int32_t YangP2pRtc::publishMsg(YangFrame* msgFrame){
	int32_t ret=0;
	for(int32_t i=0;i<m_pushs.vec.vsize;i++){
		YangPeerConnection* rtc=m_pushs.vec.payload[i];
...
int32_t YangP2pRtc::connectPeer(int32_t nettype, char* server,int32_t localPort,int32_t pport,char* app,char* stream) {
    ...
    //连接对端http服务交换sdp
	yang_p2p_getHttpSdp(server,pport,localSdp,remoteSdp);
...
int32_t YangP2pRtc::removePeer(int32_t puid){
	...
	pthread_mutex_lock(&m_mutex);
	yang_insert_yangint32Vector(&m_removeList,&puid);

	pthread_mutex_unlock(&m_mutex);
	return Yang_Ok;
}
void YangP2pRtc::checkAlive(){
	if(m_removeList.vsize>0) return;
	for(int32_t i=0;i<m_pushs.vec.vsize;i++){
		YangPeerConnection* rtc=m_pushs.vec.payload[i];
...
int32_t YangP2pRtc::erasePeer(int32_t uid){
	...
	for(int32_t i=0;i<m_pushs.vec.vsize;i++){
		...
		if(m_pushs.vec.payload[i]->peer.streamconfig.uid==uid){

			YangStreamOptType streamOpt=m_pushs.vec.payload[i]->peer.streamconfig.streamOptType;
			yang_destroy_peerConnection(m_pushs.vec.payload[i]);
			yang_free(m_pushs.vec.payload[i]);
			m_pushs.remove(&m_pushs.vec,i);
...
void YangP2pRtc::removeStream(){
	...
	for(;m_removeList.vsize>0;){
		int32_t puid=m_removeList.payload[0];
		yang_remove_yangint32Vector(&m_removeList,0);
		erasePeer(puid);
	}
}

int32_t YangP2pRtc::connectMediaServer() {
	if(m_pushs.vec.vsize>0) return Yang_Ok;
...
void YangP2pRtc::startLoop() {
	...
	while (m_isConvert == 1) {
		...
		if (m_pushs.vec.vsize == 0) {
			yang_usleep(2000);
			continue;
		}

		if(m_removeList.vsize>0){
			pthread_mutex_lock(&m_mutex);
			removeStream();
			pthread_mutex_unlock(&m_mutex);
			continue;
