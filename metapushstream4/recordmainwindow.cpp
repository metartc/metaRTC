...
#include <QSettings>
void g_qt_push_receiveData(void* context,char* data,int32_t nb){
...
{
    char* str=ui->m_t_data->text().toLatin1().data();
    if(m_context->channeldataSend.sendData)
        m_context->channeldataSend.sendData(m_context->channeldataSend.context,str,strlen(str));
}
