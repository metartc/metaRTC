...
int32_t YangStreamSrt::reconnect() {
	m_bufLen=0;
	char url[1024]={0};
	sprintf(url,"%s.sls.com/%s/%s",m_conf.streamOptType?"uplive":"live",m_conf.app,m_conf.stream);
...
int32_t YangStreamSrt::connectMediaServer(){
	...
	if(!m_srt->m_contextt){
		m_bufLen=0;
		//char s[512]={0};
		sprintf(m_conf.url,"%s.sls.com/%s/%s",m_conf.streamOptType==Yang_Stream_Publish?"uplive":"live",m_conf.app,m_conf.stream);
		//m_conf.url=s;
		m_srt->init((char*)m_conf.serverIp,m_conf.serverPort);
	}
	int32_t ret=m_srt->initConnect((char*)m_conf.url);
	if(ret) return ret;
	return m_srt->connectServer();
