...
YangVideoCaptureAndroid::YangVideoCaptureAndroid(YangVideoInfo *pcontext,void* pwindow) {
	...
	m_camera=new YangCameraAndroid((ANativeWindow*)pwindow);
...
int32_t YangVideoCaptureAndroid::init() {
	//if(m_camera) m_camera->initSurface();
	m_camera->initCamera();
	return Yang_Ok;
}
...
void YangVideoCaptureAndroid::startLoop() {
	...
	//while (m_isloop) {
		//yang_usleep(20000);
		//if(m_pre_videoBuffer->size()>0){
			//m_pre_videoBuffer->getVideo(&frame);
			//if(frame.payload) m_camera->drawFrame((float*)frame.payload);
		//}
