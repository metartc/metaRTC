...
int32_t YangEncoderMediacodec::init(YangVideoInfo *pvideo, YangVideoEncInfo *penc) {
	if (m_isInit == 1)		return Yang_Ok;

	setVideoPara(pvideo, penc);
	m_width=pvideo->outWidth;
	m_height=pvideo->outHeight;
	...
	AMediaFormat_setInt32(format,AMEDIAFORMAT_KEY_COLOR_FORMAT,19); //19 yuv420p 21 nv12 17 NV21
	AMediaFormat_setInt32(format,AMEDIAFORMAT_KEY_FRAME_RATE,pvideo->frame);
	AMediaFormat_setInt32(format,AMEDIAFORMAT_KEY_BIT_RATE,pvideo->rate*1000);
