...
void YangP2pPublish::sendKeyframe(){
...
int32_t YangP2pPublish::startAudioCapture(YangAudioPlayBuffer *pbuf) {
	if (isStartAudioCapture == 1)	return Yang_Ok;
	m_context->streams.m_playBuffers->push_back(new YangSynBuffer());
	m_context->streams.m_playBuffers->back()->m_uid=0;
	m_context->streams.m_playBuffers->back()->setInAudioBuffer(pbuf);
	if (m_capture == NULL)		m_capture=new YangP2pCapture(m_context);
	int32_t err=Yang_Ok;
